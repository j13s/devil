...
int setcornerlight(struct point *spos, unsigned int light, struct node *n,
                   int wd, int cd,
                   unsigned short *add)
{
    int j;
    ...
    if (n->d.c->walls[wd] != NULL) {
        add[wd * 4 +
            cd] = ( tmp = add[wd * 4 + cd] + (unsigned long)(s * light) ) >
                  theMaxLight ?
                  theMaxLight : tmp;
    }
                      ...
                      struct point *t)
{
    ...
    if (init_test & 8) {
        fprintf(
            ...
            t->x[0] / 65536.0 / 65536.0, t->x[1] / 65536.0 / 65536.0,
            t->x[2] / 65536.0 / 65536.0);
    }
                 ...
                 int *nc_w)
{
    ...
    if (init_test & 8) {
        fprintf(errf, "Checking p1 %g %g %g p2 %g %g %g num_s %d\n",
                p1->x[0] / 65536.0, p1->x[1] / 65536.0, p1->x[2] / 65536.0,
                p2->x[0] / 65536.0, p2->x[1] / 65536.0, p2->x[2] / 65536.0,
                num_s);
    }
    ...
    for (side = num_s - 1; side >= 0; side--) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            r.x[i] = nc[side]->d.c->p[wallpts[nc_w[side]][1]]->d.p->x[i] -
                     ea.x[i];
            s.x[i] = nc[side]->d.c->p[wallpts[nc_w[side]][3]]->d.p->x[i] -
                     ea.x[i];
            a.x[i] = p1->x[i] - ea.x[i];
        }
        ...
        if ( !checkcolltriangle(&a, &m, &r, &s, &t) ) { /* missed the triangle
                                                          at all */
            for (i = 0; i < 3; i++) {
                ea.x[i] = nc[side]->d.c->p[wallpts[nc_w[side]][2]]->d.p->x[i];
                r.x[i] =
                    nc[side]->d.c->p[wallpts[nc_w[side]][3]]->d.p->x[i] -
                    ea.x[i];
                s.x[i] =
                    nc[side]->d.c->p[wallpts[nc_w[side]][1]]->d.p->x[i] -
                    ea.x[i];
                a.x[i] = p1->x[i] - ea.x[i];
            }
            ...
            if ( !checkcolltriangle(&a, &m, &r, &s, &t) ) {
                ...
                fprintf(errf, "2. checkcolltriangle true\n");
            }
        }
        else if (init_test & 8) {
            fprintf(errf, "1. checkcolltriangle true\n");
        }
    }
...
int illum_checkwall(struct cube *c, int w) {
    ...
    switch (c->d[w]->d.d->type1) {
        case door1_blow:
        case door1_normal:
        case door1_switchwithwall:
            return 0;
            break;

        case door1_onlytxt:
        case door1_shootthrough:
            return (c->walls[w]->texture1 < pig.num_rdltxts
                   && pig.rdl_txts[c->walls[w]->texture1].pig != NULL
                   && pig.rdl_txts[c->walls[w]->texture1].txtlistno[txt1_wall
                    ] >= 0
                   && pig.rdl_txts[c->walls[w]->texture1].txtlistno[
                        txt1_normal] < 0)
                   || (c->walls[w]->texture2 < pig.num_rdltxts &&
                       c->walls[w]->texture2 != 0
                      && pig.rdl_txts[c->walls[w]->texture2].pig != NULL
                      && pig.rdl_txts[c->walls[w]->texture2].txtlistno[
                           txt2_wall] >= 0
                      && pig.rdl_txts[c->walls[w]->texture2].txtlistno[
                           txt2_normal] < 0);
...
void calclseffects(struct point *lsp, unsigned int light, struct node *cube_n,
                   int depth, struct node **nc, int *nc_w,
                   struct list *effects)
{
    ...
    for (w = 0; w < 6; w++) {
        if (c->nc[w] != NULL) {
            ...
            if (init_test & 4) {
                fprintf(errf,
                        "ILLUM: Checking cube %d from cube %d wall %d over ",
                        c->nc[w]->no, cube_n->no,
                        w);

                for (j = 0; j < depth; j++) {
                    fprintf(errf, " %d,%d", nc[j]->no, nc_w[j]);
                }

                fprintf(errf, "\n");
            }

            for (n = effects->head, lse = NULL; n->next != NULL; n =
                     n->next) {
            ...
            for (wd = 0, onevis = 0; wd < 6; wd++) {
                for (cd = 0; cd < 4; cd++) {
                        ...
                        endp.x[j] =
                            (c->nc[w]->d.c->p[wallpts[wd][(cd +
                             ...
                             c->nc[w]->d.c->p[wallpts[wd][(cd -
                                                          ...
                                                          3]]->d.p->x[j]) *
                            0.01 +
                            c->nc[w]->d.c->p[wallpts[wd][cd]]->d.p->x
                            [j] * 0.98;
                    }

                    /* check if the light can go from point p to point c->nc[w],p
                       by
                    ...
                    if ( vis && (!lse || lse->add_light[w * 4 + cd] == 0) ) {
                        onevis |= setcornerlight(lsp, light, c->nc[w], wd, cd,
                                                 add);
                    }

                    if (init_test & 4) {
                        fprintf(errf, " %d:%d:%d:%d\n", wd, cd, vis,
                                add[wd * 4 + cd]);
                    }
                }
            }
            ...
            for (j = 0; j < 24; j++) {
                /* only change if add!=0 for corners that are
                   illuminated along other ways */
                if (add[j] != 0) {
                    lse->add_light[j] = add[j];
                }
            }
            ...
            if (depth < maxdepth - 1) {
                calclseffects(lsp, light, c->nc[w], depth + 1, nc, nc_w,
                              effects);
            }
...
void calcillumwall(struct node *c, int wall, int *light) {
    int i, j, x, y, x2, y2, minx, maxx, miny, maxy, e1x, e1y, e2x, e2y, e3x,
        e3y, e4x, e4y, d1, d2,
    ...
    for (i = 0; i < 4; i++) {
        if (minx > w->corners[i].x[0])                    {
            minx = w->corners[i].x[0];
        }

        if (maxx < w->corners[i].x[0]) {
            maxx = w->corners[i].x[0];
        }

        if (miny > w->corners[i].x[1]) {
            miny = w->corners[i].x[1];
        }

        if (maxy < w->corners[i].x[1]) {
            maxy = w->corners[i].x[1];
        }
    }
            ...
            for (x = 0; x < ILLUM_GRIDSIZE; x++) {
                for (y = 0; y < ILLUM_GRIDSIZE; y++) {
                        ...
                        for (y2 = 0; y2 < ILLUM_SUBGRIDSIZE; y2++) {
                            ...
                            ox = gx + x * MAXLSCOORD / ILLUM_GRIDSIZE +
                                 (2 * x2 +
                                  1) *
                                 (MAXLSCOORD /
                                  ILLUM_GRIDSIZE) / ILLUM_SUBGRIDSIZE / 2;
                            ...
                            dp1 = px * e2y - py * e2x;
                            a = (float)dp1 / d1;
                            /* a is the factor before e1 in the linear combination
                               of p, b -> e2,
                               so a*e1+b*e2=p */
                            sub_in1[y2 * ILLUM_SUBGRIDSIZE +
                                    x2] =
                                sub_in2[y2 * ILLUM_SUBGRIDSIZE + x2] = 0;
                    ...
                    if (init_test & 4) {
                        fprintf(
                            errf,
                            "Cube %d wall %d x %d,%d y %d,%d sub_in_sum %d\n",
                    ...
                    if (sub_in_sum > ILLUM_SUBGRIDSIZE) {
                        ...
                        for (x2 = 0; x2 < ILLUM_SUBGRIDSIZE; x2++) {
                            for (y2 = 0; y2 < ILLUM_SUBGRIDSIZE; y2++) {
                                if (sub_in1[j = y2 * ILLUM_SUBGRIDSIZE +
                                                x2] || sub_in2[j]) {
                                    if (sub_in1[j]) {
                                        for (i = 0; i < 3; i++) {
                                            lsp.x[i] += w->p[0]->d.p->x[i] +
                                                        sub_a[j] * e1.x[i] +
                                                        sub_b[j] *
                                                        e2.x[i];
                                        }
                        ...
                        /* move the lightsource a bit into the middle of the cube,
                           away from
                           the wall, so we don't get trouble with flat walls */
                        for (i = 0; i < 3; i++) {
                            lsp.x[i] = lsp.x[i] / sub_in_sum * 0.99 +
                                       center.x[i] * 0.01;
                        }

                        a = sub_in_sum >= ILLUM_SUBGRIDSIZE *
                            (ILLUM_SUBGRIDSIZE - 1) ? 1.0 :
                        ...
                        if (init_test & 4) {
                            fprintf( errf, "-->a %g pos %g %g %g light %d\n",
                                    ...
                                    lsp.x[2] / 65536.0,
                                    (int)(light[y * ILLUM_GRIDSIZE + x] * a) );
                        }
                        ...
                        calclseffects(
                            &lsp,
                            (unsigned int)(light[y * ILLUM_GRIDSIZE + x] * a),
                            c,
                            0, nc,
                            nc_w, &illum_cubes);

                        for (n = illum_cubes.head->next; n != NULL;
                             n = n->next) {
                            for (ne = effects.head; ne->next != NULL;
                                 ne = ne->next) {
                                if (ne->d.lse->cube ==
                                    n->prev->d.lse->cube) {
                                    break;
                                ...
                                for (i = 0; i < 24; i++)                    {
                                    ne->d.lse->add_light[i] =
                                        (tmp = ne->d.lse->add_light[i] +
                                               n->prev->d.lse->add_light[i])
                                        > theMaxLight ? theMaxLight : tmp;
                                }
    ...
    if (init_test & 4) {
        ...
        for (n = ls->effects.head; n->next != NULL; n = n->next) {
            ...
            for (i = 0; i < 24; i++) {
                if (n->d.lse->add_light[i] != 0) {
                    fprintf(errf, " %d,%d:%d", i / 4, i % 4,
                            n->d.lse->add_light[i]);
                }
            }
                     ...
                     struct point *cmp)
{
    ...
    for (sc = 0; sc < 3; sc++) {
        ...
        if (nc->d.c->walls[w2]) {
            ...
            VECTOR(&nv, &d1, &d2);
            normalize(&nv);
            sm_corners[smc_length].smooth[sc] =
                (SCALAR(&nv, cmp) >= MAX_SMOOTHANGLE);
        }

        if ( nc->d.c->nc[w2] && illum_checkwall(nc->d.c, w2) ) {
            for (i = 0; i < smc_length; i++) {
                if (nc->d.c->nc[w2] == sm_corners[i].cube) {
                    break;
                }
            }
            ...
            if (i == smc_length) { /* new cube */
                for (p2 = 0; p2 < 8; p2++) {
                    if (nc->d.c->nc[w2]->d.c->p[p2] == nc->d.c->p[pno]) {
                        break;
                    }
                }
                ...
                if (p2 < 8) {
                    for (i = 0; i < 3; i++) {
                        if (nc->d.c->nc[w2]->d.c->nc[wallno[p2][0][i]] ==
                            nc) {
                            break;
                        }
                    }
                    ...
                    if (i < 3) {
                        new_smcl =
                            rec_getnbcorners(sm_corners, new_smcl,
                                             nc->d.c->nc[w2],
                                             wallno[p2][0][i],
                                             wallno[p2][1][i],
                                             cmp);
...
int getnbcorners(struct smoothcorner *nbcorners, struct node *c, int w,
                 int p)
{
    int i;
...
void smoothlight(void) {
    ...
    int progress, last_progress = -1, num_nbs, w, p, sum_light, num_light, i,
        j;
    ...
    for (nls = l->lightsources.head; nls->next != NULL; nls = nls->next) {
        if ( (progress = (nls->no * 100 / l->lightsources.size) / 5 * 5) !=
            last_progress ) {
        ...
        for (nlse = nls->d.ls->effects.head; nlse->next != NULL;
             nlse = nlse->next) {
            for (w = 0; w < 6; w++) {
                for (p = 0; p < 4; p++) {
                    if (!nlse->d.lse->smoothed[w * 4 + p] &&
                        nlse->d.lse->add_light[w * 4 + p]) {
                        if (init_test & 4) {
                            fprintf(
                                errf,
                                "Smoothing cube %d,wall %d,corner %d (light %d)\n",
                                nlse->d.lse->cube->no, w, p,
                                nlse->d.lse->add_light[w * 4 + p]);
                        }

                        num_nbs =
                            getnbcorners(nbcorners, nlse->d.lse->cube, w,
                                         p);

                        for (i = 0, sum_light = 0, num_light = 0; i < num_nbs;
                             i++) {
                            if (init_test & 4) {
                                fprintf(
                                    errf,
                                    ...
                                    i,
                                    nbcorners[i].cube->no,
                                    nbcorners[i].w[0], nbcorners[i].w[1],
                                    nbcorners[i].w[2], nbcorners[i].c[0],
                                    nbcorners[i].c[1],
                                    nbcorners[i].c[2],
                                    nbcorners[i].smooth[0],
                                    nbcorners[i].smooth[1],
                            ...
                            if (nbcorners[i].cube == nlse->d.lse->cube) {
                                ...
                                for (nlse2 = nls->d.ls->effects.head;
                                     nlse2->next != NULL;
                                     nlse2 = nlse2->next) {
                                    if (nlse2->d.lse->cube ==
                                        nbcorners[i].cube) {
                            ...
                            for (j = 0; j < 3; j++) {
                                if (nbcorners[i].smooth[j]
                                   && nbcorners[i].cube->d.c->walls[nbcorners
                                                                    [i].w[j]])
                                {
                                    sum_light +=
                                        nbcorners[i].lse->add_light[nbcorners
                                                                    [i].w
                                                                    [j] * 4 +
                                                                    nbcorners
                                                                    [i].c[j]];
                        ...
                        if (num_light > 1) {
                            ...
                            for (i = 0; i < num_nbs; i++) {
                                if (nbcorners[i].lse) {
                                    for (j = 0; j < 3; j++) {
                                        if (nbcorners[i].smooth[j]
                                           && nbcorners[i].cube->d.c->walls[
                                                nbcorners[i].w[j]]) {
                                            nbcorners[i].lse->smoothed[
                                                nbcorners[i].w[j] * 4 +
                                                nbcorners
                                                [i].c[j]] = 1;

                                            if (nbcorners[i].cube->d.c->walls
                                                [nbcorners[i].w[j]]) {
                                                if (init_test & 4) {
                                                    fprintf(
                                                        errf,
                                                        "Smoothing with cube %d(==%d) wall %d corner %d\n",
                                                        nbcorners[i].lse
                                                        ->cube->no,
                                                        nbcorners[i].cube->no,
                                                        nbcorners[i].w[j],
...
void calccornerlight(int withsmooth) {
    ...
    int ldrawn, per, w, c, i, j, x, y, light[ILLUM_GRIDSIZE * ILLUM_GRIDSIZE],
        overall;
    clock_t time1;
    struct ws_event ws;
    ...
    time1 = clock();
    sqr_qw_mw =
        (view.illum_quarterway - MINWAY) * (view.illum_quarterway - MINWAY);
    ...
    for (ntc = l->tagged[tt_cube].head, per = 0, ldrawn = -10;
         ntc->next != NULL;
         ntc = ntc->next, per++) {
        if ( (clock() - time1) / CLOCKS_PER_SEC > ldrawn ) {
            ws_getevent(&ws, 0);
            printmsg(TXT_CALCLIGHT, ntc->d.n->no,
                     (per * 100) / (float)l->tagged[tt_cube].size,
                     (clock() - time1) /
        ...
        for (w = 0; w < 6; w++) {
            if (ntc->d.n->d.c->walls[w] != NULL) {
                ...
                if (ntc->d.n->d.c->walls[w]->ls) { /* an old lightsource. Delete
                                                     it but keep the fl */
                    if (ntc->d.n->d.c->walls[w]->ls->d.ls->fl) {
                        fl = *ntc->d.n->d.c->walls[w]->ls->d.ls->fl;
                    }

                    freenode(&l->lightsources, ntc->d.n->d.c->walls[w]->ls,
                             freelightsource);
                }
                ...
                if (ntc->d.n->d.c->walls[w]->texture1 < pig.num_rdltxts
                   && pig.rdl_txts[ntc->d.n->d.c->walls[w]->texture1].pig !=
                    NULL) {
                    for (i = 0; i < ILLUM_GRIDSIZE * ILLUM_GRIDSIZE; i++) {
                        overall += (light[i] =
                                        pig.rdl_txts[ntc->d.n->d.c->walls[w]
                                                     ->texture1].my_light[i]);
                    }
                }

                if (ntc->d.n->d.c->walls[w]->texture2 < pig.num_rdltxts
                   && pig.rdl_txts[ntc->d.n->d.c->walls[w]->texture2].pig !=
                    NULL
                    ...
                    for (i = 0; i < ILLUM_GRIDSIZE; i++) {
                        for (j = 0; j < ILLUM_GRIDSIZE; j++) {
                            switch (ntc->d.n->d.c->walls[w]->txt2_direction)
                            {
                                ...
                                case 1:
                                    y = ILLUM_GRIDSIZE - 1 - j;
                                    x = i;
                                    break;

                                case 2:
                                    y = ILLUM_GRIDSIZE - 1 - i;
                            ...
                            overall += (light[y * ILLUM_GRIDSIZE + x] +=
                                            pig.rdl_txts[ntc->d.n->d.c->walls
                                                         [w]->texture2].
                                            my_light[
                                                i * ILLUM_GRIDSIZE + j]);
                        }
                ...
                if (overall > 0) {
                    ...
                    if (fl.delay > 0 && ntc->d.n->d.c->walls[w]->ls) {
                        checkmem(
                             ntc->d.n->d.c->walls[w]->ls->d.ls->fl = MALLOC(
                                sizeof(struct flickering_light) ) );
                        *ntc->d.n->d.c->walls[w]->ls->d.ls->fl = fl;
                        ntc->d.n->d.c->walls[w]->ls->d.ls->fl->ls =
                            ntc->d.n->d.c->walls[w]->ls;
                    }
                }
            }
        }
    ...
    for (ntc = l->tagged[tt_cube].head; ntc->next != NULL; ntc = ntc->next) {
        for (w = 0; w < 6; w++) {
            if (ntc->d.n->d.c->walls[w] != NULL) {
                for (c = 0; c < 4; c++) {
                    ntc->d.n->d.c->walls[w]->corners[c].light =
                        view.illum_minvalue;
                }
            }
        }
    ...
    for (ntc = l->lightsources.head; ntc->next != NULL; ntc = ntc->next) {
        ...
        for (nlse = ntc->d.ls->effects.head; nlse->next != NULL;
             nlse = nlse->next) {
            for (i = 0; i < 6; i++) {
                if (nlse->d.lse->cube->d.c->walls[i]) {
                    for (j = 0; j < 4; j++) {
                        if ( (overall = nlse->d.lse->add_light[i * 4 + j]) !=
                            0 ) {
                            if (init_test & 4) {
                                fprintf(errf, " %d,%d,%d:%d",
                                        nlse->d.lse->cube->no,
                            ...
                            if (ntc->d.ls->cube->d.c->tagged) {
                                overall +=
                                    nlse->d.lse->cube->d.c->walls[i]->corners
                                    [j].
                                    light;
                                nlse->d.lse->cube->d.c->walls[i]->corners[j].
                                light =
                                    overall >
                                    theMaxLight ? theMaxLight : overall;
                            }

                            if (ntc->d.ls->fl != NULL) {
                                checkmem( addnode(&nlse->d.lse->cube->d.c->
                                                  fl_lights, -1,
                                                  ntc->d.ls->fl) );
                            }
                        }
...
void setinnercubelight(void) {
    ...
    for (ntc = l->tagged[tt_cube].head->next; ntc != NULL; ntc = ntc->next) {
        ...
        for (w = 0; w < 6; w++) {
            if (ntc->prev->d.n->d.c->walls[w] != NULL) {
                ...
                for (c = 0; c < 4; c++) {
                    overall +=
                        ntc->prev->d.n->d.c->walls[w]->corners[c].light;
                }
            }
        }
    ...
    while (l->tagged[tt_cube].head->next != NULL && c++ < 5) {
        for (ntc = l->tagged[tt_cube].head->next; ntc != NULL;
             ntc = ntc->next) {
            overall = 0;
            i = 0;

            for (w = 0; w < 6; w++) {
                if (ntc->prev->d.n->d.c->nc[w] &&
                    !ntc->prev->d.n->d.c->nc[w]->d.c->tagged) {
...
void dec_setlsfile(int ec) {
    ...
    if ( ( new_ls_name =
              getfilename(&path, old_ls_name, ext, TXT_SETLSFILE,
                          0) ) == NULL ) {
        return;
    }
...
void start_adjustlight(struct wall *wall) {
    ...
    for (nl = l->lightsources.head; nl->next != NULL; nl = nl->next) {
        for (n = nl->d.ls->effects.head; n->next != NULL; n = n->next) {
            ...
            for (w = 0; w < 6; w++) {
                if (n->d.lse->cube->d.c->walls[w]) {
                    for (c = 0; c < 4; c++) {
                        if ( (sum = -n->d.lse->add_light[w * 4 + c]) != 0 ) {
                            sum +=
                                n->d.lse->cube->d.c->walls[w]->corners[c].
                                light;
                            n->d.lse->cube->d.c->walls[w]->corners[c].light =
                                sum < 0 ? 0 : sum;
                        }
                    }
                }
                    ...
                    for (c = 0; c < 4; c++) {
                        if (n->d.c->walls[w]->corners[c].light) {
                            if (!lse) {
                                checkmem( lse =
                                             MALLOC( sizeof(struct ls_effect) ) );
                                checkmem( addnode(&wall->ls->d.ls->effects,
                                                  -1, lse) );
                            }
