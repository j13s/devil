...
void dec_usecubetag(int ec) {
    ...
    if (view.currmode != tt_wall && view.currmode != tt_pnt &&
        view.currmode != tt_edge) {
        printmsg(TXT_MODEUSECUBETAG);
        return;
    }
...
void dec_usesidetag(int ec) {
    ...
    for (n = l->tagged[tt_wall].head; n->next != NULL; n = n->next) {
        for (j = 0; j < 4; j++) {
            if (view.currmode == tt_pnt) {
                tag(tt_pnt, n->d.n->d.c->p[wallpts[n->no % 6][j]]);
            }
            else {
                tag(tt_edge, n->d.n, n->no % 6, j);
            }
        }
    }
...
void dec_usepnttag(int ec) {
    ...
    for (n = l->tagged[tt_pnt].head; n->next != NULL; n = n->next) {
        for (n2 = n->d.n->d.lp->c.head; n2->next != NULL; n2 = n2->next) {
            for (j = 0; j < 3; j++) {
                tag(tt_edge, n2->d.n, wallno[n2->no][0][j],
                    wallno[n2->no][1][j]);
            }
        }
    }
...
void dec_tagspecial(int ec) {
    ...
    switch (view.currmode) {
            ...
            for (n =
                ...
                for (i = 0; i < 3; i++) {
                    if (n->d.n->d.c->walls[wallno[n->no][0][i]]) {
                        switch_tag(tt_edge, n->d.n, wallno[n->no][0][i],
                                   wallno[n->no][1][i]);
                    }
                }
            }
        ...
        case tt_pnt:
            ...
            for (n = view.pcurrpnt->d.lp->c.head; n->next != NULL;
                 n = n->next) {
                for (i = 0; i < 3; i++) {
                    if (n->d.n->d.c->walls[wallno[n->no][0][i]]) {
                        switch_tag(tt_edge, n->d.n, wallno[n->no][0][i],
