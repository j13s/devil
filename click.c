...
void init_beam(struct point *x0, struct point *v, struct w_window *w, int sx,
               int sy)
{
    int xs, k;
    ...
    for (k = 0; k < 3; k++) {
        v->x[k] = e0.x[k] + x * er[0].x[k] + y * er[1].x[k] - x0->x[k];
    }
}
...
float checkpnt(struct point *a, struct point *m, struct point *p) {
    ...
    if (t < view.minclicksodist || t > view.maxvisibility * 1.2) {
        return 10.0;
    }
...
struct clickhit *checkclick(struct point *p, int clickradius, int x, int y,
                            struct node *data,
                            int wall)
{
    struct pixel pix;
    ...
    if ( !in_getpixelcoords(p, &pix)
       || (pix.d > view.maxvisibility * 1.1 && wall != 7) ) {
        return NULL;
    }
...
void add_clickhit(struct clickhit **nearest, struct clickhit *ch) {
    ...
    for (i = 0; i < MAX_CLICKHITS; i++) {
        if (!nearest[i] || nearest[i]->d > ch->d) {
            if (nearest[MAX_CLICKHITS -
                        1])                                          {
                FREE(nearest[MAX_CLICKHITS - 1]);
            }

            for (j = MAX_CLICKHITS - 1; j > i; j--) {
...
void run_thingloop(struct clickhit **hits, int clickradius, int x, int y) {
    ...
    for (n = l->things.head; n->next != NULL; n = n->next) {
        if ( ( ch =
                  checkclick(&n->d.t->p[0], clickradius, x, y, n,
                             -1) ) != NULL ) {
...
void run_doorloop(struct clickhit **hits, int clickradius, int x, int y) {
    ...
    for (n = l->doors.head; n->next != NULL; n = n->next) {
        if ( ( ch =
                  checkclick(&n->d.d->p, clickradius, x, y, n,
                             -1) ) != NULL ) {
            add_clickhit(hits, ch);
        }
    }
...
void run_pntloop(struct clickhit **hits, int clickradius, int x, int y) {
    ...
    for (n = l->pts.head; n->next != NULL; n = n->next) {
        if ( ( ch =
                  checkclick(n->d.p, clickradius, x, y, n, -1) ) != NULL ) {
            add_clickhit(hits, ch);
        }
    }
}
    ...
    for (n = l->cubes.head; n->next != NULL; n = n->next) {
        for (w = 0; w < 6; w++) {
            for (i = 0; i < 3; i++) {
                ...
                for (j = 0; j < 4; j++) {
                    p.x[i] += n->d.c->p[wallpts[w][j]]->d.p->x[i] / 4.0;
                }
            }
...
void run_edgeloop(struct clickhit **hits, int clickradius, int x, int y) {
    ...
    for (n = l->cubes.head; n->next != NULL; n = n->next) {
        for (w = 0; w < 6; w++) {
            if (!n->d.c->nc[w] || n->d.c->nc[w]->no > n->no) {
                for (i = 0; i < 3; i++) {
                    c.x[i] = 0.0;

                    for (j = 0; j < 4; j++) {
                        c.x[i] += n->d.c->p[wallpts[w][j]]->d.p->x[i] / 4.0;
                    }
                }
                ...
                for (e = 0; e < 4; e++) {
                    for (i = 0; i < 3; i++) {
                        p.x[i] = c.x[i] * 0.4 +
                                 n->d.c->p[wallpts[w][e]]->d.p->x[i] * 0.6;
                    }

                    if ( ( ch =
                              checkclick(&p, clickradius, x, y, n,
                                         w * 4 + e + 100) ) != NULL ) {
                        add_clickhit(hits, ch);
                    }
                }
...
void plotclickmarker(int lr, struct clickhit *ch, int color, int xor) {
    ...
    if (ch->wall == 6) { /* cube */
        for (i = 0; i < 8; i++) {
            if ( getscreencoords(lr, &ch->p, ch->data->d.c->p[i]->d.p, &spix,
                                 &epix, 0) ) {
                ws_drawline(w_xwinincoord(l->w,
                                          spix.x), w_ywinincoord(l->w, spix.y),
                            w_xwinincoord(l->w,
                                          epix.x), w_ywinincoord(l->w, epix.y),
        ...
        for (i = 0; i < 4; i++) {
            if ( getscreencoords(lr, &ch->p,
                                 ch->data->d.c->p[wallpts[ch->wall][i]]->d.p,
                                 &spix, &epix, 0) ) {
                ws_drawline(w_xwinincoord(l->w,
                                          spix.x), w_ywinincoord(l->w, spix.y),
                            w_xwinincoord(l->w,
        ...
        for (i = 0; i < 4; i++) {
            if ( i != ( ( (ch->wall - 100) % 4 + 2 ) & 3 ) ) {
                if ( getscreencoords(lr, &ch->p,
                                     ch->data->d.c->p[wallpts[(ch->wall -
                                                               100) /
                                                              4][i]]->d.p,
                                     &spix, &epix, 0) ) {
                    ws_drawline(w_xwinincoord(l->w,
                                              spix.x),
                                w_ywinincoord(l->w, spix.y),
                                w_xwinincoord(l->w,
                                              epix.x),
                                w_ywinincoord(l->w, epix.y),
                                view.color[color], xor);
                }
            }
        }
...
void(*run_loop[tt_number]) (struct clickhit **hits, int cr, int x,
                            int y) = {
    run_cubeloop, run_wallloop, run_edgeloop, run_pntloop, run_thingloop,
    run_doorloop
};
...
struct track *check_for_corr(struct w_window *w, int sx, int sy) {
    ...
    if (l->cur_corr != NULL) {
        for (t = l->cur_corr->tracking.head; t->next != NULL; t =
                 t->next)                               {
            if (t->d.ct->fixed <= 0 &&
                ( f = checkeverypnt(&a, &m, &t->d.ct->x) ) < min_f) {
                min_f = f;
                min_t = t;
            }
        }
...
void scan_setclipping(struct w_window *w, int onoff) {
    ...
    if (!l->whichdisplay) {
        ws_setclipping( w_xwinincoord(w, 0), w_ywinincoord(w, 0),
                       w_xwinincoord( w,
                                     w_xwininsize(w) ),
                       w_ywinincoord( w, w_ywininsize(w) ) );
    }
    else if (lr) {
        ws_setclipping( w_xwinincoord(w,
                                      w_xwininsize(w) / 2),
                       w_ywinincoord(w, 0),
                       w_xwinincoord( w,
                                     w_xwininsize(w) ),
...
void start_scansequence(struct w_window *w, int wx) {
    ...
    if (clickradius < view.clickradius) {
        clickradius = view.clickradius;
    }
}


struct clickhit *newpos_scansequence(
    struct w_window *w, int wx, int wy,
    void (*run_loop)(struct clickhit **nearest, int cr, int x,
                     int y),
...
void click_in_level(struct w_window *w, struct w_event *we) {
    ...
    do {
        first = newpos_scansequence(w, wx, wy, run_loop[view.currmode],
                                    nearest);
        ...
        if ( (ws.kbstat & ws_ks_ctrl) == 0 && t == 1 && nearest[0]
           && nearest[0]->data == ctrl_pressed && nearest[0]->wall !=
            7 /* no corr */ ) {
            ctrl_pressed = NULL;
            ...
            switch (view.currmode) {
                ...
                case tt_wall:
                    switch_tag(tt_wall, nearest[0]->data, nearest[0]->wall);
                    break;

                case tt_edge:
                    switch_tag(tt_edge, nearest[0]->data,
                               (nearest[0]->wall - 100) / 4,
                               (nearest[0]->wall - 100) % 4);
                    break;
        ...
        for (i = 0; i < MAX_CLICKHITS && nearest[i] != NULL; i++) {
            plotclickmarker(lr, nearest[i], i ==
                            0 ? HILIGHTCOLORS : HILIGHTCOLORS + 3,
                            1);
        }

        ws_drawcircle(x, y, clickradius,
                      view.color[t ? HILIGHTCOLORS + 2 : WHITE], 1);

        do {
            ws_getevent(&ws, 0);
        } while ( ws.x == x && ws.y == y && ws.flags == ws_f_none
                && t == ( (ws.kbstat & ws_ks_ctrl) != 0 ) );
        ...
        if ( (ws.buttons & ws_bt_right) != 0 && nearest[0] != NULL ) { /* move
                                                                         or rotate
                                                                         */
            right_pressed = 1;
            scan_setclipping(w, 0);
            ...
            if (nearest[0]->wall == 7) {
                ...
                moveobj_mouse(
                     ( (ws.kbstat & ws_ks_ctrl) != 0 ? 1 : 0 ) |
                    ( (ws.kbstat & ws_ks_alt) != 0 ? 2 : 0 ),
                    w_xscreencoord(w, ws.x),
                    w_yscreencoord(w,
                                   ws.y), nearest[0]->data, nearest[0]->wall );
            }

            plotlevel();

            if (nearest[0]->wall != 7) {
                if (view.currmode == tt_thing) {
                    drawopt(in_thing);
                }
        ...
        if (ws.buttons != ws_bt_none) {
            ...
            else if ( wx > (lr ? xsize * 2 : xsize) ) {
                wx = (lr ? xsize * 2 : xsize) - 1;
            }
    ...
    if (nearest[0] == NULL || nearest[0]->wall == 7) {
        return;
    }
    ...
    switch (view.currmode) {
        ...
        case tt_edge:
            ...
            printmsg(TXT_CLICKEDEDGE, view.pcurrcube->no, view.currwall,
                     view.curredge);
            break;
