...
void *D1_REG_readsdoor(FILE *lf) {
    ...
    switch (D1_sd.flags & 0x149) {
        case 0x00:
            sd->type = switch_nothing;
            break;

        case 0x01:
            sd->type = switch_opendoor;
            break;

        case 0x40:
            sd->type = switch_producer;
            break;
        ...
        case 0x100:
            sd->type = switch_secretexit;
            break;

        default:
            fprintf(errf, "Unknown sdoor %lx: %x\n", ftell(lf), D1_sd.flags);
            my_assert(0);
    }

    if (init_test & 2) {
        fprintf(errf,
                "read sdoor: type: %hx flags: %hx num: %hd v: %lx l: %lx\n",
                sd->type, sd->flags, sd->num, sd->value,
                sd->link_num);

        for (i = 0; i < 10; i++) {
            fprintf(errf, " %hd,%hd", sd->cubes[i], sd->walls[i]);
        }
...
void *readthing(FILE *lf) {
    ...
    if (fread(&t->type1, sizeof(struct thing) -
              ( (unsigned char *)&t->type1 -
               (unsigned char *)t ), 1,
              lf) != 1) {
        FREE(t);
        return NULL;
    ...
    size =
        getsize(ds_thing,
                t) + (unsigned char *)&t->type1 - (unsigned char *)t;

    if (init_test & 2) {
        fprintf(errf, "%x %x size: %ld pos: %ld %ld %ld cube: %hd\n",
                (int)t->type1, (int)t->type2, (long)getsize(ds_thing,
                                                            t), t->pos[0],
    ...
    switch (t->type1) {
        case tt1_coopstart:
            ...
            if (t->size == 0) {
                t->size = stdcoopstart.t.size;
            }

            if (t->movement == mt_none) {
                special = 1;
            }
        ...
        case tt1_robot:
            ...
            if (t->size == 0) {
                t->size = (t->type2 < NUM_ROBOTS) ? robotsize[t->type2] :
                          D2_stdrobot.t.size;
            }
        ...
        default:
            waitmsg("Unknown thing-type: %.2x.", (unsigned)t->type1);
            t->color = 2;
            ...
            switch (t->control) {
                case ct_mine:
                    size += sizeof(struct control_mine);
                    break;

                case ct_ai:
                    size += init.d_ver < d2_10_sw ?
                            sizeof(struct D1_control_ai) : sizeof(struct
                                                                  D2_control_ai);

                    if (init.d_ver >= d2_10_sw && loading_level_version <
                        d2_10_sw) {
                        special = 3;
                    }
                ...
                case ct_powerup:
                    size += sizeof(struct control_powerup);
                    break;
                ...
                default:
                    waitmsg(
                        "Unknown thing control-type: %.2x.\n" \
                        "Can't load level complete.",
                        (unsigned)t->control);
                    FREE(t);
                    return NULL;
            }
            ...
            switch (t->movement) {
                ...
                case mt_physics:
                    size += sizeof(struct movement_physics);
                    break;

                case mt_spinning:
                    size += sizeof(struct movement_spinning);
                    break;

                default:
                    waitmsg(
                        "Unknown thing movement-type: %.2x.\n" \
                        "Can't load level complete.",
                        (unsigned)t->movement);
                    FREE(t);
                    return NULL;
            ...
            switch (t->render) {
                case rt_polyobj:
                    size += sizeof(struct render_polyobj);
                    break;
                ...
                default:
                    waitmsg("Unknown thing render-type: %.2x.\n" \
    ...
    switch (special) {
        case 1: /* a start with movement mt_none */
            ...
            if (fread( (unsigned char *)t + sizeof(struct thing) +
                      sizeof(struct movement_physics), size -
                      sizeof(struct thing) -
        ...
        case 3: /* loading a Descent 1 level in Descent 2 mode */
            ...
            if (fread(&D1_r.m, sizeof(struct D1_robot) - sizeof(struct thing),
                      1, lf) != 1) {
                FREE(t);
                return NULL;
            }
            ...
            for (j = 0; j < MAX_AIFLAGS; j++) {
                D2_r->c.flags[j] = D1_r.c.flags[j];
            }
    ...
    if (j != 3) {
        for (j = 0; j < 3; j++) {
            for (i = 0; i < 3; i++) {
                t->orientation[j * 3 + i] = view.e[j].x[i] * 65536;
            }
        }
    }
    ...
    if (init_test & 2) {
        ...
        for (j = &t->type1 - (unsigned char *)t; j < size; j++) {
            if ( j == sizeof(struct thing) ) {
                fprintf(errf, " / ");
            }

            fprintf( errf, " %.2x", *( (unsigned char *)t + j ) );

            if ( ( j - (&t->type1 - (unsigned char *)t) ) % 20 == 19 ) {
                fprintf(errf, "\n");
            }
        }
...
struct edoor *readedoors(FILE *f, unsigned long num, unsigned long size) {
    ...
    if (num > 1) {
        waitmsg("Only one end door struct allowed. Ignoring the rest.");
    }

    if ( size != sizeof(struct edoor) ) {
        waitmsg("Wrong end door struct size.");
    }
...
void *readchangedlight(FILE *lf) {
    ...
    if (init_test & 2) {
        fprintf(errf, "Read changed light: %hd %d %d / %d %d %d %d\n",
                cl->cube,
                (int)cl->side, (int)cl->stuff, (int)cl->sub[0],
                (int)cl->sub[1],
                (int)cl->sub[2],
                (int)cl->sub[3]);
    }
...
void *D1_REG_readcube(FILE *lf) {
    ...
    if (init_test & 2) {
        fprintf(errf, "Read cube (%lx): %x ", ftell(lf),
                (int)controlbyte);
    }

    for (j = 0; j < 6; j++) {
        if ( ( controlbyte & (1 << j) ) != 0 ) {
            if (fread(&c->nextcubes[j], sizeof(unsigned short int), 1,
                      lf) != 1) {
                FREE(c);
                return NULL;
            }
    ...
    if (init_test & 2) {
        fprintf(errf, "%hx %hx %hx %x %hd %hd %hd %hd %hd %hd %hd %hd\n",
                c->type,
                c->prodnum, c->value, (int)controlbyte, c->pts[0], c->pts[1],
                c->pts[2],
                c->pts[3], c->pts[4], c->pts[5], c->pts[6],
                c->pts[7]);
    }
...
void *D2_REG_readcube(FILE *lf) {
    ...
    if (init_test & 2) {
        fprintf(errf, "Offset %lx Read cube: %x ",
    ...
    if (init_test & 2) {
        fprintf(errf, "%hx %hx %hx %hx %hx %hx\n", c->nextcubes[0],
                c->nextcubes[1],
                c->nextcubes[2], c->nextcubes[3], c->nextcubes[4],
                c->nextcubes[5]);
...
int D2_REG_readcube2(FILE *lf, struct node *n) {
    ...
    if (init_test & 2) {
        fprintf(
            ...
            n->no, n->d.c->type, n->d.c->prodnum, n->d.c->value,
            n->d.c->flags,
            light, n->d.c->light);
    }
...
int readlist(FILE *f, struct list *l, void *(*readdata)(FILE *), int num) {
    ...
    for (i = 0; i < n; i++) {
        if (init_test & 2) {
            fprintf(errf, "Reading list entry %d (%d)\n", i, n);
        }

        if ( ( d = readdata(f) ) == NULL ) {
            return 0;
        }
...
int D1_REG_savecube(FILE *f, struct node *n, va_list args) {
    ...
    if (exitcube != NULL && n->no == exitcube->no) {
        cbnext |= 1 << exitwall;
    }
    ...
    if ( cbnext & (1 << 6) ) {
        if (fwrite(&c->type, sizeof(char), 4, f) != 4
           || fwrite(&c->light, sizeof(short), 1, f) != 1) {
            return 0;
        }
    }
    else {
        if (fwrite(&c->light, sizeof(short int), 1, f) != 1)           {
            return 0;
        }
    }
    ...
    for (j = 0; j < 6; j++) {
        if (c->nextcubes[j] == 0xffff || c->doors[j] != 0xff) {
            ...
            if (c->walls[j]->texture2 != 0) {
                t1 |= 0x8000;
                t2 = c->walls[j]->texture2 |
                     (c->walls[j]->txt2_direction << 14);
            }
            else {
                t2 = 0;
            }
            ...
            if (t2 != 0) {
                if (fwrite(&t2, sizeof(short int), 1, f) != 1) {
                    return 0;
                }
            }
...
int readasciicube(struct leveldata *ld, FILE *f) {
    ...
    if (fscanf(f, " segment %d", &segnr) != 1 || segnr != ld->cubes.size) {
        return 0;
    }

    for (sidenr = 0; sidenr < 6; sidenr++) {
        if (fscanf(f, " side %d", &i) != 1 || i != sidenr) {
            return 0;
        }

        if (fscanf(f, " tmap_num %d tmap_num2 %d", &t1, &t2) != 2) {
            return 0;
        }
    ...
    if (fscanf(f, " children %d %d %d %d %d %d", &child[0], &child[1],
               &child[2],
               &child[3], &child[4], &child[5]) != 6) {
        return 0;
    }

    for (i = 0; i < 8; i++) {
        if (fscanf(f, " vms_vector %d %g %g %g", &j, &pnts[i].x[0],
                   &pnts[i].x[1],
                   &pnts[i].x[2]) != 4 || j != i) {
            return 0;
        }
    ...
    if (fscanf(f, " static_light %d", &slight) != 1) {
        return 0;
    }
