...
{
    if (transparent) {
        psys_256_transparent_plottxt(p, start, w_ywinincoord(l->w,
                                                             0) * init.xres +
                                     w_xwinincoord(l->w,
                                                   lr ? w_xwininsize(l->w) /
                                                   2 + 1 : 0), txt);
        ...
        psys_256_plottxt(p, start, w_ywinincoord(l->w, 0) * init.xres +
                         w_xwinincoord(l->w, lr ? w_xwininsize(l->w) / 2 +
                                       1 : 0), txt);
    }
}
...
void plotline(int o_x1, int o_y1, int o_x2, int o_y2, int color, int xor) {
    psys_plotline(w_xwinincoord(l->w, o_x1), w_ywinincoord(l->w, o_y1),
                  w_xwinincoord(l->w, o_x2), w_ywinincoord(l->w,
                                                           o_y2), color, xor);
}
...
void pol_init_rscoords(struct polygon *p) {
    ...
    if (p->pnts[0].corner != NULL) {
        for (i = 0; i < 2; i++) {
            ...
            v.x[i] =
                (p->pnts[p->num_pnts - 1].corner->x[i] << 3) - p->a_txt.x[i];
        }
...
int clipline(struct point_2d *sp, struct point_2d *ep, struct point_2d *sb,
             struct point_2d *eb, float *e_f1,
             float *e_f2)
{
    ...
    *e_f2 = 0.0;
    ...
    if (DEBUG > 2) {
        fprintf(
            errf, "Checking boundary: %g %g -> %g %g\n" \
                  "m1 %g %g m2 %g %g det %g m1_ds %g m1_de %g\n",
            sb->x[0], sb->x[1], eb->x[0], eb->x[1], m1.x[0], m1.x[1],
            m2.x[0], m2.x[1],
            det, m1_ds, m1_de);
    }

    if (m1_ds > ZERO) { /* start outside */
        if (m1_de < -ZERO) { /* end inside */
            *e_f1 = m1_ds / det;
            return 1;
        }
        else { /* end on border or outside */
            return 0;
        }
    }
    else if (m1_ds < -ZERO) { /* start inside */
        if (m1_de < -ZERO) { /* both pnts inside */
            return 1;
        }
        else { /* end on border or outside */
            *e_f2 = 1.0 - m1_ds / det;
        ...
        if (m1_de > ZERO) { /* end outside */
            return 0;
        }
        else if (m1_de < -ZERO) { /* end inside */
            *e_f1 = m1_ds / det;
            return 2;
        }
        else { /* end on border */
...
struct render_point *pol_clip_pnts(struct polygon *p,
                                   struct render_point *render_pnts,
                                   struct render_point *bounds, int xsize,
                                   int ysize)
{
    ...
    for (i = 0, vis = 0, pp = &p->pnts[0], rp = render_pnts; i < p->num_pnts;
         i++, pp = pp->next,
         rp++) {
        SUB_3D(&v_p[i], pp->p_3d, &x0);
        v_s2[i] = SCALAR_3D(&v_p[i], &er[2]);
    ...
    for (i = 0; i < p->num_pnts; i++) {
        v_s0[i] = SCALAR_3D(&v_p[i], &er[0]);
        v_s1[i] = SCALAR_3D(&v_p[i], &er[1]);
    }

    num_rp = cur_rp = p->num_pnts;
    first_rp = render_pnts;
    ...
    for (i = 0, pp = &p->pnts[0], rp = render_pnts, vis = 0; i < p->num_pnts;
        ...
        if (v_s2[i] <= z_dist && v_s2[old_i] <= z_dist) {
            ...
            if (v_s2[old_i] <= z_dist) {
                ...
                render_pnts[cur_rp].next = rp;
                rp = &render_pnts[cur_rp++];
                num_rp++;
                rp->prev->next = rp;
                rp->next->prev = rp;
                ...
                if (pp->corner) {
                    rp->light =
                        ( ( (long)pp->corner->light + (pp->corner->light -
                                                       pp->prev->
                                                       corner->light) ) <<
                         6 ) * f1 - 1;
                }
                ...
                if (DEBUG > 1) {
                    fprintf(errf, "Inserted rp %d (%d) %g: %g %g %lx\n", i,
                            cur_rp - 1,
                            f1, rp->x[0], rp->x[1],
                            rp->light);
                }

                if ( fabs(rp->x[0]) < xs || fabs(rp->x[1] < ys) ) {
                    vis = 1;
                }
            ...
            if (v_s2[i] <= z_dist) { /* clip the endpoint against the viewplane
                                       */
                ...
                if (pp->corner) {
                    rp->light = ( ( (long)pp->prev->corner->light +
                                   (pp->prev->corner->light -
                                    pp->corner->light) ) << 6 ) * f1 - 1;
                }
                else {
                    rp->light = 0;
                }
                ...
                if (DEBUG > 1) {
                    fprintf(errf, "Changed rp %d (%d) %g: %g %g %lx\n", i,
                            cur_rp - 1,
                            f1, rp->x[0], rp->x[1],
                            rp->light);
                }
                ...
                if (pp->corner) {
                    rp->light = ( (long)pp->corner->light << 6 ) - 1;
                }
                else {
                    rp->light = 0;
                }
                ...
                if (DEBUG > 1) {
                    fprintf(errf, "Calc rp %d (%d): %g %g %lx\n", i, cur_rp -
                            1,
                            rp->x[0], rp->x[1],
                            rp->light);
                }
    ...
    do {
        if (DEBUG > 1) {
            fprintf(errf, "Next bounds: %g %g -> %g %g\n",
        ...
        for (rp = first_rp, i = 0; i < o_num_rp; i++, rp = rp->next) {
            ...
            if ( ( clip_status =
                      clipline(&sp, &ep, &b_sp, &b_ep, &f1, &f2) ) != 0 ) { /*
                                                                              something
                                                                              of
                                                                              the
                                                                              line
                ...
                if (f1 != 0.0 || clip_status == 2) {
                    ...
                    if (DEBUG > 1) {
                        fprintf(errf, "Inserted rp %d (%d): %g %g %lx\n", i,
                                cur_rp - 1,
                                rp->x[0], rp->x[1],
                                rp->light);
                    }
                ...
                if (f2 != 0.0) {
                    ...
                    if (DEBUG > 1) {
                        fprintf(errf, "Changed rp %d: %g %g %lx\n", i,
                                rp->x[0],
                                rp->x[1], rp->light);
                    }
                }
                ...
                if (DEBUG > 1) {
                    fprintf(errf, "Killed rp %d: %g %g %lx\n", i, rp->x[0],
                            rp->x[1], rp->light);
                }
            }
        ...
        for (i = 0; i < o_num_rp; i++) {
            ...
            if (fabs(rp->x[0] - rp->prev->x[0]) < 0.1
               && fabs(rp->x[1] - rp->prev->x[1]) < 0.1) {
                if (DEBUG > 1)                                       {
                    fprintf(errf, " Killing this point: num_rp=%d\n",
                            num_rp - 1);
                }
                ...
                if (rp == first_rp) {
                    first_rp = min_rp = rp->prev;
                }
            }
            else if (rp->x[1] < min_rp->x[1]) {
                min_rp = rp;
            }
                           ...
                           int sublight)
{
    ...
    if ( SCALAR(&p->n_3d,
                &er[2]) >= (xviewphi > yviewphi ? xviewphi : yviewphi) ) {
        return;
    }

    for (i = 0; i < p->num_pnts; i++) {
        for (j = 0; j < 3; j++) {
            d.x[j] = p->pnts[i].p_3d->x[j] - x0.x[j];
        }

        if (SCALAR(&p->n_3d, &d) < -LENGTH(&d) * 0.01) {
            break;
        }
    }
    ...
    if (maxlight < 0) {
        ...
        maxlight = (maxlight << 16) + 0xffff;
    }

    rp->x[0] = round(rp->x[0]);
    rp->x[1] = round(rp->x[1]);
    rp->light =
        ( rp->light * ( NUM_LIGHTCOLORS - 1 - (view.gamma_corr >> 10) ) ) /
    ...
    if (rp->light > maxlight) {
        rp->light = maxlight;
    }

    rp->light =
        ( ( (int)view.gamma_corr << 6 ) & 0x1f0000 ) + (rp->light & 0x1fffff);
    ...
    do {
        ...
        if (db_rp->x[0] == db_rp->prev->x[0]
           && db_rp->x[1] == db_rp->prev->x[1]) {
            ...
            if (rp == db_rp) {
                rp = (db_rp->prev->x[1] <= db_rp->next->x[1] ?
            ...
            if (db_rp->light > maxlight) {
                db_rp->light = maxlight;
            }

            db_rp->light =
                ( ( (int)view.gamma_corr <<
                   6 ) & 0x1f0000 ) + (db_rp->light & 0x1fffff);
    ...
    if (DEBUG) {
        ...
        do {
            fprintf(errf, " %g,%g,%lx", db_rp->x[0], db_rp->x[1],
                    db_rp->light);
            my_assert(
                db_rp->x[0] >= -max_xcoord && db_rp->x[0] <= max_xcoord
                     && db_rp->x[1] >= -max_ycoord && db_rp->x[1] <=
                max_ycoord);
            db_rp = db_rp->next;
        } while (db_rp != rp);
...
void initfilledside(struct cube *cube, int wall) {
    ...
    if (fabs(tt) < 0.0001) { /* render the side with four points */
        if ( (p1 = cube->polygons[wall * 2]) == NULL ) {
            checkmem( cube->polygons[wall *
                                     2] = p1 = MALLOC( sizeof(struct polygon) ) );
        }
    ...
    else if (tt > 0.0) { /* render the side as two triangles 3,0,1 / 1,2,3 */
        if ( (p1 = cube->polygons[wall * 2]) == NULL ) {
            checkmem( cube->polygons[wall *
                                     2] = p1 = MALLOC( sizeof(struct polygon) ) );
        }
        ...
        if ( (p2 = cube->polygons[wall * 2 + 1]) == NULL ) {
            checkmem( cube->polygons[wall * 2 +
                                     1] = p2 = MALLOC( sizeof(struct polygon) ) );
        }
        ...
        pol_init_rscoords(p1);
        pol_init_rscoords(p2);
    }
    else { /* render the side as two triangles 0,1,2 / 2,3,0 */
        if ( (p1 = cube->polygons[wall * 2]) == NULL ) {
            checkmem( cube->polygons[wall *
                                     2] = p1 = MALLOC( sizeof(struct polygon) ) );
        }
...
unsigned char *gettexture(int rdlno, char txt1or2) {
    ...
    if (rdlno < 0 || rdlno >= pig.num_rdltxts || pig.rdl_txts[rdlno].pig ==
        NULL) {
        if (txt1or2 == 1) {
            memset(unknown_t1, view.color[HILIGHTCOLORS + 1], 64 * 64);
            return unknown_t1;
        }
        else {
    ...
    if (pig.rdl_txts[rdlno].pig->data == NULL) {
        ...
        memset(pig.rdl_txts[rdlno].pig->data, txt1or2 == 1 ? 0xfe : 0xff,
               64 * 64);
                 ...
                 int sublight)
{
        ...
        for (n = cube->d.c->fl_lights.head; n->next != NULL; n = n->next) {
            if (!n->d.fl->calculated) {
                ...
                if (j != n->d.fl->state) {
                    ...
                    if (j) {
                        for (ne = n->d.fl->ls->d.ls->effects.head;
                             ne->next != NULL; ne = ne->next) {
                            for (w = 0; w < 6; w++) {
                                if ( (wall =
                                          ne->d.lse->cube->d.c->walls[w]) !=
                                    NULL ) {
                                    for (x = 0; x < 4; x++) {
                                        overflow =
                                            (long)wall->corners[x].light +
                                            ne->d.lse->add_light[w * 4 + x];
                                        wall->corners[x].light = overflow >
                                                                 theMaxLight
                                                                 ?
    ...
    for (w = 0; w < 6; w++) {
        ...
        if (cube->d.c->nc[w] != NULL && cube->d.c->nc[w] != from) {
            for (j = 0; j < 2; j++) {
                ...
                if (cube->d.c->polygons[w * 2 + j]
                   && ( render_start =
                           pol_clip_pnts(cube->d.c->polygons[w * 2 + j],
                                         render_pnts[depth],
                                         bounds, scr_xsize,
                                         scr_ysize) ) != NULL) {
                    ...
                    if (DEBUG) {
                        fprintf(errf, "Recursion:\n");
                    }

                    do {
                        m1.x[0] = rp->next->x[0] - rp->x[0];
                        m1.x[1] = rp->next->x[1] - rp->x[1];
                        m2.x[0] = rp->prev->x[0] - rp->x[0];
                        m2.x[1] = rp->prev->x[1] - rp->x[1];

                        if (DEBUG) {
                            fprintf( errf, " %g,%g,%g,%g", rp->x[0], rp->x[1],
                        ...
                        if (SCALAR_2D(&m1, &m2) * SCALAR_2D(&m1, &m2) >=
                            SCALAR_2D(&m1,
                                      &m1) *
                            SCALAR_2D(&m2, &m2) * 0.999999) { /* pnt is useless,
                                                                it lies on the
                                                                line from prev
                                                                to next */
                            if (DEBUG) {
                    ...
                    if (render_start->next != render_start->prev) {
                        render_cube(
                            depth + 1, cube, cube->d.c->nc[w], render_start,
                            cube->d.c->d[w] != NULL &&
                            cube->d.c->d[w]->d.d->type1 == door1_cloaked
                            ? sublight -
                            (31 - cube->d.c->d[w]->d.d->cloaking) : sublight);
                    }
                }
            }
        }
        ...
        if ( cube->d.c->walls[w] && ( !cube->d.c->d[w]
            ...
            if (cube->d.c->walls[w]->texture2 != 0) {
                ...
                txt = gettexture(cube->d.c->walls[w]->texture1, 1);
            }
                ...
                if (cube->d.c->polygons[w * 2 + j]
                   && ( render_start =
                           pol_clip_pnts(cube->d.c->polygons[w * 2 + j],
                                         ...
                                         bounds, scr_xsize,
                                         scr_ysize) ) != NULL) {
                    render_filled_polygon(render_lr,
    ...
    if ( (render_drawwhat & DW_CUBES) != 0 ) {
        in_plotcube(render_lr, cube, 0, 0, 0, 1, 1);
    }

    if ( (render_drawwhat & DW_DOORS) != 0 ) {
        for (w = 0; w < 6; w++) {
            if (cube->d.c->d[w]) {
                in_plotdoor(render_lr, cube->d.c->d[w], 0, 0, 0);
            }
        }
    }
    ...
    for (n = ld->lightsources.head; n->next != NULL; n = n->next) {
        if (n->d.ls->fl != NULL) {
            ...
            if (!fl->state) {
                ...
                for (ne = n->d.ls->effects.head; ne->next != NULL;
                     ne = ne->next) {
                    my_assert(ne->d.lse->cube != NULL);

                    for (w = 0; w < 6; w++) {
                        if (ne->d.lse->cube->d.c->walls[w]) {
                            for (c = 0; c < 4; c++) {
                                overflow =
                                    (long)ne->d.lse->cube->d.c->walls[w]->
                                    corners[
                                        c].light +
                                    ne->d.lse->add_light[w * 4 + c];
                                ne->d.lse->cube->d.c->walls[w]->corners[c].
                                light = overflow > theMaxLight ?
                                        theMaxLight
                                        : overflow;
                            }
                        }
...
unsigned long render_level(int lr, struct leveldata *ld,
                           struct node *start_cube, int drawwhat,
                           int depth)
{
