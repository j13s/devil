...
const char *txt_names[txt_all] = {
    "NormalTxt", "ExtraTxt", "WNormalTxt",
    "WExtraTxt", "WDoors"
};
...
int readtxts(FILE *f) {
    ...
    switch (init.d_ver) {
        ...
        case d1_14_reg:
            ...
            if (head.ul[0] > 0x10000) { /* D1 1.4 */
                ...
                if (fread(&numshort, sizeof(unsigned long), (size_t)1,
                          f) != 1) {
                    return 0;
                }

                if (fread(&numlong, sizeof(unsigned long), (size_t)1,
                          f) != 1) {
                    return 0;
                }
            }
    ...
    for (i = 0; i < pig.num_pigtxts; i++) {
        if (init.d_ver >= d2_10_sw) {
            ...
            pig.pig_txts[i].rlxsize = D1_txt.ruxsize ==
                                      0x80 ? 0x40 : D1_txt.rxsize;
        ...
        pig.pig_txts[i].xsize = pig.pig_txts[i].rlxsize +
                                (pig.pig_txts[i].ruxsize << 8);
...
int readcustomtxts(FILE *f) {
    ...
    if ( (numshort <= pig.num_pigtxts) && (pig.pig_txts != NULL) ) {
        ...
        for (i = 0; i < numshort; i++) {
            ...
            /* Weird construction, I know, but it seems it works with animations
              txts */
            for (j = 0, k = 0;
            ...
            if (j < pig.num_pigtxts) {
                pig.pig_txts[j].rlxsize = D1_txt.ruxsize ==
                                          0x80 ? 0x40 : D1_txt.rxsize;
                ...
                pig.pig_txts[j].offset = D1_txt.offset + 8 + 17 * numshort +
                                         20 * numlong;
                pig.pig_txts[j].xsize = pig.pig_txts[j].rlxsize +
...
void copytxtnums(enum txttypes dest, enum txttypes source) {
    ...
    memmove( &pig.txtlist[dest][pig.num_txtlist[dest]],
            pig.txtlist[source], pig.num_txtlist[source] *
