...
void askpath(char *oldpath, const char *text, const char *testfile) {
    ...
    do {
        printf("%s Verzeichnis (bisher: \"%s\", abbrechen mit 'q'): ", text,
               oldpath);
        scanf("%199s", testpath);
...
void testscreen(int grfxdrv, int res) {
    ...
    if (grfxdrv < 0 || grfxdrv >= NUM_GRFXDRVS || res < 0 || res >=
        NUM_RESOLUTIONS) {
        return;
    }
    ...
    printf(
        "Teste Grafiktreiber %s mit Auflîsung %dx%d:\n" \
    ...
    if ( ws_initgrfx(xs, ys, 256, init.fontname) ) {
        ...
        ws_drawtext( ( xs - ws_pixstrlen(
                          TESTTEXT1) ) / 2, 10, xs, TESTTEXT1, 3, -1 );
        ws_drawtext( ( xs - ws_pixstrlen(
        ...
        ws_textmode();
        printf(
            "Huppsa. Kann Grafikmodus nicht initialisieren.\n" \
...
int checkd2version(char *path) {
    ...
    if ( ( f = fopen(fname, "rb") ) != NULL ) {
        ...
        printf("Kann Datei \"%s\" nicht im Descent 2 Verzeichnis finden.\n",
               fname);
    }
                  ...
                  int *resolution)
{
    ...
    do {
        printf("\n\n");
        printf(
            "Wilkommen zu Devil " VERSION ", dem Descent 1 und 2 Editor.\n");
        printf(
            "---------------------------------------------------------\n\n");
        ...
        switch (x) {
            case 'A':
                askpath(devilpath, "Devil", "config/devilg.ini");
                break;

            case 'B':
                askpath(d1path, "Descent 1", "descent.pig");
                break;
            ...
            case 'G':
                testscreen(*grfxdriver, *resolution);
                break;
            ...
            case 'Q':
                ...
                if ( !checkpath(devilpath, "config/devilg.ini", "Devil") ) {
                    break;
                }

                if ( *d_ver == 1 &&
                    !checkpath(d1path, "descent.pig", "Descent 1") ) {
                    break;
                }
                ...
                if (*d_ver == 2) {
                    ...
                    *d_ver = checkd2version(d2path);
                    printf(
                        "BestÑtige Descent 2 version %s in %s\n",
                        *d_ver ==
                        d2_10_reg ? "1.0" : (*d_ver ==
                                             d2_11_reg ? "1.1" : "1.2"),
                        d2path);
