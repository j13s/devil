...
#define COLORNUM2(d, min) ( ( (d) >= view.maxvisibility ) ? (min) : \
                           (d <=\
                            0.0) ? GRAYSCALE : ( (int)( (GRAYSCALE - \
                                                         1) *\
                                                       (1 - (d) /\
                                                        view.maxvisibility) ) + 1 ) )
#define WCOLORNUM(d, h) view.color[(h) <=\
                                   0 ? COLORNUM2(d, (h) == -1 ? BLACK : 1) : \
                                   ( (h) == 256 ? BLACK : HILIGHTCOLORS +\
                                    (h) - 1 )]
#define TCOLORNUM(c, h) view.color[(h) <= 0 ? \
                                   ( (h) == 0 ? (c) +\
                                    THINGCOLORS : BLACK ) : HILIGHTCOLORS +\
                                   (h) - 1]
...
int partcolor(struct door *d, float dist, int h) {
    ...
    if (h < 0 && dist > view.maxvisibility) {
        return BLACK;
    }

    if (d->tagged) {
        return HILIGHTCOLORS + 2;
    }
...
#define PCOLORNUM(dist, door, h) view.color[(h) <=\
                                            0 ? partcolor(door, dist, h) : \
                                            ( (h) ==\
                                             256 ? BLACK : HILIGHTCOLORS +\
                                             (h) - 1 )]
...
void copytoscreen(void) {
    psys_copytoscreen( w_xwinincoord(l->w, 0), w_ywinincoord(l->w, 0),
                      w_xwinincoord(l->w, 0), w_ywinincoord(l->w,
                                                            0),
                      w_xwininsize(l->w),
                      w_ywininsize(l->w) );
}
                     ...
                     struct point *x0)
{
    ...
    if (l->whichdisplay && lr >= 0) {
        for (i = 0; i < 3; i++) {
            e0->x[i] = view.e0.x[i] - er[2].x[i] * view.distcenter;
        }
    }
    else {
        *e0 = view.e0;
...
void makeview(int lr) {
    ...
    if (lr == -1) {
        ...
        scr_xsize =
            ( view.whichdisplay ? w_xwininsize(l->w) / 2 : w_xwininsize(l->w) );
    ...
    max_axis = max_xcoord >= max_ycoord ?
               (min_axis = max_ycoord, max_xcoord) : (min_axis = max_xcoord,
                                                      max_ycoord);
...
int in_getpixelcoords(struct point *p, struct pixel *pix) {
    ...
    if (SCALAR(&x, &er[2]) < xviewphi || SCALAR(&y, &er[2]) < yviewphi) {
        return 0;
    }
...
/* calculates point of intersection of two lines m1*t1+b1, m2*t2+b2:
    a=b2-b1, e=m1, d=m2 */
int calchit(float *r, float ax, float ay, float ex, float ey, float dx,
            float dy)
{
...
/* get the (clipped) screen coords for line sp->ep (lr==0 on left,==1 on right
    display) and return them in spix,epix. If the line is not visible, return
   a 0,otherwise a 1. if checkdist==1 then check if the line is too
   far away */
int getscreencoords(int lr, struct point *sp, struct point *ep,
                    struct pixel *spix, struct pixel *epix,
                    int checkdist)
{
    ...
    if (checkdist && f1 > view.maxvisibility && f2 > view.maxvisibility) {
        return 0;
    }
    ...
    if ( ( (s_s[0] > 0.0 && e_s[0] > 0.0) || (s_s[0] < 0.0 && e_s[0] < 0.0) )
       && s_s[0] * s_s[0] > l2_s * xviewphi && e_s[0] * e_s[0] > l2_e *
        xviewphi ) {
        return 0;
    }

    if ( ( (s_s[1] > 0.0 && e_s[1] > 0.0) || (s_s[1] < 0.0 && e_s[1] < 0.0) )
       && s_s[1] * s_s[1] > l2_e * yviewphi && e_s[1] * e_s[1] > l2_e *
        yviewphi ) {
        return 0;
    }
    ...
    if (FABS(sp2d.x[0]) > max_xcoord) {
        if (ep2d.x[0] == sp2d.x[0]) {
            return 0;
        }

        if (sp2d.x[0] > 0.0) {
            f1 = (sp2d.x[0] - max_xcoord) / (sp2d.x[0] - ep2d.x[0]);
        }
        else {
            f1 = (-max_xcoord - sp2d.x[0]) / (ep2d.x[0] - sp2d.x[0]);
        }
    ...
    if (FABS(sp2d.x[1]) > max_ycoord) {
        if (ep2d.x[1] == sp2d.x[1]) {
            return 0;
        }

        if (sp2d.x[1] > 0.0) {
            f1 = (sp2d.x[1] - max_ycoord) / (sp2d.x[1] - ep2d.x[1]);
        }
        else {
            f1 = (-max_ycoord - sp2d.x[1]) / (ep2d.x[1] - sp2d.x[1]);
        }
    ...
    if (FABS(ep2d.x[0]) > max_xcoord) {
        ...
        if (ep2d.x[0] > 0.0) {
            f1 = (ep2d.x[0] - max_xcoord) / (ep2d.x[0] - sp2d.x[0]);
        }
        else {
            f1 = (-max_xcoord - ep2d.x[0]) / (sp2d.x[0] - ep2d.x[0]);
        }
    ...
    if (FABS(ep2d.x[1]) > max_ycoord) {
        ...
        if (ep2d.x[1] > 0.0) {
            f1 = (ep2d.x[1] - max_ycoord) / (ep2d.x[1] - sp2d.x[1]);
        }
        else {
            f1 = (-max_ycoord - ep2d.x[1]) / (sp2d.x[1] - ep2d.x[1]);
        }
...
void in_plotmarker(int w, struct point *p, int hilight) {
    ...
    for (i = 0; i < 8; i += 2) {
        if ( getscreencoords(w, &dp[i], &dp[i + 1], &spix, &epix, hilight ==
                             0 || hilight == 257) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
                     WCOLORNUM( (epix.d + spix.d) / 2.0,
                               ANALYZE_HILIGHT(hilight) ), hilight < 0);
        }
        else if (hilight == 257) {
            if ( getscreencoords(w, &dp[i], &dp[i + 1], &spix, &epix, 0) ) {
                plotline(spix.x, spix.y, epix.x, epix.y,
                         WCOLORNUM( (epix.d + spix.d) / 2.0,
                                   ANALYZE_HILIGHT(256) ), 0);
            }
        }
    }
}
...
void in_plotdoor(int w, struct node *n, int hilight, int drawswitched,
                 int xor) {
    struct door *d = n->d.d;
    struct pixel spix, epix;
    struct node *sdn;
    ...
    for (j = 0; j < 4; j++) {
        if ( getscreencoords(w, &d->p, d->w->p[j]->d.p, &spix, &epix,
                             hilight == 0) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
        ...
        switch ( getsdoortype(d->sd->d.sd) ) {
            case sdtype_door:
                ...
                for (j = 0; j < d->sd->d.sd->num; j++) {
                    in_plotdoor(w, d->sd->d.sd->target[j], drawswitched >
                                0 ? 2 : -1, 0,
                                0);
                }
            ...
            case sdtype_cube:
                ...
                for (j = 0; j < d->sd->d.sd->num; j++) {
                    in_plotcube(w, d->sd->d.sd->target[j], drawswitched >
                                0 ? 2 : -1, 0, 0, 1,
                                0);
                }
            ...
            case sdtype_side:
                ...
                for (j = 0; j < d->sd->d.sd->num; j++) {
                    in_plotwall(w, d->sd->d.sd->target[j]->d.c,
                                d->sd->d.sd->walls[j],
                                drawswitched > 0 ? 1 : -1,
                                0);
                }
                 ...
                 int withlockedsides)
{
    ...
    if ( hilight == 1 && testtag(tt_cube, n) ) {
        hilight = 6;
    }
    ...
    if (!withalllines) {
        for (j = 0; j < 6; j++) {
            if (c->nc[j] != NULL && c->nc[j]->no < n->no) {
                switch (j) {
                    case 0:
                        next &= ~(0x4 | 0x40 | 0x400 | 0x800);
                        break;

                    case 1:
                        next &= ~(0x8 | 0x80 | 0x100 | 0x800);
                        break;

                    case 2:
                        next &= ~(0x1 | 0x10 | 0x100 | 0x200);
                        break;

                    case 3:
                        next &= ~(0x2 | 0x20 | 0x200 | 0x400);
                        break;

                    case 4:
                        next &= ~(0x10 | 0x20 | 0x40 | 0x80);
                        break;

                    case 5:
                        next &= ~(0x1 | 0x2 | 0x4 | 0x8);
                        break;
                }
            }
        }
    ...
    for (j = 0; j < 4; j++) {
        if ( ( next & (1 << j) ) != 0
           && getscreencoords(w, c->p[j]->d.p, c->p[j == 3 ? 0 : j + 1]->d.p,
                              &spix, &epix,
                              testdist) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
        ...
        if ( ( next & (0x10 << j) ) != 0
           && getscreencoords(w, c->p[j + 4]->d.p,
                              c->p[j == 3 ? 4 : j + 5]->d.p, &spix, &epix,
                              testdist) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
                     WCOLORNUM( (spix.d + epix.d) / 2, hilight ), xor);
        }
        ...
        if ( ( next & (0x100 << j) ) != 0
           && getscreencoords(w, c->p[j]->d.p, c->p[j + 4]->d.p, &spix, &epix,
                              testdist) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
                     WCOLORNUM( (spix.d + epix.d) / 2, hilight ), xor);
        }
    ...
    if (withlockedsides) {
        for (j = 0; j < 6; j++) {
            if (n->d.c->walls[j] != NULL && n->d.c->walls[j]->locked) {
                in_plottagwall(w, n->d.c, j, n->d.c->tagged_walls[j] !=
                               NULL ? 4 : 2,
                               xor);
            }
        }
...
void in_plotthing(int w, struct thing *t, int hilight) {
    ...
    for (i = 1; i < (t->type1 == 7 ? 9 : 11); i += 2) {
        if ( getscreencoords(w, &t->p[i], &t->p[i + 1], &spix, &epix,
            ...
            plotline(spix.x, spix.y, epix.x, epix.y,
                     TCOLORNUM( t->color, ANALYZE_HILIGHT(
                                   hilight) ), hilight < 0);
        }
        else if (hilight == 257) {
            if ( getscreencoords(w, &t->p[i], &t->p[i + 1], &spix, &epix,
...
void in_plotwall(int w, struct cube *c, int wno, int hilight, int xor) {
    ...
    for (j = 0; j < 4; j++) {
        if ( getscreencoords(w, c->p[wallpts[wno][j]]->d.p,
                             c->p[wallpts[wno][(j + 1) & 3]]->d.p, &spix,
                             &epix, hilight == 0) ) {
            plotline(spix.x, spix.y, epix.x, epix.y,
...
void in_plottagwall(int w, struct cube *c, int wallno, int hilight,
                    int xor) {
...
void in_plotfirstmacrocube(int w) {
    ...
    if (view.pcurrmacro == NULL || view.pcurrmacro->exitcube == NULL
       ...
       || view.pcurrmacro->exitcube->d.c->nc[view.pcurrmacro->exitwall] !=
        NULL
    ...
    offset =
        view.pcurrcube->d.c->p[wallpts[view.currwall][view.curredge]]->d.p;
    eoffset = view.pcurrmacro->exitcube->d.c->p[
        wallpts[view.pcurrmacro->exitwall][0]]->d.p;
    getcubecoords(view.pcurrmacro->exitcube->d.c, view.pcurrmacro->exitwall,
                  0,
                  eaxis,
                  0);
    ...
    for (j = 0; j < 8; j++) {
        for (i = 0; i < 3; i++) {
            hp1.x[i] = view.pcurrmacro->exitcube->d.c->p[j]->d.p->x[i] -
                       eoffset->x[i];
        }
    ...
    for (j = 0; j < 6; j++) {
        if ( ( marker[j] =
                  (view.pcurrmacro->exitcube->d.c->nc[j] != NULL) ) != 0 ) {
            in_plottagwall(w, view.pcurrmacro->exitcube->d.c, j, 2, 1);
        }
    }

    for (j = 0; j < 8; j++) {
        view.pcurrmacro->exitcube->d.c->p[j]->d.p = op[j];
    }
...
void in_plotcurrent(int w) {
    ...
    if (oldpcurrdoor) {
        in_plotdoor(w, oldpcurrdoor, -1, -1, 0);
    }

    if (oldpcurrthing) {
        in_plotthing(w, oldpcurrthing->d.t,
                     testtag(tt_thing, oldpcurrthing) ? 3 : 257);
    }

    if (oldpcurrpnt) {
        in_plotmarker(w, oldpcurrpnt->d.p, testtag(tt_pnt,
                                                   oldpcurrpnt) ? 3 : -1);
    }

    if (oldpcurrcube) {
        if (oldcurredge >= 0)                    {
            in_plotpnt(w, oldpcurrcube, oldcurrwall, oldcurredge,
                       testtag(tt_edge, oldpcurrcube, oldcurrwall,
                               oldcurredge) ? 3 : -1);
        }
    ...
    if (oldcurredge >= 0) {
        in_plotpnt(w, view.pcurrcube, oldcurrwall, oldcurredge,
                   testtag(tt_edge, view.pcurrcube, oldcurrwall,
                           oldcurredge) ? 3 : -1);
    }

    if (view.pcurrthing) {
        in_plotthing(w, view.pcurrthing->d.t,
                     testtag(tt_thing, view.pcurrthing) ? 6 : 1);
    }

    if (view.pcurrdoor) {
        in_plotdoor(w, view.pcurrdoor,
                    testtag(tt_door, view.pcurrdoor) ? 6 : 1, 1, 0);
    }

    if (view.pcurrpnt) {
        in_plotmarker(w, view.pcurrpnt->d.p,
    ...
    in_plotpnt(w, view.pcurrcube, view.currwall, view.curredge,
               testtag(tt_edge, view.pcurrcube, view.currwall,
                       view.curredge) ? 6 : -1);
...
void in_plotcoordaxis(int lr) {
    ...
    if ( ( view.coord_axis & (1 << l->whichdisplay) ) == 0 ||
        view.distcenter == 0.0 ) {
        return;
    }
    ...
    if (!l->whichdisplay) {
        for (i = 0; i < 3; i++) {
            sp.x[i] += er[2].x[i] * view.distcenter;
        }
    }
...
void in_plotpnt(int w, struct node *c, int wn, int pn, int hilight) {
    ...
    for (i = 0; i < 3; i++) {
        p1.x[i] = c->d.c->p[wallpts[wn][pn]]->d.p->x[i] + p1.x[i] *
                  view.tsize * 3;
    }

    in_plot3dline(w, c->d.c->p[wallpts[wn][pn]]->d.p, &p1,
                  WCOLORNUM( 0.0, ANALYZE_HILIGHT(
                                hilight) ), hilight < 0, hilight == 0);
}
...
unsigned long cont_plotlevel(struct lightsource **ls) {
    ...
    for (lr = 0; lr <= l->whichdisplay; lr++) {
        ...
        if (lr) {
            ...
            plotline(w_xwininsize(l->w) / 2 - 1, 0, w_xwininsize(
                         l->w) / 2 - 1,
                     w_ywininsize(l->w) - 1, view.color[WHITE], 0);
        }
        ...
        if (!l->whichdisplay && view.render > 1 && l->inside) {
            ...
            if ( (view.drawwhat & DW_CUBES) != 0 ||
                ...
                if ( (view.drawwhat & DW_ALLLINES) == 0 ) {
                    ...
                    for (n = l->cubes.head; n->next != NULL; n =
                             n->next)              {
                        if (n->d.c->tagged == NULL) {
                            in_plotcube(lr, n, 0, 0, 0, 0, 1);
                        }
                    }
                }
            }
            ...
            if (view.render >= 1) {
                dt = render_level(lr, l, view.pcurrcube, 0,
                                  view.render == 1 ? 1 : 0);
            }
            ...
            if (l->exitcube) {
                ...
                in_plotmarker(
                    lr, l->exitcube->d.c->p[wallpts[l->exitwall][0]]->d.p, 2);
            }

            if (l->cur_corr) {
                in_plotcorridor(lr, l->cur_corr);
            }
        }
        ...
        if ( (view.drawwhat & DW_XTAGGED) != 0 ) {
            for (n = l->tagged[tt_cube].head; n->next != NULL; n = n->next) {
                for (i = 0; i < 6; i++) {
                    in_plottagwall(lr, n->d.n->d.c, i, 3, 0);
                }
            }
        }

        for (n = l->tagged[tt_wall].head; n->next != NULL; n = n->next) {
            in_plottagwall(lr, n->d.n->d.c, n->no % 6,
                           n->d.n->d.c->walls[n->no % 6] != NULL
    ...
    for (n = l->lightsources.head, *ls = NULL; n->next != NULL; n =
             n->next) {
...
void addline(struct node *c, int s, int e) {
    ...
    for (n = l->lines.tail; n->prev != NULL; n = n->prev) {
        if (n->d.l->s->no == sp->no && n->d.l->e->no == ep->no) {
            return;
        }
    }
...
/*     cn1=findnbcubetoline(i,j,(j+1)&3); */
                    ...
                    if (n->d.c->nc[cn1] == NULL) {
                        addline(n, wallpts[i][j], wallpts[i][(j + 1) & 3]);
                    }
                    else {
                        /* ok, wall is there, neighbour cube is also there. Let's
                           have a
                           look if there's a wall in the neighbourcube */
                        wno =
                            findnbwalltoline(n, n->d.c->nc[cn1]->d.c, i, j,
                                             (j + 1) & 3);

                        if (wno == -1 ||
                            (w = n->d.c->nc[cn1]->d.c->walls[wno]) == NULL) {
                            addline(n, wallpts[i][j], wallpts[i][(j + 1) & 3]);
                            continue;
                        }
...
void in_plotcorridor(int lr, struct corridor *c) {
    ...
    for (n = c->cubes.head; n->next != NULL; n = n->next) {
        in_plotcube(lr, n, 2, 0, 1, 0, 0);
    }
    ...
    for (n = c->tracking.head; n->next != NULL; n = n->next) {
        if (n->d.ct->fixed <= 0) {
            ...
            if (size < view.tsize * 2) {
                size = view.tsize * 2;
            }
            else if (size > view.tsize * 20) {
                size = view.tsize * 20;
            }

            switch (c->b_stdform->d.ls->selected) {
                case 0:
                    make_o_marker(&n->d.ct->x, n->d.ct->coords, size, p);
                    num_pts = 10;
                    plot_pnts = &p[1];
                    break;
                ...
                case 1:
                case 2:
                    makemarker(&n->d.ct->x, plot_pnts = p);
                    num_pts = 8;
                    break;
            }
...
void move_render(struct point *wanted_e0, struct point *cur_e0) {
    ...
    for (i = 0; i < 3; i++) {
        m.x[i] = wanted_e0->x[i] - cur_e0->x[i];
    }
        ...
        for (j = 0; j < 2; j++) {
            if ( (p = l->rendercube->d.c->polygons[w * 2 + j]) != NULL ) {
                ...
                for (pp = 2; pp < p->num_pnts; pp++) {
                    ...
                    for (i = 0; i < 3; i++) {
                        c.x[i] = p->pnts[pp].p_3d->x[i] - a.x[i];
                    }
                    ...
                    if (det > 0.0) {
                        ...
                        /* if this a wall, we should be a bit larger than zero
                          radius */
                        if (t < 0.0) {
                            continue;
                        }
                        ...
                        if (l->rendercube->d.c->nc[w] == NULL) {
                            /* in the next step we only want to use the part of
                               m*(1.0-t) parallel
                               to the wall */
                            for (i = 0; i < 3; i++) {
                                new_m.x[i] =
