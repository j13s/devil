...
int vprintmsg(const char *txt, va_list args, int wait) {
    ...
    return w_okcancel(buffer, wait ? TXT_OK : NULL, wait >
                      1 ? TXT_CANCEL : NULL,
                      NULL);
}
...
/* makes a file-requester with the files files and the dirs dirs in the
   ...
   successful. */
int makefilebuttons(struct w_window *w, int x, int y, int ysize, char **files,
                    int nofiles, char **dirs, int nodirs, const char *defname,
                    struct w_button **b)
{
                  ...
                  int overwrite_warning)
{
    ...
    checkmem( b[8] = w_addstdbutton(ow, w_b_press, b[7]->xsize, 0,
                                    w_xwininsize(ow) / 2, b[7]->ysize,
                                    TXT_CANCEL, &b_cancel, 1) );
...
int uio_getallhoglevels(FILE *hogfile, struct hoglevel ***hognames) {
    ...
    *hognames = NULL;
    ...
    while (fread(name, 1, 13, hogfile) == 13) {
        ...
        if (strlen(name) >= 5 &&
            strstr(init.alllevelexts, &name[strlen(name) - 3]) != NULL) {
            checkmem( *hognames =
                         REALLOC( *hognames, sizeof(struct hoglevel *)*
                                 (++hognum) ) );
            checkmem( (*hognames)[hognum -
                                  1] = MALLOC( sizeof(struct hoglevel) ) );
    ...
    if (!hoglevnum) {
        ...
        for (i = 0; i < bh->no_strings; i++) {
            ...
            if (hoglev_arr[i]->secret) {
                ...
                bh->strings[i][strlen(bh->strings[i]) -
                               2] = hoglev_arr[i]->secret / 10 + '0';
                bh->strings[i][strlen(bh->strings[i]) -
                               1] = hoglev_arr[i]->secret % 10 + '0';
            }
        }
    }
    ...
    *bf = ( (struct w_button **)b->data )[5]->d.sls;
    ...
    for (i = 0; i < bh->no_strings; i++) {
        if (bh->sel_arr[i] && hoglev_arr[i]->size > 0) {
            ...
            if (fread(levdat, 1, hoglev_arr[i]->size,
                      f) != hoglev_arr[i]->size) {
                waitmsg(TXT_CANTFINDLEVINHOG, hoglev_arr[i]->name,
                        hogfilename);
                fclose(f);
            ...
            if ( ( f = fopen(fullpath, "wb") ) == NULL ) {
                waitmsg(TXT_CANTWRITEHOGLEV, hoglev_arr[i]->name, fullpath);
                FREE(levdat);
                return;
            }

            if (fwrite(levdat, 1, hoglev_arr[i]->size,
                       f) != hoglev_arr[i]->size) {
                waitmsg(TXT_CANTWRITEHOGLEV, hoglev_arr[i]->name, fullpath);
                FREE(levdat);
                fclose(f);
    ...
    FREE(bf->strings);
    FREE(bf->sel_arr);
    bf->strings = ws_getallfilenames(current_path, current_ext,
                                     &bf->no_strings);
...
void uio_hogfile_write(struct w_button *b) {
    ...
    newhogname =
        getfilename(&init.missionpath, hogfilename, "HOG", "HOG-File",
                    1);
    strcpy(current_path, buffer);
    strcpy(current_ext, ext);
    ...
    if (strcmp(msnfname,
               "DESCENT") == 0 || strcmp(msnfname, "DESCENT2") == 0) {
            ...
            while (fscanf(f, "\n%[^\n]", buffer) == 1) {
                ...
                if (equal) {
                    if (strstr(buffer, "NAME") != NULL) {
                        ...
                        b_arr[17]->d.str->str[b_arr[17]->d.str->max_length] =
                            0;
...
void hogfilemanager(void) {
    ...
    b_msnname.l_char_entered = b_msnname.r_char_entered =
                                   b_msnname.l_string_entered =
                                       b_msnname.r_string_entered = NULL;
    ...
    checkmem( b[11] =
                 w_addstdbutton(ow, w_b_list, 0, b[10]->ypos + b[10]->ysize,
                                b[9]->xsize, no_rows *
                                (w_titlebarheight() + 2), NULL, &b_hoglevels,
                                1) );
    checkmem( b[12] =
