...
void changedata(struct infoitem *i, int withtagged, void *data) {
        ...
        for (n = l->tagged[i->tagnr].head, no_obj = 0; n->next != NULL;
            ...
            switch (i->infonr) {
                ...
                case ds_flickeringlight:
                    ...
                    if (view.pcurrcube != n->d.n || view.currwall != n->no %
                        6) {
                        if (i->sidefuncnr >= 0 && i->sidefuncnr <
                            sc_number) {
                            no_obj +=
                ...
                case ds_corner:
                    ...
                    if (view.pcurrcube != n->d.n || view.currwall !=
                        (n->no % 24) / 4
                ...
                case ds_thing:
                    ...
                    if (view.pcurrthing &&
                        ...
                        || i->sidefuncnr ==
                         sc_thingtype) && view.pcurrthing != n->d.n) {
                        if (i->sidefuncnr >= 0 && i->sidefuncnr <
                            sc_number) {
                            no_obj +=
                                do_sideeffect(i->sidefuncnr, i, data, n->d.n,
                ...
                case ds_door:
                    ...
                    if (view.pcurrdoor &&
                        (n->d.n->d.d->type1 == view.pcurrdoor->d.d->type1
                        || i->sidefuncnr == sc_walltype ||
                         i->sidefuncnr == sc_switch)
    ...
    if (i->sidefuncnr >= 0 && i->sidefuncnr < sc_number) {
        do_sideeffect(i->sidefuncnr, i, data, getnode(
                          i->infonr), view.currwall,
                      view.curredge, 0);
    }
    else {
        setno(i, data, NULL);
...
void l_uvcoord_entered(struct w_button *b) {
    float_entered(b, MWT(b,
                         0), 1.0, "%10.0f", 1, -16.0 * 32 * 64, 16.0 * 32 *
                  64);
}
...
void r_uvcoord_entered(struct w_button *b) {
    float_entered(b, MWT(b,
                         1), 1.0, "%10.0f", 1, -16.0 * 32 * 64, 16.0 * 32 *
                  64);
}
...
int makeoptbuttons(struct w_window *w, int num, struct infoitem *is, int y) {
    ...
    for (i = is, maxy = 0; i - is < num; i++) {
        ...
        switch (i->type) {
            ...
            case it_markbutton:
                ...
                for (n = 0; n < i->od->size; n++) {
                    ...
                    b_tagged->tagged[n] = data == NULL ? 0 :
                                          (i->od->data[n]->no < i->length *
                                           8 ?
                                           ( ( data[i->od->data[n]->no /
                                                    8] &
                                              ( 1 <<
                                               (i->od->data[n]->no % 8) ) ) ) != 0 : 0);
                }
                ...
                checkmem( i->b =
                             w_addstdbutton(w, w_b_tag, 0, y, w_xwininsize(w),
                                            -1, i->txt,
                                            b_tagged, 0) );
                break;
            ...
            case it_sidelight:
            case it_deltalight:
                activate = i->type == it_light || i->type ==
                           it_deltalight ? getno(i, &no, NULL) :
                ( i->type ==
                ...
                sprintf(b_string->str, "%6.2f%%", i->type ==
                        it_deltalight ? no / 0.32 :
                        no / 327.67);
                b_string->allowed_char = isfloat;
                b_string->l_char_entered = b_string->r_char_entered = NULL;
                b_string->l_string_entered = i->type ==
                                             it_deltalight ?
                                             left_dlight_entered :
                                             left_light_entered;
                b_string->r_string_entered = i->type ==
                                             it_deltalight ?
                                             right_dlight_entered :
                                             right_light_entered;
                checkmem( i->b =
            ...
            case it_uvcoord:
                activate = getno(
                    i, i->type == it_size || i->type == it_fl_delay
                                || i->type ==
                    it_uvcoord ? (void *)&sno : (void *)&dno, NULL);
                checkmem( b_string = MALLOC( sizeof(struct w_b_string) ) );
            ...
            case it_float:
                ...
                b_string->l_char_entered = b_string->r_char_entered = NULL;
                b_string->l_string_entered = i->type ==
                                             it_degree ? left_degrees_entered
                                             : left_float_entered;
                b_string->r_string_entered = i->type ==
                                             it_degree ?
                                             right_degrees_entered
                                             : right_float_entered;
                checkmem( i->b =
                             w_addstdbutton(w, w_b_string, 0, y,
                                            w_xwininsize(w), -1, i->txt,
            ...
            case it_fl_mask:
                ...
                b_string->allowed_char = i->type ==
                                         it_fl_mask ? isbinary : isxdigit;
                b_string->l_char_entered = b_string->r_char_entered = NULL;
            ...
            case it_thingtexture:
                ...
                switch (i->multifuncnr) {
                    case 0: /* texture itself */
                        ...
                        b_press->delay = 0;
                        b_press->repeat = 0;
                        b_press->l_pressed_routine = i->type ==
                                                     it_dooranim ?
                                                     b_l_playdooranim : NULL;
                        b_press->r_pressed_routine = NULL;
                        b_press->l_routine = i->type ==
                                             it_texture ? b_l_fitbitmap :
                                             (i->type ==
                                              it_dooranim ? b_l_enddooranim :
                                              NULL);
                        b_press->r_routine = i->type ==
                                             it_texture ? b_r_fitbitmap :
                                             NULL;
                        checkmem( i->b =
                                     w_addimagebutton(w, w_b_press, 0, y, -1,
                                                      -1,
                                                      i->type ==
                                                      it_texture ? pig.txt_bm
                                                      : (i->type ==
                                                         it_thingtexture ?
                                                         pig
                                                         .
                    ...
                    case 9:
                        ...
                        b_press->delay = 25;
                        b_press->repeat =
                            (i->multifuncnr == 2 || i->multifuncnr == 3
                            || i->multifuncnr == 7 ||
                             i->multifuncnr == 8 ? 10 : -1);
                        b_press->l_pressed_routine =
                            b_press->r_pressed_routine =
                                b_press->l_routine =
                                    b_press->r_routine = NULL;

                        if ( (i->multifuncnr - 1) % 5 == 1 ||
                            (i->multifuncnr - 1) % 5 == 2 ) { /* up or down */
                            ...
                            b_press->l_routine =
                                i->type ==
                                it_dooranim ? b_l_change_dooranim :
                                b_l_change_txt;
                            b_press->r_routine =
                                i->type ==
                                it_dooranim ? b_r_change_dooranim :
                                b_r_change_txt;
                        }
                    ...
                    case 5:
                        ...
                        if (no < pig.num_rdltxts && pig.rdl_txts[no].pig !=
                            NULL) {
                            sprintf(b_string->str, "%s (%lu)",
                                    (no == 0 && i->offset == 2) ? TXT_NOTHING
...
void makeoptwindow(enum infos what) {
    ...
    y = makeoptbuttons(optionwins[what], init.infonum[what], init.info[what],
                       y);
    ...
    switch (i->type) {
        case it_selbutton:
            ...
            if ( ( activate = getno(i, &no, NULL) ) != 0 ) {
                for (n = 0, i->b->d.ls->selected = -1; n < i->od->size;
                     n++) {
                    if (no == i->od->data[n]->no) {
                        i->b->d.ls->selected = n;
                        break;
        ...
        case it_markbutton:
            ...
            if ( ( activate =
                      ( ( data =
                             getdata(i->infonr, NULL) ) != NULL ) ) != 0 ) {
                data += i->offset;

                for (n = 0; n < i->od->size; n++) {
                    i->b->d.tag->tagged[n] = i->od->data[n]->no < i->length *
                                             8 ?
                                             ( ( data[i->od->data[n]->no /
                                                      8] &
                                                ( 1 <<
        ...
        case it_fl_delay:
            activate =
                ( ( w = (struct wall *)getdata(ds_wall, NULL) ) != NULL );
        ...
        case it_texture:
            activate =
                ( ( w = (struct wall *)getdata(i->infonr, NULL) ) != NULL );
        ...
        case it_dooranim:
            ...
            if (i->multifuncnr == 0) {
                ...
                if (activate && w != NULL && no >= 0 && no < pig.num_anims &&
                    pig.anims[no] != NULL) {
                    drawdooranim(NULL, pig.anims[no], 0, w->texture1,
                                 w->texture2,
                                 w->txt2_direction);
                }
            }
        ...
        case it_thingtexture:
            ...
            if (i->multifuncnr == 0) {
                ...
                if (activate && sno >= 0 && sno < pig.num_rdltxts &&
                    pig.rdl_txts[sno].pig) {
                ...
                switch (what) {
                    case in_cube:
                        ...
                        if (act && view.pcurrcube) {
                            i = view.pcurrcube->no;
                            b_optwins[what][3]->d.s->on = testtag(
                                what, view.pcurrcube);
                                ...
                                if (view.b_levels->selected != j) {
                                    if (view.pcurrmacro
                                       && strcmp(view.b_levels->options[j],
                                                 view.pcurrmacro->fullname)
                                        == 0) {
                                        b_optwins[what][6]->d.ls->selected =
                                            k;
                                    }
                    ...
                    case in_wall:
                        ...
                        if (act && view.pcurrcube) {
                            i = view.currwall;
                            b_optwins[what][3]->d.s->on = testtag(
                                what, view.pcurrcube, view.currwall);
                            b_refreshtagno(what);
                            ...
                            if (b_optwins[what][6]) {
                                b_optwins[what][6]->d.s->on =
                                    ...
                                    == view.currwall
                                    && view.
                                    pdefcube->d.c->walls[view.defwall];
                            }
