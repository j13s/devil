                     ...
                     struct node *cube)
{
    ...
    for (i = 0; i < 9; i++) {
        n->d.t->orientation[i] = ld->secret_orient[i];
    }
...
int checkpntcube(struct node *nc, struct point *p) {
    return ( testvector(nc->d.c, 0, 1, 3, 4,
                        p) && testvector(nc->d.c, 6, 2, 5, 7, p) );
}
...
void setthingpts(struct thing *t) {
    ...
    switch (t->type1) {
        ...
        default:
            ...
            for (i = 0; i < 3; i++) {
                for (j = 0; j < 3; j++) {
                    coords[i].x[j] = t->orientation[i * 3 + j] / 65536.0;
                }
            }
...
unsigned char *vl_getdata(enum datastructs it, struct node *n,
                          va_list args) {
    void *data = NULL;
    int a, b;
    ...
    switch (it) {
        ...
        case ds_corner:
            ...
            if (n != NULL && n->d.c->walls[a] != NULL) {
                data = &n->d.c->walls[a]->corners[b];
            }
...
size_t getsize(enum datastructs it, struct thing *t) {
    ...
    switch (it) {
        ...
        case ds_leveldata:
            size = sizeof(struct leveldata);
            break;

        case ds_internal:
            size = sizeof(struct viewdata);
            break;
        ...
        case ds_thing:
            ...
            switch (t->type1) {
                ...
                default:
                    fprintf(errf, "getsize: Unknown thing-type: %.2x\n",
                            (unsigned)t->type1);
                    my_exit();
            }
...
int setno(struct infoitem *i, void *no, struct node *n, ...) {
    ...
    if ( ( data =
              vl_getdata(i->infonr, n, args) ) != NULL && i->offset >= 0 ) {
...
enum sdoortypes getsdoortype(struct sdoor *sd) {
    switch (sd->type) {
        ...
        case switch_turnofflight:
        case switch_turnonlight:
            return sdtype_side;
        ...
        case switch_closedoor:
        case switch_illusion_on:
        case switch_illusion_off:
        case switch_unlockdoor:
        case switch_lockdoor:
        case switch_openwall:
        case switch_closewall:
        case switch_wall_to_ill:
            return sdtype_door;

        default:
            fprintf(errf, "Unknown sdoortype: %x\n", sd->type);
            my_assert(0);
            return 0;
    }
}
