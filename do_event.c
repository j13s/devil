...
void dec_loadlevel(int ec) {
    ...
    if ( ( fname = getfilename(&init.levelpath, NULL, init.alllevelexts,
        ...
        if ( ( ld = openlevel(fname) ) == NULL ) {
            waitmsg(TXT_CANTOPENLVL, fname);
        }
        else {
            printmsg(TXT_LEVELREAD, ld->fullname, ld->filename);
        }
...
void dec_savelevel(int ec) {
    ...
    if (l->filename != NULL && init.d_ver >= d2_10_sw
       && strchr(l->filename,
                 '.') - l->filename == strlen(l->filename) - 4) {
...
void help_refresh(struct w_window *w) {
    ...
    ws_drawfilledbox(w_xwinincoord(w, 0), w_ywinincoord(w, o_y),
                     w_xwininsize(w), w_ywininsize(
                         w) - o_y, view.color[BLACK], 0);

    for (i = cur_help_pos, x = 0, y = 0;
         i < (view.num_keycodes > num_codes + cur_help_pos ?
              num_codes +
              cur_help_pos : view.num_keycodes);
         i++, y++) {
        ...
        for (j = 0; j < NUM_KEYDESCR; j++) {
            if (view.ec_keycodes[i].key == txt_key[j].key) {
                sprintf(&buffer[strlen(buffer)], "%s: ", txt_key[j].txt);
                break;
            }
        }
        ...
        if (j >= NUM_KEYDESCR) {
            if ( !isgraph(view.ec_keycodes[i].key) ) {
                ...
                sprintf(&buffer[strlen(
                                    buffer)], "%c: ", view.ec_keycodes[i].key);
            }
        }
...
void dec_help(int ec) {
    ...
    if ( (w_ymaxwinsize() - 20) / w_titlebarheight() * num_columns >
        view.num_keycodes ) {
        num_columns = view.num_keycodes * w_titlebarheight() /
        (w_ymaxwinsize() - 20) + 1;
    }
    ...
    checkmem( b_up =
                 w_addstdbutton(w, w_b_press, 0, 0, 50, -1, TXT_UP, &up, 1) );
    checkmem( b_down =
                 w_addstdbutton(w, w_b_press, 50, 0, 50, -1, TXT_DOWN, &down,
                                1) );
    checkmem( b_ok =
                 w_addstdbutton(w, w_b_press, 100, 0, w_xwininsize(w) - 100,
                                -1,
                                TXT_OK, &ok, 1) );
    w->data = (void *)(b_ok->ysize + 2);
    cur_help_pos = 0;
    num_codes =
        (w_ymaxwinsize() - b_ok->ysize -
         2) / w_titlebarheight() * num_columns;
    help_refresh(w);
    exitkey.kbstat = 0;
    exitkey.key = 27;
    exitkey.event = -1;
...
void dec_changefullname(int ec) {
    ...
    for (i = 0; i < view.b_levels->num_options; i++) {
        if (view.b_levels->options[i] == l->fullname) {
            break;
        }
    }
    ...
    s.l_char_entered = s.r_char_entered = s.l_string_entered =
                                              s.r_string_entered = NULL;
    checkmem( b =
                 w_addstdbutton(NULL, w_b_string, 0, 0, -1, -1, TXT_LVLNAME,
                                &s, 0) );
    ...
    w_closewindow(ow);
    FREE(l->fullname);
    checkmem( view.b_levels->options[i] = l->fullname =
                                              MALLOC(strlen(s.str) + 1) );
...
void dec_nextlevel(int ec) {
    if (!l) {
        if (view.levels.size > 0)          {
            changecurrentlevel(view.levels.tail->d.lev);
        }
    }
    else {
...
void dec_prevlevel(int ec) {
    if (!l) {
        if (view.levels.size > 0)          {
            changecurrentlevel(view.levels.head->d.lev);
        }
    }
    else {
...
void dec_drawwhat(int ec) {
    switch (ec) {
        case ec_drawalllines:
            view.drawwhat ^= DW_ALLLINES;

            if ( !(view.drawwhat & DW_ALLLINES) ) {
                initdescmap();
            }
    ...
    for (i = 0; i < tlw_num; i++) {
        if (tl_win[i].oldxsize != 0) {
            fprintf(f, " %d %d %d %d %d\n", tl_win[i].zoom.selected,
                    tl_win[i].oldxpos, tl_win[i].oldypos, tl_win[i].oldxsize,
                    tl_win[i].oldysize);
        }
        else {
            fprintf(f, " -1\n");
...
void(*do_event[ec_num_of_codes]) (int ec) = {
    ...
    dec_gowall, dec_sidecube, dec_aligntoaxis, dec_aligntoaxis,
    dec_aligntoaxis,
    ...
    dec_movefactor, dec_rotangle, dec_rotangle, dec_visibility,
    dec_visibility,
    ...
    dec_savepos, dec_savepos, dec_savepos, dec_savepos, dec_savepos,
    dec_savepos,
    dec_savepos, dec_savepos, dec_savepos, dec_savepos, dec_savepos,
    dec_savepos,
    dec_gotopos, dec_gotopos, dec_gotopos, dec_gotopos, dec_gotopos,
    dec_gotopos,
    dec_gotopos, dec_gotopos, dec_gotopos, dec_gotopos, dec_gotopos,
    dec_gotopos,
