...
void untagcube(struct node *tn, va_list args) {
    if (tn->d.c->tagged) {
        freenode(&l->tagged[tt_cube], tn->d.c->tagged, NULL);
    }
...
void untagthing(struct node *tn, va_list args) {
    if (tn->d.t->tagged) {
        freenode(&l->tagged[tt_thing], tn->d.t->tagged, NULL);
    }
...
void untagdoor(struct node *tn, va_list args) {
    if (tn->d.d->tagged) {
        freenode(&l->tagged[tt_door], tn->d.d->tagged, NULL);
    }
...
int tag(enum tagtypes tt, struct node *data, ...) {
    ...
    if (data != NULL) {
        return tag_something[tt](data, args);
    }
...
void(*untag_something[tt_number]) (struct node *tn, va_list args) = {
    untagcube,
...
void untagall(enum tagtypes tt) {
    ...
    for (n = l->tagged[tt].head; n->next != NULL; n = n->next) {
        switch (tt) {
            ...
            case tt_wall:
                untag(tt, n->d.n, n->no % 6);
                break;

            case tt_edge:
                untag(tt, n->d.n, (n->no % 24) / 4, (n->no % 24) % 4);
                break;

            default:
                waitmsg("Unknown tagtype in untagall.");
        }
    }
}
...
int testtag(enum tagtypes tt, struct node *n, ...) {
    ...
    switch (tt) {
        case tt_cube:
            ret = n->d.c->tagged != NULL;
            break;

        case tt_wall:
            ret = n->d.c->tagged_walls[va_arg(args, int)] != NULL;
            break;
        ...
        case tt_pnt:
            ret = n->d.lp->tagged != NULL;
            break;

        case tt_thing:
            ret = n->d.t->tagged != NULL;
            break;

        case tt_door:
            ret = n->d.d->tagged != NULL;
            break;

        default:
            waitmsg("Unknown tagmode %d in testtag.", tt);
            ret = 0;
    }
...
int tag_testfunc(int lr, struct point *p, int x1, int y1, int x2,
                 int y2)
{
    struct pixel pix;


    if (!getpixelcoords(lr, p, &pix) || pix.d > view.maxvisibility * 1.1) {
        return 0;
    }
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            for (n = l->cubes.head; n->next != NULL; n = n->next) {
                for (i = 0; i < 8; i++) {
                    if ( !tag_testfunc(lr, n->d.c->p[i]->d.p, dx1, dy1, dx2,
                                       dy2) ) {
                        break;
                    }
                }
        ...
        case tt_wall:
            ...
            for (n = l->cubes.head; n->next != NULL; n = n->next) {
                for (j = 0; j < 6; j++) {
                    for (i = 0; i < 4; i++) {
                        if ( !tag_testfunc(lr, n->d.c->p[wallpts[j][i]]->d.p,
                                           dx1, dy1, dx2, dy2) ) {
                            break;
                        }
                    }
            ...
            for (n = l->cubes.head; n->next != NULL; n = n->next) {
                for (j = 0; j < 6; j++) {
                    for (k = 0; k < 3; k++) {
                        ...
                        for (i = 0; i < 4; i++) {
                            c.x[k] += n->d.c->p[wallpts[j][i]]->d.p->x[k] /
                                      4.0;
                        }
                    }
                    ...
                    for (i = 0; i < 4; i++) {
                        for (k = 0; k < 3; k++) {
                            p.x[k] = c.x[k] * 0.4 +
                                     n->d.c->p[wallpts[j][i]]->d.p->x[k] *
                                     0.6;
                        }

                        if ( tag_testfunc(lr, &p, dx1, dy1, dx2, dy2)
                           && tag_testfunc(lr, n->d.c->p[wallpts[j][i]]->d.p,
                                           dx1, dy1, dx2, dy2) ) {
                            if (op) {
...
void tagobject(enum infos what) {
    ...
    switch (what) {
        case tt_cube:
            ...
            if ( !tag(what, view.pcurrcube) ) {
                untag(what, view.pcurrcube);
            }
        ...
        case tt_pnt:
            ...
            if ( !tag(what, view.pcurrpnt) ) {
                untag(what, view.pcurrpnt);
            }
...
int comp_tagfilter(struct infoitem *i, unsigned char *d1,
                   unsigned char *d2) {
    int j;


    for (j = 0; j < i->length; j++) {
        if ( *(d1 + i->offset + j) != *(d2 + j) ) {
            break;
        }
    }
...
void tagfilter(struct infoitem *i, int op, void *data) {
    ...
    if (i->type == it_cubelight || i->type == it_sidelight || i->type ==
        it_thingcoord
    ...
    switch (i->tagnr) {
        ...
        case tt_door:
            ...
            for (n = i->tagnr == tt_cube ? l->cubes.head : l->doors.head,
                 found = tagged = 0;
        ...
        case tt_thing:
            ...
            for (n = l->things.head, found = tagged = 0; n->next != NULL;
                 n = n->next) {
        ...
        case tt_wall:
            ...
            for (n = l->cubes.head, tagged = found = 0; n->next != NULL;
                 n = n->next) {
                for (w = 0; w < 6; w++) {
                    if (n->d.c->walls[w]) {
                        if ( comp_tagfilter(i,
        ...
        default:
            fprintf(errf, "Unknown tagnr: %d\n", i->tagnr);
            my_assert(0);
    }

    if (op) {
        printmsg(TXT_TAGFILTERUNTAGGED, -tagged, found + tagged);
    }
    else {
        printmsg(TXT_TAGFILTERTAGGED, tagged, found - tagged);
    }
    ...
    do {
        ...
        if (ox != x || oy != y) {
            ws_drawbox(sx, sy, w_xwinincoord(we->w, ox + xo) - sx,
                       w_ywinincoord(we->w,
                                     oy + yo) - sy, view.color[WHITE], 1);
            ox = x;
            oy = y;
            ws_drawbox(sx, sy, w_xwinincoord(we->w,
                                             x + xo) - sx,
                       w_ywinincoord(we->w, y + yo) - sy,
                       view.color[WHITE], 1);
        }
