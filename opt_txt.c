...
const char *tl_title[tlw_num] = {
    "Texture 1", "Texture 2", "Animation",
...
const char *tlw_modes[2] = {
    TXT_TLW_TXTMODE, TXT_TLW_LISTMODE
};
const char *tlw_zoom[3] = {
    "/1", "/2", "/4"
};
...
int next_txtno(struct txt_list_win *tlw, int offset, int add) {
    ...
    while (add != 0) {
        ...
        if (tlw->t[offset]->txtlistno[tt] >= 0 && tlw->t[offset]->rdlno !=
            rdlno) {
            add -= incdec;
...
int tl_gettxtno(struct w_button *b) {
    ...
    return next_txtno(
               ...
               (b->ypos -
                tlw->txt_yoffset) / tlw->txt_ysize * tl_xnumtxt(tlw) );
}
...
void initdisplay(struct txt_list_win *tlw) {
    ...
    if (tlw->mode.selected == 0) {
        ...
        if (tlw->type == tlw_anim && rdl_curtxt >= 0 && rdl_curtxt <
            pig.num_anims) {
    ...
    if (i < tlw->maxnum) {
        tlw->marked_txts[i] = 1;
        tlw->offset = next_txtno(tlw, i - tl_xnumtxt(tlw) * tl_ynumtxt(
                                     tlw) / 2 < 0 ? -1 :
...
void tl_drawmarktxt(struct w_button *b, int on) {
    ...
    ws_drawbox(w_xwinincoord(tlw->win, b->xpos - 1),
               w_ywinincoord(tlw->win,
                             b->ypos - 1), b->xsize + 2, b->ysize + 2,
    ...
    for (y = 0, i = tlw->offset; y < tl_ynumtxt(tlw); y++) {
        for (x = 0; x < tl_xnumtxt(tlw); x++) {
            switch (tlw->zoom.selected) {
                case 0:
                    size = 64;
                    memset(tlw->txt_buffer[y * tl_xnumtxt(
                                               tlw) + x], view.color[BLACK],
                           64 * 64);

                    if (i < tlw->maxnum) {
                        readbitmap(tlw->txt_buffer[y * tl_xnumtxt(
                                                       tlw) + x], NULL,
                                   tlw->t[i], 0);
                    }
                ...
                case 1:
                    ...
                    if (i < tlw->maxnum) {
                        ...
                        for (x2 = 0; x2 < 32; x2++) {
                            for (y2 = 0; y2 < 32; y2++) {
                                tlw->txt_buffer[y * tl_xnumtxt(tlw) + x][y2 *
                                                                         32 +
                                                                         x2]
                                =
            ...
            if (i < tlw->maxnum && tlw->t[i] != NULL) {
                ...
                if (view.littlebulbson && x2 < ILLUM_GRIDSIZE *
                    ILLUM_GRIDSIZE) {
            ...
            tl_drawmarktxt(tlw->b_texture[y * tl_xnumtxt(
                                              tlw) + x], i < tlw->maxnum
                          && tlw->marked_txts[i]);
    ...
    if (tlw->oldxsize != w->xsize || tlw->oldysize != w->ysize
       || tlw->oldmode != tlw->mode.selected) {
        ...
        if (tl_xtxtspace(tlw) % tlw->txt_xsize != 0
           || tl_ytxtspace(tlw) % tlw->txt_ysize != 0 || tlw->oldmode !=
            tlw->mode.selected) {
            ...
            if ( x > w_xmaxwininsize(tlw->win) ) {
                x =
                    (w_xmaxwininsize(tlw->win) /
                     tlw->txt_xsize) * tlw->txt_xsize;
            }
            ...
            if ( y > w_ymaxwininsize(tlw->win) ) {
                y =
                    ( (w_ymaxwininsize(tlw->win) - tlw->b_up->ysize *
                       2) / tlw->txt_ysize ) *
                    tlw->txt_ysize + tlw->b_up->ysize * 2;
            }

            tlw->b_up->xsize = x - tlw->b_zoom->xpos - tlw->b_zoom->xsize;
            tlw->b_down->xsize = x - tlw->b_display->xpos -
                                 tlw->b_display->xsize;
            tlw->b_m_down->xsize = x - tlw->b_m_save->xpos -
                                   tlw->b_m_save->xsize;
            tlw->b_down->ypos = tlw->b_display->ypos = tlw->b_m_clear->ypos =
                                                           tlw->b_m_load->
                                                           ypos =
                                                               tlw->b_m_save
                                                               ->ypos = tlw->b_m_level->ypos =
                                                                            tlw
                                                                            ->
                                                                            b_m_down
                                                                            ->ypos = y - tlw->b_down->ysize;
            tlw->dontcalltwice = 1;
            w_resizeinwin(w, x, y);
            tlw->dontcalltwice = 0;
        }
        ...
        for (y = 0; y < tl_ynumtxt(tlw); y++) {
            for (x = 0; x < tl_xnumtxt(tlw); x++) {
                checkmem( tlw->txt_buffer[y * tl_xnumtxt(tlw) + x] =
                             MALLOC( (tlw->txt_xsize - TXT_EDGE *
                                      2) * (tlw->txt_ysize - TXT_EDGE * 2) ) );
                checkmem( tlw->txt_bitmap[y * tl_xnumtxt(
                                              tlw) + x] = ws_createbitmap(
                             tlw->txt_xsize - TXT_EDGE * 2, tlw->txt_ysize -
                             TXT_EDGE * 2,
                             tlw->txt_buffer[y * tl_xnumtxt(tlw) + x]) );
                checkmem( tlw->b_texture[y * tl_xnumtxt(
                                             tlw) +
                                         x] = w_addimagebutton(tlw->win,
                                                               w_b_press,
                                                               x *
                                                               tlw->txt_xsize
                                                               ...
                                                               *
                                                               tlw->txt_ysize
                                                               + tlw->txt_yoffset + TXT_EDGE - 1, -1, -1,
                                                               tlw
                                                               ->
                                                               txt_bitmap[y *
                                                                          tl_xnumtxt(tlw) + x],
                                                               ...
                                                               ->
                                                               txt_bitmap[y *
                                                                          tl_xnumtxt(tlw) + x], &tlw->texture, 1, 0) )
...
void tl_makelvllist(struct txt_list_win *tlw) {
    ...
    switch (tlw->type) {
        ...
        case tlw_t2:
            ...
            if (tlw->type == tlw_t2) {
                checkmem( tlw->leveltxtlist =
                             MALLOC( ++tlw->num_leveltxtlist *
                                    sizeof(struct ham_txt *) ) );
...
void tl_change_texture(struct w_button *b, int withtagged) {
    ...
    if (txt_num >= 0 && txt_num < tlw->maxnum) {
        ...
        if (tlw->type == tlw_anim) {
            for (animnum = 0; animnum < pig.num_anims;
                 animnum++)                               {
...
unsigned short tl_readfile(unsigned short **data) {
    ...
    fname =
        getfilename(&init.txtlistpath, NULL, init.d_ver <
                    d2_10_sw ? "TL1" : "TL2",
                    TXT_TLW_TITLELOAD,
                    0);
            ...
            for (n = l->cubes.head; n->next != NULL; n = n->next) {
                for (w = 0; w < 6; w++) {
                    if (n->d.c->walls[w]) {
                        ...
                        if ( (tlw->type == tlw_t2 && rdlno ==
                              0) || rdlno < 0 || rdlno >= pig.num_rdltxts
                           || !pig.rdl_txts[rdlno].pig ) {
                            continue;
                        }
...
void texture_list(struct infoitem *i, enum txttypes tt, int no) {
    ...
    if (!tl_win[tlw_type].win) {
        ...
        checkmem( tl_win[tlw_type].b_mode =
                     w_addstdbutton(tl_win[tlw_type].win,
                                    w_b_choose, 0, 0,
                                    -1, -1, TXT_TLW_MODE,
                                    &tl_win[tlw_type].mode, 1) );
        ...
        checkmem( tl_win[tlw_type].b_m_down =
                     w_addstdbutton(tl_win[tlw_type].win,
                                    ...
                                    tl_win[tlw_type].b_m_save->xsize,
                                    tl_win[tlw_type]
                                    .b_display->ysize, TXT_TLW_DOWN,
                                    &tl_win[tlw_type].down, 0) );
        checkmem( tl_win[tlw_type].b_down =
                     w_addstdbutton(tl_win[tlw_type].win,
                                    ...
                                    TXT_TLW_DOWN,
                                    &tl_win[tlw_type].down, 1) );
        tl_win[tlw_type].b_mode->data = tl_win[tlw_type].b_up->data =
                                            tl_win[tlw_type].b_display->data
                                            = tl_win[tlw_type].b_down->data =
                                                  tl_win
                                                  [tlw_type].b_zoom->data =
                                                      tl_win[tlw_type].b_m_clear->data =
                                                          tl_win
                                                          [
                                                              tlw_type].b_m_load->data =
                                                              tl_win
                                                              [tlw_type].b_m_save->data =
                                                                  tl_win
                                                                  [
                                                                      tlw_type
                                                                  ].b_m_level->data = tl_win[tlw_type].b_m_down->data =
                                                                                          &
                                                                                          tl_win
                                                                                          [
                                                                                              tlw_type];
        ...
        tl_win[tlw_type].oldtxttyp = view.pcurrcube != NULL
                                     && view.pcurrcube->d.c->nc[view.currwall
                                     ] != NULL;
...
void refresh_txtlist(struct infoitem *i) {
    ...
    int newtxttyp =
        (view.pcurrcube != NULL && view.pcurrcube->d.c->nc[view.currwall] !=
         NULL);
...
void drawdooranim(struct w_button *b, struct ham_txt *t, int anim, int t1,
                  int t2,
                  int t2_d)
{
    if (t->pig && t->pig->anim_t2) {
        ...
        if (t != NULL) {
            readbitmap(pig.door_buffer, &pig.pig_txts[t->pigno + anim], NULL,
                       0);
        }
    }
...
void b_l_playdooranim(struct w_button *b) {
    if (view.pcurrdoor != NULL && view.pcurrdoor->d.d->w != NULL
       ...
       && pig.anims[view.pcurrdoor->d.d->animtxt]->pig != NULL) {
        b->d.p->repeat = 50 /
                         pig.anims[view.pcurrdoor->d.d->animtxt]->pig->
                         num_anims;
...
void drawtxtlines(struct w_button *b, struct wall *w) {
    ...
    ws_setclipping( w_xwinincoord(b->w,
                                  b->xpos + 1),
                   w_ywinincoord(b->w, b->ypos + 1),
    ...
    l =
        sqrt( (c->x[0] -
               c0->x[0]) *
             (c->x[0] - c0->x[0]) + (c->x[1] - c0->x[1]) * (c->x[1] -
                                                            c0
                                                            ->x
                                                            [1]) ) / 32;
...
void fb_refreshwin(struct w_window *w, void *d) {
    ...
    ws_setclipping(w_xwinincoord(w, 0), w_ywinincoord(w, 0),
                   w_xwinincoord(w,
                                 0) + w_xwininsize(w) - 1, w_ywinincoord(w,
                                                                         0) +
                   w_ywininsize(w) - 1);
    ...
    for (y = -yn + 1; y < yn; y++) {
        for (x = -xn + 1; x < xn; x++) {
            ws_copybitmap(NULL, xo + x * 64, yo + y * 64, fb_data.bm_txt, 0,
                          0, 64, 64,
                          1);
        }
    }
    ...
    for (x = 0; x < 4; x++) {
        ws_drawline(
            xo + fb_data.c[x].x[0] / 32, yo + fb_data.c[x].x[1] / 32,
            xo +
            fb_data.c[(x +
                       1) &
                      3].x[0] / 32, yo + fb_data.c[(x + 1) & 3].x[1] / 32,
            view.color[HILIGHTCOLORS +
                       ( x == 0 ? 3 : (x == 3 ? 4 : 1) )], 0);
    }

    if (c != NULL) {
        ws_drawcircle(xo + fb_data.c[*c].x[0] / 32, yo + fb_data.c[*c].x[1] /
                      32, 3,
                      view.color[HILIGHTCOLORS + 1],
                      0);
    }
...
void fb_clickwin(struct w_window *w, void *d, struct w_event *we) {
    ...
    if ( we->x < 0 || we->y < 0 || we->x >= w_xwininsize(w) || we->y >=
        w_ywininsize(w) ) {
        return; /* click on a system button */
    }

    if ( (we->ws->buttons & ws_bt_left) == 0 ) {
        move_texture_with_mouse(w_xwinincoord(w,
                                              we->x), w_ywinincoord(w, we->y),
                    ...
                    if ( (ws.buttons & ws_bt_left) == 1 ) {
                        if (ws.kbstat & ws_ks_shift) {
                            if ( yesnomsg(TXT_FB_RECALCUV) )
                            {
                                ...
                                for (i = 0; i < 4; i++) {
                                    fb_data.c[i] = view.pcurrwall->corners[i];
                                }
