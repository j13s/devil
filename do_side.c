                          ...
                          int tagged)
{
    ...
    for (k = 0; k < 6; k++) {
        if (n->d.c->walls[k] != NULL) {
            for (j = 0; j < 4; j++) {
                n->d.c->walls[k]->corners[j].light = *no;
            }
        }
    }
                          ...
                          int tagged)
{
    ...
    if (n->d.c->walls[wallno] != NULL) {
        for (j = 0; j < 4; j++) {
            n->d.c->walls[wallno]->corners[j].light = *no;
        }
    }
                   ...
                   int tagged)
{
    ...
    if (*no != n->d.t->type1) {
        ...
        if (n->d.t->type1 == tt1_secretstart) {
            l->secretstart = n;
        }
              ...
              int tagged)
{
    ...
    if (n->d.t->type1 == tt1_robot) {
        if (*no >= 0 && *no < NUM_ROBOTS) {
            n->d.t->size = robotsize[*no];
        }
        else {
            n->d.t->size = D2_stdrobot.t.size;
        }
    }
        ...
        if (*no >= 0 && *no < NUM_REACTORS) {
            ...
            ( (struct reactor *)n->d.t )->r.model_num =
                stdreactor.r.model_num;
        }
    }
    else {
        n->d.t->size = view.tsize * 2;
    }
...
int itemgrfx[48] = {
    0x24, 0x12, 0x13, 0x14, 0x18, 0x1a, 0x19, 0x12, 0x12, 0x12,
    0x22, 0x23, 0x33, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d,
    0x2e, 0x2f, 0x12,
    0x31, 0x12, 0x12, 0x45, 0x46, 0x47, 0x48, 0x4d, 0x49, 0x4a, 0x4b, 0x4c,
    0x53, 0x4e, 0x59,
...
int changedoortype(struct node *n, int no, int tagged) {
    ...
    if (no == door1_blow) {
        ...
        if (!tagged && view.pcurrwall == n->d.d->w) {
            drawopt(in_wall);
        }
    }
    else {
    ...
    if (no == door1_onlyswitch) { /* only switch: set texture to empty/nothing
        ...
        n->d.d->w->texture1 = init.d_ver >=
                              d2_10_sw ? D2_std_emptytxt : D1_std_emptytxt;
    ...
    if (no == door1_switchwithwall) {
        ...
        if (n->d.d->sd) {
            n->d.d->sd->d.sd->flags |= switchflag_once;
        }
    ...
    if (no != door1_blow && no != door1_normal) { /* no door, no blow door -> no
                                                    anim*/
        n->d.d->animtxt = -1;
    }

    if (no != door1_normal) {
        n->d.d->key = 1;             /* no door, no key */
    }
                 ...
                 int tagged)
{
    ...
    if ( ok && n->d.d->d && (no == door1_normal || no == door1_blow
                            || n->d.d->d->d.d->type1 == door1_normal ||
                             n->d.d->d->d.d->type1 == door1_blow) ) {
                 ...
                 int tagged)
{
    ...
    if (!pig.anims[no]->pig->anim_t2) {
        ...
        if (n->d.d->old_txt1 == -1) {
            n->d.d->old_txt1 = n->d.d->w->texture1;
        }
        ...
        if (n->d.d->old_txt1 >= 0) {
            n->d.d->w->texture1 = n->d.d->old_txt1;
        }
    ...
    if ( !tagged
       && (n->d.d->w == view.pcurrwall || n->d.d->d->d.d->w ==
           view.pcurrwall) ) {
        drawopt(in_wall);
    }
               ...
               int tagged)
{
    ...
    switch ( getsdoortype(sd) ) {
        ...
        case sdtype_cube:
            ...
            for (sn = l->tagged[tt_cube].head; sn->next != NULL;
                 sn = sn->next) {
                if (sn->d.n->d.c->type != 4) {
                    if (!pntno)                           {
                        printmsg(TXT_TAGGEDCUBENOPROD, sn->d.n->no);
                    }
                ...
                if (sd->num >= 10) {
                    if (!pntno)                 {
                        printmsg(TXT_TOOMUCHCUBESTAGGED);
                    }
        ...
        case sdtype_side:
            ...
            for (sn = l->tagged[tt_wall].head; sn->next != NULL;
                 sn = sn->next) {
                if (sd->num >= 10) {
                    if (!pntno)                 {
                        printmsg(TXT_TOOMUCHCUBESTAGGED);
        ...
        case sdtype_door:
            ...
            for (sn = l->tagged[tt_door].head; sn->next != NULL;
                 sn = sn->next) {
                if (sn->no == n->no) {
                ...
                if (sn->d.n->d.d->c->d.c->nc[sn->d.n->d.d->wallnum] ==
                    NULL) {
                    printmsg(TXT_NOSWITCHABLEWALL);
                    continue;
                }

                if ( (no == switch_opendoor || no == switch_closedoor ||
                      no == switch_unlockdoor
                     || no ==
                      switch_lockdoor) && sn->d.n->d.d->type1 !=
                    door1_normal ) {
                    printmsg(TXT_TAGGEDWALLNODOOR, sn->d.n->no);
                    continue;
                }

                if ( (no == switch_openwall || no == switch_closewall ||
                      no == switch_illusion_off
...
int dsc_illum_brightness(struct infoitem *i, void *d, struct node *n,
                         int wallno, int pntno,
                         int tagged)
{
    float dno = *(float *)d;
...
int dsc_changedrawwhat(struct infoitem *i, void *d, struct node *n,
                       int wallno, int pntno,
                       int tagged)
{
    if ( !setno(i, d, n) ) {
                 ...
                 int tagged)
{
    ...
    if (!l || !n || !n->d.c->walls[wallno] || !n->d.c->walls[wallno]->ls) {
        return 0;
    }
    ...
    if (!n->d.c->walls[wallno]->ls->d.ls->fl) {
        ...
        for (nc = n->d.c->walls[wallno]->ls->d.ls->effects.head;
             nc->next != NULL;
             nc = nc->next) {
        ...
        fl = n->d.c->walls[wallno]->ls->d.ls->fl;
    }
...
int(*dsc_sideeffect[sc_number]) (struct infoitem *i, void *d,
                                 struct node *n, int wallno, int pntno,
                                 int tagged) = {
    dsc_cubetype, dsc_set_avg_cubelight, dsc_set_t2_dir,
    dsc_set_avg_sidelight,
