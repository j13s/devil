...
enum sdoortypes {
    sdtype_none, sdtype_door, sdtype_cube, sdtype_side
};
...
enum cubetypes {
    cube_normal = 0, cube_fuel = 1, cube_reactor = 3, cube_producer = 4,
    cube_blueflag = 5, cube_redflag = 6
};
/* several standard textures etc. */
enum miscel {
    std_anim_blowdoor = 6, std_switch_t2 = 414, D1_std_emptytxt = 289,
    D2_std_emptytxt = 302, std_blowdoorhp = 0x64 << 16
};
enum thingtype1 {
    tt1_robot = 2, tt1_hostage = 3, tt1_dmstart = 4, tt1_mine = 5,
    tt1_item = 7, tt1_reactor = 9, tt1_coopstart = 14,
    /* for me: */ tt1_secretstart = 0xff
};
/* only the needed IDs */
enum itemid {
    item_id_bluekey = 4, item_id_redkey = 5, item_id_yellowkey = 6,
    item_id_blueflag = 46, item_id_redflag = 47
};
/* subtypes of things */
enum controltype {
    ct_none = 0, ct_ai = 1, ct_slew = 5, ct_mine = 9, ct_powerup = 13,
    ct_control = 16
};
enum movementtype {
    mt_none = 0, mt_physics = 1, mt_spinning = 3
};
enum rendertype {
    rt_polyobj = 1, rt_hostage = 4, rt_powerup = 5
};
enum doortype1 {
    door1_blow = 1, door1_normal = 2, door1_onlytxt = 3,
    door1_onlyswitch = 4, door1_shootthrough = 5, door1_switchwithwall = 6,
    door1_cloaked = 7
};
enum doortype2 {
    door2_locked = 8, door2_closeagain = 0x10, door2_illusionoff = 0x20,
    door2_robotswontpass = 0x80
};
/* switchtypes */
enum switchtypes {
    switch_opendoor = 0, switch_closedoor = 1, switch_producer = 2,
...
enum switchflags {
    switchflag_nomsg = 1, switchflag_once = 2
};
