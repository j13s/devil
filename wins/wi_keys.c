    ...
    do {
        ...
        if ( b->b.type == w_b_press
           ...
           && (delay || b->b.d.p->repeat >= 0) ) {
            ...
            else if (b->b.d.p->r_pressed_routine) {
...
int wi_bk_choose(struct ws_event *ws, struct wi_button *b) {
    ...
    do {
        if (i >= 0 && i < b->b.d.ls->num_options && i != oldi) {
            ...
            ws_drawframedbox(oldx, oldy, cxs - 1, shapes.titlebar_height + 1,
                             1,
        ...
        if (ws->flags == ws_f_keypress) {
            switch (ws->key) {
                case 0x248:
                    i = i <= 0 ? b->b.d.ls->num_options - 1 : i - 1;
                    break;                                      /* up */

                case 0x250:
                    i = i >= b->b.d.ls->num_options - 1 ? 0 : i + 1;
                    break;                                      /* down */
                ...
                default:
                    ...
                    if (ws->key == L_BUTKEY && (ws->kbstat & R_KBSTAT) !=
                        R_KBSTAT) {
                        end = 2;
                    }
                    else if (ws->key == R_BUTKEY) {
                        end = 3;
                    }
            }
        }
    ...
    if (end > 1 && i >= 0 && i < b->b.d.ls->num_options) {
        if (end == 2 && b->b.d.ls->select_lroutine) {
...
int wi_bk_tag(struct ws_event *ws, struct wi_button *b) {
    int pos, leftright, xp, yp, xs, ys, num_selected, end = 0, cols,
        collength, cxs;
    ...
    do {
        ws_drawbox(xp + (pos / collength) * cxs + 1, yp + (pos % collength) *
                   ...
                   shapes.titlebar_height + 3,
                   notes.colindex[cv_buttonmarker],
                   0);
        ws_getevent(ws, 1);
        ...
        if (ws->flags == ws_f_keypress) {
            switch (ws->key) {
                case 0x248:
                    pos = pos <= 0 ? b->b.d.tag->num_options - 1 : pos - 1;
                    break;                                             /*up*/

                case 0x250:
                    pos = pos >= b->b.d.tag->num_options - 1 ? 0 : pos + 1;
                    break;                                            /*down*/
                ...
                default:
                    ...
                    if (ws->key == L_BUTKEY || ws->key == R_BUTKEY) {
                        if ( (num_selected +=
                            ...
                            b->b.d.tag->max_options &&
                            b->b.d.tag->max_options > 0 ) {
        ...
        if (ws->flags == ws_f_keypress) {
            switch (ws->key) {
                ...
                case 0x20:
                    ...
                    if (b->b.d.sls->sel_arr[pos] && ++num_sel >
                        b->b.d.sls->max_selected) {
            ...
            if (pos < b->b.d.sls->start_no) {
                ...
                if ( (b->b.d.sls->start_no = pos - b->b.d.sls->no_rows + 1) <
                    0 ) {
                    b->b.d.sls->start_no = 0;
                }
            }
