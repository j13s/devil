...
struct wi_button *wi_checkforbutton(struct wi_window *wi, int x, int y) {
    ...
    for (n = wi->buttonlist.head, bi = NULL; n->next != NULL; n = n->next) {
        if (n->d.w_b->drawn && x > n->d.w_b->b.xpos
           && x < n->d.w_b->b.xpos + n->d.w_b->b.xsize - 1 && y >
            n->d.w_b->b.ypos
...
int w_handleuser( int num_endbuts, struct w_button **endbuttons, int num_wins,
                 struct w_window **wins, int num_keycodes,
                 struct w_keycode *ec_keys, void(
                     **keyhandling) (int evcode) )
{
    struct ws_event ws;
    ...
    do {
        ...
        if (msg_win) {
            if (ws.buttons != ws_bt_none && ws.x >= msg_win->xpos
               && ws.x < msg_win->xpos + msg_win->xsize && ws.y >=
                msg_win->ypos
            ...
            i =
                ( !event ||
                 (ws.x >= msg_win->xpos && ws.x < msg_win->xpos +
                  msg_win->xsize
                 && ws.y >= msg_win->ypos && ws.y <
                  msg_win->ypos + msg_win->ysize) );

            if ( msg_win_x < ws.x - 20 || msg_win_y < ws.y - 20 ||
                msg_win_x > ws.x + 20
               || msg_win_y > ws.y + 20 ||
                ( event && (ws.flags == ws_f_keypress
                           || ws.buttons !=
                            ws_bt_none) ) ) {
                w_closewindow(msg_win);
                msg_win = NULL;
        ...
        if (event && ws.flags == ws_f_keypress) {
            ...
            if ( notes.cur_but != NULL &&
                (ws.key == 0x248 || ws.key == 0x250 || ws.key == 0xd) ) {
                n = notes.cur_but;
                ...
                switch (ws.key) {
                        ...
                        do {
                            ...
                            if (n->prev == NULL) {
                                n = *(&n->prev + 1);
                                 ...
                                 || n->d.w_b->
                                  sys_button || !n->d.w_b->tobechanged
                                 || wi_butkeyhandling[
                                      n->d.w_b->b.type] == NULL) );
                    ...
                    case 0xd: /* return */
                        ...
                        if (wi_butkeyhandling[n->d.w_b->b.type] != NULL) {
                            ...
                            if (wi_butkeyhandling[n->d.w_b->b.type](&ws,
                                                                    n->d.w_b)
                               && endbuttons) {
            ...
            if ( (ws.kbstat & ws_ks_ctrl) != 0 && isalnum(ws.key) ) {
                if (num_wins == 0) {
                    for (n = notes.windows.head; n->next != NULL; n =
                             n->next)                         {
        ...
        if (num_wins == 0 && ws.y < shapes.titlebar_height + 2 *
            shapes.width_menuframe
           && event) {
            wi_handlemenu(&ws);
        }
        else {
            for (n = notes.windows.head; n->next != NULL; n = n->next) {
                if (ws.x > n->d.w_w->w.xpos && ws.x < n->d.w_w->w.xpos +
                    n->d.w_w->w.xsize - 1
                   && ws.y > n->d.w_w->w.ypos && ws.y < n->d.w_w->w.ypos +
                    n->d.w_w->w.ysize - 1) {
                    wi = n->d.w_w;
                    we->w = &wi->w;
            ...
            if (event) {
                ...
                if ( ( bi = wi_checkforbutton(wi, we->x, we->y) ) != NULL ) {
                    ...
                    bi->b.event = ws;
                    /* if window is closed, all buttons are deleted, so this must
                       be done
                       before the window is closed. */
                    winclosed =
                        (bi->sys_button && !bi->b.data && ws.flags ==
                         ws_f_lbutton
                        && (wi->w.buttons & wb_close) != 0);

                    if ( bi->tobechanged && wi_buttonhandling[bi->b.type] !=
                        NULL
                       && wi_buttonhandling[bi->b.type](&ws, bi) ) {
                        ...
                        if (wins && winclosed && ++numclosedwins ==
                            num_wins) {
                            end = -1;
                        }
                    }
               ...
               const char *help)
{
    ...
    bxsize = ws_pixstrlen(ws_pixstrlen(ok) > ws_pixstrlen(
                              cancel) ? ok : cancel) + 10;
    i = 0;
    numlines = -1;
    ...
    for (i = 0; i < numlines && pos[i] != NULL; i++) {
        ws_drawtext(w_xwinincoord(w,
                                  ( w_xwininsize(w) -
                                   ws_pixstrlen(pos[i]) ) / 2),
                    w_ywinincoord(w,
                                  5 + i * shapes.titlebar_height), w->xsize,
                    pos[i],
    ...
    if (ok != NULL) {
        ...
        b_press.l_pressed_routine = b_press.r_pressed_routine =
                                        b_press.l_routine =
                                            ...
                                            r_routine = NULL;
        checkmem( b_arr[0] =
                     w_addstdbutton(w, w_b_press, cancel != NULL ? 5 :
                                    (w_xwininsize(w) -
                                     ws_pixstrlen(ok) + 4) / 2,
                                    w_ywininsize(w) -
                                    shapes.titlebar_height - 10, bxsize, -1,
                                    ok, &b_press, 1) );

        if (cancel != NULL) {
            checkmem( b_arr[1] =
                         w_addstdbutton(w, w_b_press, w_xwininsize(w) -
                                        bxsize - 5,
                                        w_ywininsize(w) -
