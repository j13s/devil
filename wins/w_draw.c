...
void w_clearwindow(struct w_window *w) {
    ...
    if (wi->in) {
        ws_clearbitmap(wi->in, notes.colindex[cv_winfill]);
    }

    if (wi->win_contents) {
        ws_drawfilledbox(w_xwinincoord(w, 0), w_ywinincoord(w,
                                                            0),
                         w_xwininsize(w),
                ...
                int xor)
{
    ...
    if ( ( (struct wi_window *)w )->win_contents ) {
        ws_drawline(w_xwinincoord(w,
                                  x1),
                    w_ywinincoord(w, y1), w_xwinincoord(w, x2),
                    w_ywinincoord(w, y2), col, xor);
    }
}


void w_drawclipline(struct w_window *w, int x1, int y1, int x2, int y2,
                    int col,
                    int xor)
{
    my_assert(w);
    ws_setclipping( w_xwinincoord(w, 0), w_ywinincoord(w, 0),
                   w_xwinincoord(w,
                                 w_xwininsize(w) - 1),
                   w_ywinincoord(w, w_ywininsize(w) - 1) );
                  ...
                  int withbg)
{
    ...
    if ( xp + xs > w_xwininsize(w) ) {
        xs = w_xwininsize(w) - xp;
    }

    if ( yp + ys > w_ywininsize(w) ) {
        ys = w_ywininsize(w) - yp;
    }

    ws_copybitmap(NULL, w_xwinincoord(w, xp), w_ywinincoord(w,
                                                            yp), bm, 0, 0, xs,
                  ys,
                  withbg);
}
...
static void wi_swapbg(struct wi_window *w, struct bounds *b) {
    ...
    ws_copybitmap(w->bg, b->xp - w->w.xpos, b->yp - w->w.ypos, bm, 0, 0,
                  b->xs, b->ys,
                  1);
    ws_freebitmap(bm);
}
...
void w_wintofront(struct w_window *w) {
    ...
    if (wi->drawn) {
        ...
        for (n = notes.windows.head; n->next != NULL && n != wi->n;
             n = n->next) {
            if ( n->d.w_w->drawn && wi_whereoverlap(n->d.w_w, wi, &b) ) {
                checkmem( bm = ws_savebitmap(NULL, b.xp, b.yp, b.xs, b.ys) );
                bm->w = n->d.w_w;
                checkmem( addnode(&l, -1, bm) );
                ws_copybitmap(NULL, b.xp, b.yp, n->d.w_w->bg, b.xp -
                              n->d.w_w->w.xpos,
                              b.yp - n->d.w_w->w.ypos, b.xs, b.ys,
                              1);
            }
        }
        ...
        for (n = l.tail->prev; n != NULL; n = n->prev) {
            bm = n->next->d.w_bm;
            ws_copybitmap(bm->w->bg, bm->xpos - bm->w->w.xpos, bm->ypos -
                          bm->w->w.ypos,
                          wi->bg, bm->xpos - w->xpos, bm->ypos - w->ypos,
                          bm->xsize, bm->ysize,
                          1);
            ws_copybitmap(wi->bg, bm->xpos - w->xpos, bm->ypos - w->ypos, bm,
                          0, 0, bm->xsize,
                          bm->ysize,
                          1);
            ws_freebitmap(bm);
...
void w_refreshstart(struct w_window *w) {
    ...
    for (n = notes.windows.head; n->next != NULL && n != wi->n; n =
             n->next) {
...
void w_changewintitle(struct w_window *w, char *s) {
    ...
    for (n = wi->buttonlist.head, b = NULL;
         n->next != NULL && n->d.w_b->sys_button;
         n = n->next) {
    ...
    if (b != NULL) {
        if (b->b.inscription.head->next) {
            ...
            checkmem( b->b.inscription.head->d.w_d->contents.txt =
                         MALLOC(strlen(t) + 1) );
        ...
        if (b->b.a_inscription.head->next) {
            ...
            checkmem( b->b.a_inscription.head->d.w_d->contents.txt =
                         MALLOC(strlen(t) + 1) );
