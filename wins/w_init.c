...
void drawmenubar(void) {
    ...
    if ( (notes.cur_win != NULL && notes.cur_win->w.title != NULL)
       || notes.titlebar_text != NULL ) {
        ...
        ws_drawtext(notes.xres - ws_pixstrlen(
                        buffer) - shapes.titlebar_height / 4, 2,
                    ws_pixstrlen(buffer), buffer, notes.colindex[cv_textbg],
                    notes.colindex[cv_buttonin]);
        ws_drawtext(notes.xres - ws_pixstrlen(
                        buffer) - shapes.titlebar_height / 4 + 1, 3,
                    ws_pixstrlen(
                        buffer), buffer, notes.colindex[cv_textfg], -1);
    }
}
...
void wi_drag_drag(struct w_button *b, int x, int y) {
    ...
    ws_drawbox(ox, oy, b->w->xsize, b->w->ysize, notes.colindex[cv_windrag],
               1);
...
int w_savecfg(FILE *f) {
    ...
    for (n = notes.windows.tail; n->prev != NULL; n = n->prev) {
        if (n->d.w_w->w.title) {
            if (fprintf(f, "%s\n%d %d %d %d %d %d\n", n->d.w_w->w.title,
                        n->d.w_w->w.xpos,
                        n->d.w_w->w.ypos, n->d.w_w->w.xsize,
