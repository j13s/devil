...
void wi_drawmenutxt(struct wi_menu *m, int hl) {
    ...
    ws_drawtext(m->x1 + 1 + shapes.titlebar_height / 4, m->y1 + 1, m->xs,
                m->txt,
                notes.colindex[hl ? cv_textfg : cv_textbg],
                notes.colindex[cv_buttonin]);
    ws_drawtext(m->x1 + shapes.titlebar_height / 4, m->y1, m->xs, m->txt,
                notes.colindex[hl ? cv_textbg : cv_textfg], -1);
    ws_drawline(
        m->x1 + m->hklinex + shapes.titlebar_height / 4, m->y1 + m->hkliney,
        m->x1 + m->hklinex + m->hklinexs - 1 +
        shapes.titlebar_height / 4,
        m->y1 + m->hkliney,
        notes.colindex[hl ? cv_textbg : cv_textfg], 0);
}
...
void wi_undrawmenu(struct wi_menu *m, struct list *l, int level) {
    ...
    for (n = l->tail->prev; n != NULL; n = n->prev) {
        if (n->next->d.w_m->level >= level && n->next->d.w_m->bm != NULL) {
            ws_restorebitmap(n->next->d.w_m->bm);
            n->next->d.w_m->bm = NULL;
        }

        if (n->next->d.w_m->level > level) {
            killnode(l, n->next);
        }
    }
...
struct wi_menu *wi_drawsubmenu(struct list *l, struct wi_menu *m) {
    ...
    for (n = m->submenus.head, size = 0; n->next != NULL; n = n->next) {
        size += n->d.w_m->ys;
    }
    ...
    if (size != 0) {
        ...
        checkmem( ( m->bm =
                       ws_savebitmap(NULL, m2->x1 - shapes.width_menuframe,
                                     m2->y1 - shapes.width_menuframe,
                                     m2->xs + 2 * shapes.width_menuframe,
                                     size + 2 * shapes.width_menuframe) ) );
        ws_drawframedbox(m2->x1 - shapes.width_menuframe,
                         m2->y1 - shapes.width_menuframe, m2->xs + 2 *
        ...
        for (n = m->submenus.head; n->next != NULL; n = n->next) {
            ...
            if (l) {
                checkmem( addnode(l, n->no, n->d.w_m) );
            }
        }
    }
    else {
    ...
    do {
        for (n = l->head, m = NULL; n->next != NULL; n = n->next) {
            if (n->d.w_m->y1 <= ws->y && n->d.w_m->y1 + n->d.w_m->ys > ws->y
               && n->d.w_m->x1 <= ws->x && n->d.w_m->x1 + n->d.w_m->xs >
                ws->x) {
                m = n->d.w_m;
                break;
            }
        ...
        if (ws.flags != ws_f_keypress) {
            ...
            switch (ws.key) {
                ...
                default:
                    ...
                    for (n = sm->submenus.head; n->next != NULL; n =
                             n->next) {
...
int wi_handlemenu(struct ws_event *ws) {
    ...
    if (ws->flags != ws_f_keypress && ws->buttons == ws_bt_none) {
        return 0;
    }
    else {
        ws->key = toupper(ws->key);
    }
    ...
    if (m != NULL && ws->flags == ws_f_keypress) {
        if ( ( m = wi_keydrawmenu(m) ) != NULL && m->action !=
            NULL )                                                 {
            m->action(m->actionnr);
        }
    }
    ...
    for (n = l.head->next; n != NULL; n = n->next) {
        if (n->prev->d.w_m->bm) {
            ws_freebitmap(n->prev->d.w_m->bm);
            n->prev->d.w_m->bm = NULL;
        }
