...
struct ws_internals {
    ...
    { NULL, {0}, {0}, GR_BYTE_TEXT, GR_TEXT_RIGHT, GR_ALIGN_LEFT,
      GR_ALIGN_TOP },
    { 0 }, 0
};
                   ...
                   int withbg)
{
    GrBitBlt(dst == NULL ? NULL : dst->bitmap, x1, y1,
             src == NULL ? NULL : src->bitmap, xpos, ypos, xpos + xsize - 1,
             ypos + ysize - 1,
...
int altkeys_09[10] = {
    385, 376, 377, 378, 379, 380, 381, 382, 383, 384
};
...
int ws_getevent(struct ws_event *se, int wait) {
    int kbstat_ttable[6] = {
        GR_KB_SHIFT, GR_KB_ALT, GR_KB_CTRL, GR_KB_NUMLOCK,
    ...
    if (wait) {
        ...
        while (GrMousePendingEvent() && me.flags == GR_M_KEYPRESS) {
            GrMouseGetEvent(
                GR_M_KEYPRESS | GR_M_BUTTON_CHANGE | GR_M_POLL | GR_M_NOPAINT,
                &me);
        }

        if (wait > 0) {
            GrMouseGetEvent(GR_M_BUTTON_CHANGE | GR_M_KEYPRESS | GR_M_NOPAINT,
                            &me);
        }
        else {
            GrMouseGetEventT(
                GR_M_BUTTON_CHANGE | GR_M_KEYPRESS | GR_M_NOPAINT, &me,
                -wait * 10);
        }
    }
    else {
        GrMouseGetEvent(
            GR_M_BUTTON_CHANGE | GR_M_KEYPRESS | GR_M_POLL | GR_M_NOPAINT,
            &me);
        me2 = me;
    ...
    se->flags = (me.flags & GR_M_KEYPRESS) ? ws_f_keypress :
                (me.flags & GR_M_RIGHT_UP || me.flags &
                 GR_M_RIGHT_DOWN) ? ws_f_rbutton :
                (me.flags & GR_M_LEFT_UP || me.flags &
                 GR_M_LEFT_DOWN) ? ws_f_lbutton :
                ws_f_none;
    se->buttons = ws_bt_none;

    if (me.buttons & GR_M_LEFT) {
        se->buttons |= ws_bt_left;
    }

    if (me.buttons & GR_M_RIGHT) {
    ...
    if (se->kbstat & ws_ks_alt) {
        for (i = 0; i < 26; i++) {
            if (se->key == altkeys_az[i]) {
                se->key = i + ( (se->kbstat & ws_ks_shift) != 0 ? 'A' : 'a' );
                break;
            }
        }
