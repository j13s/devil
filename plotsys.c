...
#define SYS_LIGHTSCANLINE(NUM_PIXELS, ADD_F1, ADD_F2, ADD_F3) {\
        ...
        run_f2 += ADD_F2; run_f3 += ADD_F3; f2 = f1 * run_f2; f1 = f1 *\
                                                                   run_f3; \
                    ...
                    *(cur_pos++) = *( colors + \
                                     *( txt_data +\
                                       ( (txt_u >>\
                                        ...
                                        0x3f ) + ( (txt_v >> 8) & (0x3f * TXTSIZE) ) ) ); \
                    txt_u += add_txt_u; txt_v += add_txt_v; light += d_light; \
                    if (light > c_light) { c_light += d_x; colors +=\
                                               add_colors; } \
                } \
            } \
        } \
...
#define SYS_PLOTWITHOUTCOLOR { \
        ...
        { SYS_SCANLINE(d_x, er_n_3d.x[0] * d_x, er_rXd.x[0] * d_x,\
                       er_dXs.x[0] * d_x) } \
}
                      ...
                      unsigned char *txt_data)
{
    ...
    float run_line1, run_line2, run_line3, run_f1, run_f2, run_f3, add_f1,
          add_f2,
          add_f3, f1, f2;
    ...
    do {
        ...
        for (; ps_y < round( (next_lr > 0 ? ppl : ppr)->x[1] ); ps_y++,
             cur_line -= xsize, run_line1 += er_n_3d.x[1], run_line2 +=
                 er_rXd.x[1],
             run_line3 += er_dXs.x[1], pl_x0 += add_pl_x0, pr_x0 += add_pr_x0,
             l_light += add_ll,
             r_light += add_rl) {
            ps_x = round(pl_x0);
            e_ps_x = round(pr_x0);
            ...
            if (e_ps_x > ps_x) {
                ...
                f1 = f1 * run_f3;
                n_txt_u = p->a_txt.x[0] +
                          (f1 * p->r_txt.x[0]) + (f2 * p->s_txt.x[0]);
                n_txt_v = p->a_txt.x[1] +
                          (f1 * p->r_txt.x[1]) + (f2 * p->s_txt.x[1]);
                ...
                if ( ( pixel = \
                          *( txt_data +\
                            ( (txt_u >> \
                               8) &\
                             0x3f ) +\
                            ( (txt_v >> 8) & (0x3f * TXTSIZE) ) ) ) != \
                          TRANSPARENT_COLOR ) { \
                          *(cur_pos++) = *(colors + pixel);} \
                      else { cur_pos++;} \
                      txt_u += add_txt_u; txt_v += add_txt_v; light +=\
                          d_light; \
                      if (light > c_light) { c_light += d_x; colors +=\
                                                 add_colors; } \
                      } \
                      ...
                      { e_rest = d_x; d_x <<= 16; SYS_TLIGHTSCANLINE(\
                            e_rest, \
                            er_n_3d.\
                            x[0] * e_rest, er_rXd.x[0] * e_rest,\
                            er_dXs.x[0] * e_rest) } \
                      }
        ...
        {\
            ...
            { \
                if ( ( pixel = \
                      ...
                      { SYS_TSCANLINE(d_x, er_n_3d.x[0] * d_x, er_rXd.x[0] *\
                                      d_x, \
                                      er_dXs.x[0] * d_x) } \
                      }
...
void psys_copytoscreen(int x1, int y1, int xpos, int ypos, int xsize,
                       int ysize) {
...
void psys_plotline(int o_x1, int o_y1, int o_x2, int o_y2, int color,
                   int xor)                                                 {
