...
static void wi_changeobject(struct w_button *b, int dowhat) {
    ...
    switch (what) {
        ...
        case in_wall:
            ...
            if (view.pcurrcube != NULL) {
                switch (dowhat) {
                    ...
                    default:
                        view.currwall = dowhat;
                }

                if (view.currwall > 5) {
                    view.currwall = 0;
                }
                else if (view.currwall < 0) {
                    view.currwall = 5;
                }
        ...
        case in_edge:
            ...
            if (view.pcurrcube != NULL) {
                switch (dowhat) {
                    ...
                    default:
                        view.curredge = dowhat;
                }

                if (view.curredge > 3) {
                    view.curredge = 0;
                }
                else if (view.curredge < 0) {
                    view.curredge = 3;
                }
                ...
                what = in_pnt;
                dowhat =
                    view.pcurrcube->d.c->p[wallpts[view.currwall][view.
                                                                  curredge]]
                    ->no;
            }
            else {
        ...
        default:
            waitmsg("Unknown infotype in changeobject %d", what);
            return;
    }
    ...
    if (n != NULL && *n != NULL && ls != NULL) {
        switch (dowhat) {
            ...
            case - 3:
                *n = (*n)->prev->prev != NULL ? (*n)->prev : ls->tail;
                break;

            case - 2:
                *n = (*n)->next->next != NULL ? (*n)->next : ls->head;
                break;
        ...
        if ( (*n)->no > 9999 ) {
            ...
            sprintf(b->d.str->str, "%.4u", (*n)->no);
        }
    }
    else if (n != NULL && ls != NULL) {
        sprintf(b->d.str->str, "----");
    }
...
void b_tagoneobject(struct w_button *b) {
    tagobject( (enum infos)b->data );
}


void b_tagallobjects(struct w_button *b) {
    tagallobjects( (enum infos)b->data );
}
...
void b_l_insert(struct w_button *b) {
    ...
    oldmode = view.currmode;
    view.currmode = (enum infos)b->data;
...
void b_r_defaultobject(struct w_button *b) {
    drawopt( (enum infos)b->data );
}                                  /* to let the button pop out */


void b_l_lockwall(struct w_button *b) {
    if (!view.pcurrcube || !view.pcurrcube->d.c->walls[view.currwall]) {
        printmsg(TXT_EMPTYWALLNOTLOCK);
        drawopt(in_wall);
        return;
    }
...
void b_r_lockwall(struct w_button *b) {
    ...
    if (view.pcurrcube->d.c->walls[view.currwall]->locked) {
        ...
        printmsg(TXT_UNLOCKEDTAGGEDSIDES, i);
    }
...
void b_selectmacro(struct w_button *b) {
    ...
    if (b->d.ls->selected > 0 && b->d.ls->selected < b->d.ls->num_options) {
        for (n = view.levels.head; n->next != NULL; n = n->next) {
            if ( !strcmp(n->d.lev->fullname,
                         b->d.ls->options[b->d.ls->selected]) ) {
...
void b_adjustlight(struct w_button *b, int save) {
    if (b->d.s->on) {
        if (adjust_light_nc != NULL) {
            waitmsg(TXT_DOUBLEADJLIGHT, adjust_light_nc->no,
                    adjust_light_wall);
            b->d.s->on = 0;
            w_drawbutton(b);
            return;
        ...
        if (view.pcurrwall == NULL || view.pcurrwall->ls == NULL) {
            return;
        }
        ...
        if ( !save && !yesnomsg(TXT_CANCELADJLIGHT) ) {
            return;
        }
...
void b_l_adjustlight(struct w_button *b) {
    b_adjustlight(b, 1);
}


void b_r_adjustlight(struct w_button *b) {
    b_adjustlight(b, 0);
}
...
struct w_b_press b_prev = {
    25, 5, b_prevobj, NULL, b_prevobj, b_firstobj
};
struct w_b_press b_next = {
    25, 5, b_nextobj, NULL, b_nextobj, b_lastobj
};
int init_prev_no_next(enum infos what) {
    ...
    b_tagcount->str[0] = 0;
    b_tagcount->allowed_char = isdigit;
    ...
    b_ins->delay = b_del->delay = 0;
    b_ins->repeat = b_del->repeat = -1;
    b_ins->l_pressed_routine = b_ins->r_pressed_routine =
                                   b_del->l_pressed_routine =
                                       b_del->
                                       r_pressed_routine = NULL;
    b_ins->l_routine = b_l_insert;
    b_ins->r_routine = b_r_insert;
    b_del->l_routine = b_l_delete;
    b_del->r_routine = b_r_delete;
    checkmem( b_optwins[what][1] =
                 w_addstdbutton(optionwins[what], w_b_string,
                                w_xwininsize(optionwins[
                                                 what]) / 8, 0,
                                w_xwininsize(optionwins[what]) -
                                (w_xwininsize(optionwins[
                                                  what]) / 4) * 3, -1, NULL,
                                b_no, 0) );
    ...
    checkmem( b_optwins[what][2] = w_addstdbutton(optionwins[what], w_b_press,
                                                  ...
                                                  ">",
                                                  &b_next, 0) );
    checkmem( b_optwins[what][3] =
                 w_addstdbutton(optionwins[what], w_b_switch,
                                b_optwins[what][2]->xpos +
                                b_optwins[what][2]->xsize, 0,
                                w_xwininsize(optionwins[
                                                 what]) * 7 / 32,
                                b_optwins[what][1]->ysize, TXT_TAG,
                                b_tag, 0) );
    checkmem( b_optwins[what][7] =
                 w_addstdbutton(optionwins[what], w_b_string,
                                b_optwins[what][3]->xpos +
                                b_optwins[what][3]->xsize, 0,
                                w_xwininsize(optionwins[
                                                 what]) -
                                b_optwins[what][3]->xpos -
                                b_optwins[what][3]->xsize,
                                b_optwins[what][1]->ysize, NULL, b_tagcount,
                                0) );
    w_dontchangebutton(b_optwins[what][7]);

    if (what != in_pnt) {
        checkmem( b_optwins[what][4] =
                     w_addstdbutton(optionwins[what], w_b_press,
                                    0,
                                    b_optwins[what][2]->ypos +
                                    b_optwins[what][2]->ysize,
                                    w_xwininsize(optionwins
                                                 [what]) /
                                    (what == in_edge ? 1 : 2), -1, TXT_INSERT,
                                    b_ins, 0) );

        if (what != in_edge) {
            checkmem( b_optwins[what][5] =
                         w_addstdbutton(optionwins[what], w_b_press,
                                        w_xwininsize(
                                            optionwins[what]) / 2,
                                        b_optwins[what][2]->ypos +
            ...
            b_optwins[what][5] = b_optwins[what][4];
        }                                       /* see below */
    ...
    b_optwins[what][1]->data = b_optwins[what][3]->data =
                                   b_optwins[what][4]->data =
                                       b_optwins[what][5]->data =
                                           (void *)what;
    ...
    switch (what) {
        case in_cube:
            ...
            view.b_macros->select_lroutine = view.b_macros->select_rroutine =
                                                 b_selectmacro;
            checkmem( b_optwins[what][6] =
                         w_addstdbutton(optionwins[what], w_b_choose,
                                        0,
                                        b_optwins[what][5]->ypos +
                                        b_optwins[what][5]->ysize,
                                        w_xwininsize(
                                            optionwins[what]), -1, TXT_MACRO,
                                        view.b_macros, 0) );
        ...
        case in_wall:
            ...
            checkmem( b_optwins[what][6] =
                         w_addstdbutton(optionwins[what], w_b_switch,
                                        ...
                                        w_xwininsize(
                                            optionwins[what]) / 2, -1,
                                        TXT_DEFAULT, b_default, 0) );
            checkmem( b_optwins[what][8] =
                         w_addstdbutton(optionwins[what], w_b_switch,
                                        w_xwininsize(
                                            optionwins[what]) / 2,
                                        b_optwins[what][5]->ypos +
                                        ...
                                        TXT_LOCKED,
                                        b_locked, 0) );
            b_optwins[what][6]->data = b_optwins[what][8]->data =
                                           (void *)what;

            if (init.d_ver >= d2_10_sw) {
                checkmem( b_optwins[what][9] =
                             w_addstdbutton(optionwins[what], w_b_switch,
                                            0,
                                            b_optwins[what][6]->ypos +
                                            b_optwins[what][6]->ysize,
                                            w_xwininsize(
                                                optionwins[what]), -1,
                                            TXT_ADJUSTLIGHT, b_adjustlight, 0) );
