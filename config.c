...
int savelvlconfig(FILE *f, struct leveldata *ld, int no) {
    ...
    if (fprintf( f, "%d %d %d %d %d %d * current objects\n",
                CUROBJNO(ld->pcurrcube), CUROBJNO(ld->pcurrpnt), ld->currwall,
                ld->curredge,
                CUROBJNO(ld->pcurrthing), CUROBJNO(ld->pcurrdoor) ) < 0) {
        return 0;
    }
...
int savestatus(int playlevel) {
    ...
    in_changecurrentlevel(ld);                           /* so the
                                                            data in the level structure
                                                            is correct */
    ...
    if (fprintf(f, "%g %g %g %g %g %g %d %g %g\n", view.dist,
                view.maxvisibility,
                view.movefactor, view.pmovefactor, view.rotangle,
                view.protangle,
    ...
    if (fprintf(f, "%g %g %g %g %g %g %g %d %d %d %d %d\n",
                ...
                fb_savedata.xsize, fb_savedata.ysize) < 0) {
        ERRORSAVECFG(f);
    }
    ...
    for (i = 0; i < tlw_num; i++) {
        if (fprintf(f, "%d %d %d %d %d\n", tl_win[i].zoom.selected,
                    ...
                    tl_win[i].oldxsize < 0 ? 0
                    : tl_win[i].oldxsize, tl_win[i].oldysize) < 0) {
            ERRORSAVECFG(f);
        }
    }
    ...
    if (fprintf(f,
                "%d %d %d * Number of levels, current level, current macro\n",
                i,
                j, k) < 0) {
        ERRORSAVECFG(f);
    }
    ...
    for (n = view.levels.head, i = 0, j = -1; n->next != NULL; n = n->next) {
        if (playlevel) {
            ...
            if ( !savelevel(fname, n->d.lev, playlevel > 0 ? n->d.lev ==
                            l : -1, 0, init.d_ver, 0) ) {
                ERRORSAVECFG(f);
            }
...
int readlvlconfig(FILE *f, struct leveldata *ld) {
    ...
    if (fscanf(f, "%d%d%d%d%d%d", &n0, &n1, &ld->currwall, &ld->curredge, &n2,
               &n3) != 6) {
        return 0;
    }
    ...
    for (n1 = 0; n1 < 3; n1++) {
        if (fscanf(f, "%g%g%g", &ld->e[n1].x[0], &ld->e[n1].x[1],
                   &ld->e[n1].x[2]) != 3) {
            return 0;
        }
...
void readstatus(char* lfname) {
    ...
    if (fscanf(f,
               ...
               &view.displayed_group, &view.gridonoff, &view.askone,
               &view.asktagged,
               &view.drawwhat, &d, &e, &lno, &c, &view.defwall,
               &view.warn_convex,
               &view.warn_doublekeys, &view.warn_tworeactors,
               &view.warn_thingoutofbounds,
    ...
    if (c > 0 && lno > 0) {
        if ( ( view.pdeflevel = findnode(&view.levels, lno) ) != NULL ) {
            if ( ( view.pdefcube =
                      findnode(&view.pdeflevel->d.lev->cubes,
                               c) ) == NULL ) {
                view.pdeflevel = NULL;
            }
        }
    ...
    if (d >= 0) {
        for (e = 0, n = view.levels.head; e < d && n->next; n = n->next,
             e++) {
            ;
        }
    ...
    for (i2 = *is; i2 - *is < num; i2++) {
        ...
        if ( (i2->type == it_texture || i2->type == it_dooranim ||
           ...
           && i2->multifuncnr == 0 ) {
            ...
            if ( ( *is =
                      REALLOC( *is,
                              num * sizeof(struct infoitem) ) ) == NULL ) {
...
void initobjtypelists(struct infoitem *is, int num) {
    ...
    for (i = is; i - is < num; i++) {
        ...
        if (lastot != NULL) {
            ...
            if ( ( i->od->data =
                      REALLOC( i->od->data, (oldnum + curi->od->size) *
...
int readconfig(void) {
    ...
    for (i = 0; i < view.num_keycodes; i++) {
        ...
        if (view.ec_keycodes[i].event < 0 || view.ec_keycodes[i].event >=
            ...
            printf("Illegal keycode %#x %d %d\n", view.ec_keycodes[i].kbstat,
                   view.ec_keycodes[i].key, view.ec_keycodes[i].event);
            return 0;
        }
    }
    ...
    else if (init.pigpaths[d2_10_reg] && strlen(init.pigpaths[d2_10_reg]) >
             0) {
...
int saveplaymsn(int savetoddir) {
    ...
    if (fwrite(init.d_ver >= d2_10_sw ? "TMPDEVIL.RL2" : "TMPDEVIL.RDL", 1,
               13, f) != 13) {
        printmsg(TXT_CANTSAVEPLAYHOG, fname);
        fclose(f);
        return 0;
