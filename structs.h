...
#define VECTOR(c, a, b) { \
        (c)->x[0] = (float)(a)->x[1] * (b)->x[2] - (float)(a)->x[2] *\
                    (b)->x[1]; \
        (c)->x[1] = (float)(a)->x[2] * (b)->x[0] - (float)(a)->x[0] *\
                    (b)->x[2]; \
        (c)->x[2] = (float)(a)->x[0] * (b)->x[1] - (float)(a)->x[1] *\
                    (b)->x[0]; }
#define SCALAR(a, b) ( (float)(a)->x[0] * (b)->x[0] + \
                      (float)(a)->x[1] * (b)->x[1] + (float)(a)->x[2] *\
                      (b)->x[2] )
...
enum tagtypes {
    tt_cube, tt_wall, tt_edge, tt_pnt, tt_thing, tt_door, tt_number
};
enum movetypes {
...
struct corridor {
    ...
    /* the window stuff: */
    struct w_window *win;
    struct w_button *b_ok, *b_cancel, *b_ins, *b_del, *b_setend, *b_depth,
    *b_stdform;
};
struct listpoint {
    struct point p;
...
struct door {
    ...
    unsigned char switchtype NONANSI_FLAG; /* needed that you can change the
                                           switchtype from 'No switch' to anything
                                              else(the button must be activated)*/
...
enum txttypes {
    txt1_normal, txt2_normal, txt1_wall, txt2_wall, txt_door,
...
struct ham_txt {
    unsigned long flags NONANSI_FLAG, light NONANSI_FLAG,
    hitpoints NONANSI_FLAG;
...
struct pigdata {
    ...
    char *anim_txt_names; /* string with names of textures which are animated
                          like doors and therefore have several rdl-numbers for
                             an anim. */
