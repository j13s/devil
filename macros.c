...
void copy_lightsource(struct leveldata *insertinto, struct node **cubes,
                      int cubessize, struct lightsource *sls,
                      int checktagged)
{
    ...
    for (sn = sls->effects.head; sn->next != NULL; sn = sn->next) {
        if (!checktagged || sn->d.lse->cube->d.c->tagged) {
            ...
            *lse = *sn->d.lse;
            my_assert(
                sn->d.lse->cube->no >= 0 && sn->d.lse->cube->no < cubessize
...
/*   checkmem(addnode(&lp->c,j,nn)); this is wrong here because it is done
   in initlevel */
    ...
    for (tn = tl->head; tn->next != NULL; tn = tn->next) {
        j =
            getsize(ds_thing,
                    tn->d.n->d.t) + (unsigned char *)&tn->d.n->d.t->type1 -
        (unsigned char *)tn->d.n->d.t;
        checkmem( t = MALLOC(j) );
        memcpy(t, tn->d.n->d.t, j);
        t->tagged = NULL;
        t->cube = t->nc ? t->nc->no : -1;
        t->nc = NULL;
            ...
            if (n->d.c->nc[j] != NULL) {
                if ( ( n->d.c->nc[j] =
                    ...
                    if (n->d.c->d[j] != NULL) {
                        ...
                        freenode(&m->doors, n->d.c->d[j], free);
                        n->d.c->d[j] = NULL;
    ...
    for (n = m->sdoors.head; n->next != NULL; n = n->next) {
        for (j = 0, i = 0; j < n->d.sd->num; j++) {
            if ( ( n->d.sd->target[i] =
                      findnode(getsdoortype(n->d.sd) == sdtype_door ?
                               &m->doors : &m->cubes,
                               n->d.sd->target[j]->no) ) != NULL ) {
                i++;
            }
        }
    ...
    sortlist(&m->pts, 0);
    sortlist(&m->sdoors, 0);
    sortlist(&m->producers, 0);
    sortlist(&m->lightsources, 0);

    /* making the numbers */
    for (n = m->cubes.head; n->next != NULL; n = n->next) {
        for (j = 0; j < 8; j++) {
            n->d.c->pts[j] = n->d.c->p[j]->no;
        }

        for (j = 0; j < 6; j++) {
            n->d.c->doors[j] =
                (n->d.c->d[j] == NULL) ? 0xff : n->d.c->d[j]->no;
            ...
            n->d.c->nextcubes[j] =
                (n->d.c->nc[j] == NULL) ? 0xffff : n->d.c->nc[j]->no;
        }

        if (n->d.c->cp) {
            n->d.c->cp->d.cp->cubenum = n->no;
            n->d.c->prodnum = n->d.c->cp->no;
        }
        else {
            n->d.c->prodnum = -1;
        }
...
int insertmacro(struct leveldata *m, int connectnow, float scaling) {
    ...
    if (m->exitcube == NULL || m->exitcube->d.c->nc[m->exitwall] != NULL) {
        printmsg(TXT_NOCONNECTSIDE, m->fullname);
        return 0;
    }
    ...
    if ( ( (l->cubes.size <= MAX_DESCENT_CUBES
         ...
         || (l->doors.size <= MAX_DESCENT_WALLS
            && l->doors.size + view.pcurrmacro->doors.size >
             MAX_DESCENT_WALLS)
         || (l->things.size <= MAX_DESCENT_OBJECTS
            && l->things.size + view.pcurrmacro->things.size >
             MAX_DESCENT_OBJECTS) )
    ...
    sortlist(&m->sdoors, 0);
    sortlist(&m->lightsources, 0);
    sortlist(&l->cubes, m->cubes.size);
    sortlist(&l->pts, m->pts.size);
    sortlist(&l->things, m->things.size);
    sortlist(&l->doors, m->doors.size);
    ...
    for (n = m->pts.head; n->next != NULL; n = n->next) {
        ...
        /* rotating points */
        for (i = 0; i < 3; i++) {
            hp1.x[i] = n->d.lp->p.x[i] - eoffset->x[i];
        }
        ...
        /* and move&scale them */
        for (i = 0; i < 3; i++) {
            lp->p.x[i] = offset->x[i] + lp->p.x[i] * scaling;
        }
    ...
    for (n = m->things.head; n->next != NULL; n = n->next) {
        ...
        for (i = 0; i < 3; i++) {
            hp1.x[i] = n->d.t->p[0].x[i] - eoffset->x[i];
        }
        ...
        for (i = 0; i < 3; i++) {
            t->p[0].x[i] = offset->x[i] + t->p[0].x[i] * scaling;
        }

        /* Now turn the orientation of the thing */
        for (j = 0; j < 3; j++) {
            for (i = 0; i < 3; i++) {
                hp1.x[i] = n->d.t->orientation[j * 3 + i];
            }
            ...
            for (i = 0; i < 3; i++) {
                n->d.t->orientation[j * 3 + i] = hp1.x[i];
            }
        }
    ...
    for (n = l->cubes.head; n->no < m->cubes.size; n = n->next) {
        if ( !initcube(n) ) {
            n = n->next;
            freenode(&l->cubes, n->prev, freecube);
        }
    }
    ...
    for (n = l->doors.head; n->no < m->doors.size; n = n->next) {
        ...
        if (n->d.d->sd) {
            ...
            for (i = 0; i < n->d.d->sd->d.sd->num; i++) {
                if (getsdoortype(n->d.d->sd->d.sd) == sdtype_door) {
                    n->d.d->sd->d.sd->cubes[i] =
                        n->d.d->sd->d.sd->target[i]->d.d->c->no;
                    n->d.d->sd->d.sd->walls[i] =
                        n->d.d->sd->d.sd->target[i]->d.d->wallnum;
                }
                else {
                    n->d.d->sd->d.sd->cubes[i] =
                        n->d.d->sd->d.sd->target[i]->no;
                }
            }
        }
        else {
        ...
        if ( !initdoor(n) ) {
            n = n->next;
            freenode(&l->doors, n->prev, freedoor);
        }
    }
