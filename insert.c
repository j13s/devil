...
struct node *insertthing(struct node *c, struct node *t) {
    ...
    if (nt->type1 != 4 && nt->type2 != 14 && t != NULL) {
        nt->type2 = t->d.t->type2;
    }
...
struct wall *insertwall(struct node *c, int wallnum, int txt1, int txt2,
                        int dir) {
    struct wall *w;
    int j;
...
struct node *insertsingledoor(struct node *c, struct node *pd, int wallnum) {
    ...
    if (c->d.c->nc[wallnum] != NULL) {
        ...
        if (pig.anims[stdncdoor.animtxt]->pig->anim_t2) {
            checkmem( insertwall(c, wallnum, 0,
                                 pig.anims[stdncdoor.animtxt]->rdlno, 0) );
        }
        else {
            checkmem( insertwall(c, wallnum,
                                 pig.anims[stdncdoor.animtxt]->rdlno, 0, 0) );
        }
...
int insertsdoor(struct node *d) {
    ...
    *sd = stdsdoor;
    ...
    if (d->d.d->type1 == door1_switchwithwall) {
        sd->flags |= switchflag_once;
    }
...
void justdelconnect(struct node *c, int wn) {
    ...
    for (nwn = 0; nwn < 6; nwn++) {
        if (nc->d.c->nc[nwn] != NULL && nc->d.c->nc[nwn]->no == c->no) {
            break;
        }
    }
...
void insertpnt(struct node *c, int pn) {
    ...
    for (j = 0; j < 3; j++) {
        if (c->d.c->nc[wallno[pn][0][j]] != NULL) {
            justdelconnect(c, wallno[pn][0][j]);
        }
    }
...
void deleteconnect(struct node *c, int wn, int fast) {
    ...
    if (fast) {
        for (i = 0; i < 4; i++) {
            for (j = 0; j < 3; j++) {
                if (c->d.c->nc[wallno[wallpts[wn][i]][0][j]] != NULL) {
                    break;
                }
            }

            if (j == 3) {
                floatpnt(c, wallpts[wn][i]);
            }
        }
    }
}
...
void killtarget(struct sdoor *sd, int n) {
    ...
    for (j = i; j < sd->num - 1; j++) {
        sd->target[j] = sd->target[j + 1];
    }
...
void deletesingledoor(struct node *n) {
    ...
    if (n->d.d->c->d.c->nc[n->d.d->wallnum]) {
        for (i = 0; i < 4; i++) {
            untag(tt_edge, n->d.d->c, n->d.d->wallnum, i);
        }

        freewall(l, n->d.d->c->d.c, n->d.d->wallnum);

        if (view.pcurrwall == n->d.d->w) {
            view.pcurrwall = NULL;
        }
...
void delete_ref_ls(struct node* c) {
    ...
    for (p = l->lightsources.head; p->next != NULL; p = p->next) {
        if (p->d.ls) {
            ...
            if (flag) {
                for (nlse = ls->effects.head->next; nlse != NULL;
                     nlse = nlse->next) {
                    for (w = 0; w < 6; w++) {
                        if (nlse->prev->d.lse->cube->d.c->walls[w]) {
                            for (co = 0; co < 4; co++) {
                                overflow =
                                    nlse->prev->d.lse->cube->d.c->walls[w]->
                                    corners[co].light -
                                    nlse->prev->d.lse->add_light[w *
                                                                 4 + co];
                                nlse->prev->d.lse->cube->d.c->walls[w]->
                                corners[co].light =
                                    overflow <
                                    view.illum_minvalue ? view.illum_minvalue
                                    :
                                    overflow;
                            }
                        }
...
void deletecube(struct list *pcubes, struct list *ppts, struct node *n) {
    ...
    if (cubes == NULL || pts == NULL) {
        if (l->exitcube && l->exitcube->no == n->no) {
            l->exitcube = NULL;
        }
        ...
        if (n->no == view.pcurrcube->no) {
            ...
            if (k == 6) {
                view.pcurrcube =
                    view.pcurrcube->prev->prev ? view.pcurrcube->prev :
                    view.pcurrcube->next;
            }
            else {
        ...
        do {
            for (k = 0; k < 8; k++)          {
                if (view.pcurrcube->d.c->p[k] == view.pcurrpnt) {
                    break;
                }
            }
            ...
            if (k < 8) {
                ...
                if (view.pcurrpnt->next == NULL) {
                    view.pcurrpnt = l->pts.head;
                }
            }
        } while (k < 8);
    }
    ...
    for (k = 0; k < 8; k++) {
        ...
        for (cn = lp->c.head->next; cn != NULL; cn = cn->next) {
            if (cn->prev->d.n->no == n->no) {
                freenode(&lp->c, cn->prev, NULL);
            }
        }

        if (lp->c.size <= 0) {
            deletelistpnt(pts, c->p[k]);
        }
    }

    if (c->cp != NULL) {
        freenode(&l->producers, c->cp, free);
    }
...
int changepnt(struct node *oldp, struct node *newp, struct node *sn) {
    ...
    for (cn = (sn == NULL) ? oldp->d.lp->c.head : sn->next; cn->next != NULL;
         cn = cn->next) {
        n = cn->d.n;
        pn = cn->no;
        n->d.c->p[pn] = newp;

        if (addnode(&newp->d.lp->c, cn->no, n) == NULL) {
            return 0;
        }
    ...
    for (cn = ( (sn == NULL) ? oldp->d.lp->c.head : sn->next )->next;
         cn != NULL;
         cn = cn->next) {
                 ...
                 int w2)
{
    ...
    my_assert(
        nc1 != NULL && nc2 != NULL && w1 >= 0 && w1 < 6 && w2 >= 0 && w2 < 6);
    c1 = nc1->d.c;
    c2 = nc2->d.c;
    pw2 = c2->walls[w2];
    ...
    for (k = 0, i = -1; k < 4 && i == -1; k++) {
        ...
        if (changepnt(pnts[k][0], pnts[k][1], NULL) == 0) {
            i = k;
        }
    }
    ...
    for (j = 0; j < 4; j++) {
        if (pnts[j][0]->no != pnts[j][1]->no) {
            if (view.pcurrpnt == pnts[j][0]) {
                view.pcurrpnt = pnts[j][1];
            }
    ...
    if (pts == NULL) {
        if ( l->exitcube != NULL &&
            ( (l->exitcube->no == nc1->no && l->exitwall == w1)
            || (l->exitcube->no == nc2->no &&
                l->exitwall == w2) ) ) {
            l->exitcube = NULL;
        }
    }
...
int initdoor(struct node *n) {
    ...
    d->tagged = NULL;
    d->old_txt1 = d->animtxt >= pig.num_anims || pig.anims[d->animtxt] ==
                  NULL
                  || pig.anims[d->animtxt]->pig == NULL ||
                  !pig.anims[d->animtxt]->pig->anim_t2 ?
                  -1 : d->w->texture1;

    for (i = 0; i < 6; i++) {
        if (d->c->d.c->nc[d->wallnum] &&
            d->c->d.c->nc[d->wallnum]->d.c->nc[i] == d->c) {
            ...
            for (i = 0; i < sd->num; i++) {
                ...
                switch ( getsdoortype(sd) ) {
                    case sdtype_door:
                        ...
                        if (sd->walls[i] > 5) {
                            waitmsg(TXT_WRONGWALLFORSWITCH, sd->walls[i],
                                    d->sd->no);
                            sd->num--;
                        ...
                        if ( (sd->target[i] = sdn->d.c->d[sd->walls[i]]) ==
                            NULL ) {
                            waitmsg(TXT_NODOORFORSWITCH, sd->cubes[i],
                                    sd->walls[i],
                                    d->sd->no);
                            sd->num--;
...
int initcube(struct node *n) {
    ...
    for (j = 0; j < 6; j++) {
        ...
        if (c->nextcubes[j] >= 0x7fff) {
            ...
            /*for exit:*/ if (c->walls[j] == NULL) {
                insertwall(n, j, -1, -1, -1);
            }
        }
        else
        if ( ( c->nc[j] =
                  findnode(&l->cubes, (int)c->nextcubes[j]) ) == NULL ) {
            if ( yesnomsg(TXT_NONCFORCUBE, (int)c->nextcubes[j],
                          n->no) )
            {
                return -1;
            }
        ...
        if ( ( c->d[j] = findnode(&l->doors, (int)c->doors[j]) ) == NULL ) {
            if ( yesnomsg(TXT_NODOORFORCUBE, (int)c->doors[j],
                          n->no) )
            {
                return -1;
            }
    ...
    if (c->type == 4 && c->prodnum >= 0) { /* producer */
        if ( ( c->cp = findnode(&l->producers, c->prodnum) ) == NULL ) {
            if ( yesnomsg(TXT_NOPRODFORCUBE, (int)c->prodnum,
                          n->no) )
            {
                return -1;
            }
...
int testcube(struct node *nc, int withmsg) {
    ...
    for (l1n = 0; l1n < 6; l1n++) {
        for (l2n = 0; l2n < 4; l2n++) {
            ...
            if (ll1 <= 640.0 || ll1 > 32700 * 640.0) {
                if (withmsg)                                   {
                    printmsg(TXT_TPLINETOOLONG, l2n, (l2n + 1) & 3, l1n,
                             nc->no);
                }
    ...
    for (newpnum = 0; newpnum < 8; newpnum++) {
        for (j = 0; j < 3; j++) {
            ...
            l1.x[j] = nc->d.c->p[l1n]->d.p->x[j] -
                      nc->d.c->p[newpnum]->d.p->x[j];
            l2.x[j] = nc->d.c->p[l2n]->d.p->x[j] -
                      nc->d.c->p[newpnum]->d.p->x[j];
            l3.x[j] = nc->d.c->p[l3n]->d.p->x[j] -
                      nc->d.c->p[newpnum]->d.p->x[j];
        }
        ...
        if (SCALAR(&e, &l1) <= view.mincorner) {
            if (withmsg)                                    {
                printmsg(TXT_TPCUBENOTCONVEX, nc->no, newpnum);
            }
    ...
    for (j = 0; j < 6; j++) {
        ...
        if ( fabs( SCALAR(&l1,
                          &l2) ) <= view.minweirdwall * LENGTH(&l1) *
            LENGTH(&l2) ) {
            for (l1n = 0; l1n < 3; l1n++) {
                l1.x[l1n] = nc->d.c->p[wallpts[j][3]]->d.p->x[l1n] -
            ...
            if ( fabs( SCALAR(&l1,
                ...
                if (withmsg)
                {
                    printmsg(TXT_TPWEIRDWALL, j, nc->no);
                }
...
int connectsides(struct node *n, int i) {
    ...
    for (cn = l->cubes.head, sn = NULL, min_d = view.maxconndist * 4.0;
         cn->next != NULL;
         cn = cn->next) {
        if (cn == n) {
            continue;
...
void growshrink(struct node **ps, int *pntnos, int groworshrink) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < pntnos[0]; j++) {
            center.x[i] += ps[pntnos[j + 1]]->d.p->x[i];
        }
    ...
    for (j = 0; j < pntnos[0]; j++) {
        for (i = 0; i < 3; i++) {
            ps[pntnos[j +
                      1]]->d.p->x[i] =
                (ps[pntnos[j + 1]]->d.p->x[i] - center.x[i]) *
                (groworshrink ? view.distscala : 1
                                        / view.distscala) + center.x[i];
        }
    ...
    if (!ok) {
        for (j = 0; j < pntnos[0]; j++) {
            *ps[pntnos[j + 1]]->d.p = oldpnts[j];
        }
    }
    else {
        for (j = 0; j < pntnos[0]; j++) {
            newcorners(ps[pntnos[j + 1]]);
        }
    }
}
...
void delflickeringlight(struct lightsource *ls) {
    ...
    for (ne = ls->effects.head; ne->next != NULL; ne = ne->next) {
        for (nfl = ne->d.lse->cube->d.c->fl_lights.head->next; nfl != NULL;
             nfl = nfl->next) {
                        ...
                        float depth)
{
        ...
        for (j = 0; j < 3; j++)                      {
            for (i = 0, d.x[j] = 0; i < 4; i++) {
                d.x[j] += (c->d.c->p[wallpts[wallnum][i]]->d.p->x[j] -
                           c->d.c->p[wallpts[oppwalls[wallnum]][3 -
                                                                i]]->d.p->x[j
                           ]) / 4.0;
            }
        }
    ...
    for (j = 0; j < 3; j++) {
        d.x[j] = (n1.x[j] + n2.x[j]) / 2;
    }

    /* Assign the points. The new cube is the same way numbered as the
       old is. */
    for (i = 0; i < 4; i++) {
        nc->p[wallpts[oppwalls[wallnum]][3 -
                                         i]] = c->d.c->p[wallpts[wallnum][i]];
        ...
        for (j = 0; j < 3; j++) {
            lp->p.x[j] =
                nc->p[wallpts[oppwalls[wallnum]][3 - i]]->d.p->x[j] + d.x[j];
        }
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < 4; j++) {
            nc->walls[i]->corners[j] = w ==
                                       NULL ? stdcorners[j] : w->corners[j];
            nc->walls[i]->p[j] = nc->p[wallpts[i][j]];
            nc->walls[i]->tagged[j] = NULL;
        }
    ...
    if (cubes == NULL && pts == NULL) {
        ...
        if (l->exitcube != NULL && l->exitcube->no == c->no && l->exitwall ==
            wallnum) {
            l->exitcube = NULL;
        }
...
void makesidestdshape(struct cube *cube, int w) {
    ...
    for (i = 0; i < 3; i++) {
        for (pn = 0; pn < 4; pn++) {
            d[pn].x[i] = p[pn]->x[i] - c.x[i];
        }
    }

    /* using stdcubepts[4+p] because they are numbered in the right way:
       corner 0=point 4 1=5 2=6 3=7 */
    for (pn = 0, a = 0.0; pn < 4; pn++) {
        a += stdcubepts[4 + pn].x[0] * SCALAR(&d[pn], &x) +
        stdcubepts[4 + pn].x[1] * SCALAR(&d[pn], &y);
    }

    if (fabs(a) > 0.01) {
        for (pn = 0, b = 0.0; pn < 4; pn++) {
            b += stdcubepts[4 + pn].x[0] * SCALAR(&d[pn], &y) -
    ...
    for (pn = 0, a = b = 0.0; pn < 4; pn++) {
        for (i = 0; i < 3; i++) {
            np1[pn].x[i] = d[pn].x[i]
                           - ( stdcubepts[4 +
                                          pn].x[0] * cos(phi) -
                              stdcubepts[4 + pn].x[1] * sin(phi) ) * x.x[i]
                           - ( stdcubepts[4 +
                                          pn].x[0] * sin(phi) +
                              stdcubepts[4 + pn].x[1] * cos(phi) ) * y.x[i];
            np2[pn].x[i] = d[pn].x[i]
                           + ( stdcubepts[4 +
                                          pn].x[0] * cos(phi) -
                              stdcubepts[4 + pn].x[1] * sin(phi) ) * x.x[i]
    ...
    for (pn = 0; pn < 4; pn++) {
        ...
        if ( !testpnt(cube->p[wallpts[w][pn]]) ) {
            break;
        }
    }
    ...
    if (pn != 4) {
        ...
        for (pn = 0; pn < 4; pn++) {
            newcorners(cube->p[wallpts[w][pn]]);
        }
    }
}
