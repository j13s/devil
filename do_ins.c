...
void dec_newlevel(int ec) {
    ...
    const char *pigname = init.d_ver >=
                          d2_10_sw ? "GROUPA.256" : "DESCENT.256";
...
void dec_makemacro(int ec) {
    ...
    checkmem( view.pcurrcube->d.c->tagged =
                 addheadnode(&l->tagged[tt_cube], view.pcurrcube->no,
                             view.pcurrcube) );
...
void dec_savemacro(int ec) {
    ...
    if ( ( fname = getfilename(&init.macropath, view.pcurrmacro->filename,
        ...
        if ( !savelevel(fname, view.pcurrmacro, 0, 1, init.d_ver, 0) ) {
            ...
            printmsg(TXT_MACROSAVED, view.pcurrmacro->fullname, fname);
        }
...
void dec_loadmacro(int ec) {
    ...
    if ( ( fname = getfilename(&init.macropath, NULL,
                               ec == ec_readdbbfile ? "BLK" : init.levelext,
                               TXT_LOADMACRO, 0) ) !=
        NULL ) {
...
void dec_insert(int ec) {
    float scale = 1.0;
    int warn = 0, i, pn, special =
        (ec == ec_insspecial || ec == ec_insfastspecial),
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            if (special) {
                if ( !getfloat(TXT_GETSCALE, NULL, 4, 1, &scale) ) {
                    return;
                }
            ...
            if (insertmacro(view.pcurrmacro, fast, scale) != 0) {
                ...
                for (n = l->cubes.head; n->no < view.pcurrmacro->cubes.size;
                     n = n->next) {
                    tag(tt_cube, n);
                }

                for (n = l->doors.head; n->no < view.pcurrmacro->doors.size;
                     n = n->next) {
                    tag(tt_door, n);
                }

                for (n = l->things.head; n->no < view.pcurrmacro->things.size;
                     n = n->next) {
                    tag(tt_thing, n);
                }
        ...
        case tt_wall:
            ...
            if (view.pcurrcube->d.c->nc[view.currwall] != NULL) {
                ...
                if (view.pcurrcube->d.c->d[view.currwall] != NULL) {
                    printmsg(TXT_DOORINTHEWAY, view.currwall,
                             view.pcurrcube->no);
                    return;
                }
            ...
            if (view.pcurrcube->d.c->nc[view.currwall] == NULL) {
                if (view.pcurrcube->d.c->d[view.currwall] != NULL) {
                    ...
                    checkmem( n =
                                 insertcube(NULL, NULL, view.pcurrcube,
                                            view.currwall, -1.0) );

                    if (fast) {
                        makesidestdshape(n->d.c, view.currwall);
                    }
            ...
            if (l->pts.size > MAX_DESCENT_VERTICES && warn) {
                waitmsg(TXT_INSTOOMANYPTS);
            }
        ...
        case tt_edge:
            ...
            if (fast) {
                for (n = l->tagged[tt_edge].head; n->next != NULL;
                     n = n->next) {
                    pn = wallpts[(n->no % 24) / 4][(n->no % 24) % 4];

                    for (i = 0; i < 3; i++) {
                        if (n->d.n->d.c->d[wallno[pn][0][i]] != NULL) {
                            break;
                        }
                    }
            ...
            for (i = 0; i < 3; i++) {
                if (view.pcurrcube->d.c->d[wallno[pn][0][i]] != NULL) {
                    if (!fast)
                    {
                        printmsg(TXT_DOORINTHEWAY, wallno[pn][0][i],
                                 view.pcurrcube->no);
                    }
            ...
            if (i == 3) {
                insertpnt(view.pcurrcube, pn);
            }

            if (fast) {
                printmsg(TXT_TAGGEDPOINTSINSERTED);
            }
            else {
        ...
        case tt_thing:
            ...
            if ( l->things.size == MAX_DESCENT_OBJECTS &&
                !yesnomsg(TXT_INSTOOMANYTHINGS) ) {
                return;
            }
        ...
        case tt_door:
            ...
            if ( (init.d_ver >= d2_10_sw ||
               ...
               && view.pcurrcube->d.c->d[view.currwall] == NULL ) {
                ...
                printmsg(view.pcurrcube->d.c->d[view.currwall] ?
            ...
            if (warn && l->doors.size > MAX_DESCENT_WALLS) {
                printmsg(TXT_INSTOOMANYDOORS);
            }

            break;

        default:
            printmsg(TXT_NOINSERTMODE, init.bnames[view.currmode]);
    }
...
void dec_delete(int ec) {
    ...
    if ( view.askone && ec == ec_delete && !yesnomsg(TXT_REALLYDELONE) ) {
        return;
    }
    else
    if ( view.asktagged && ec == ec_deletefast &&
        l->tagged[view.currmode].size != 0
       && !yesnomsg(TXT_REALLYDELTAGGED, l->tagged[view.currmode].size) ) {
        return;
    }
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_cube].tail->prev; n != NULL;
                     n = n->prev) {
                    for (k = 0; k < 6; k++) {
        ...
        case tt_wall:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_wall].head->next; n != NULL;
                     n = n->next) {
                    if (n->prev->d.n->d.c->nc[n->prev->no % 6] == NULL) {
                        if (connectsides(n->prev->d.n,
                                         n->prev->no % 6) > 0) {
                ...
                if (connectsides(view.pcurrcube,
                                 view.currwall) < 0 && ec !=
                    ec_deletefast)
                {
                    printmsg(TXT_NOCONNCUBEFOUND, view.pcurrcube->no,
                             view.currwall);
                }
            }
            else if (ec != ec_deletefast) {
                printmsg(TXT_CUBEALREADYCONN, view.currwall,
                         view.pcurrcube->no);
                return;
            }
        ...
        case tt_thing:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_thing].tail->prev; n != NULL;
                     n = n->prev) {
                    if (l->things.size > 1) {
                        ...
                        if (tn->d.t->nc) {
                            for (n2 = tn->d.t->nc->d.c->things.head->next;
                                 n2 != NULL; n2 = n2->next) {
                                if (n2->prev->d.t == tn->d.t) {
                                    freenode(&tn->d.t->nc->d.c->things,
                                             n2->prev,
                                             NULL);
                                }
                            }
                        ...
                        printmsg(TXT_CANTKILLLASTTHING);
                    }
                }
            }
                ...
                if (tn->d.t->nc) {
                    for (n2 = tn->d.t->nc->d.c->things.head->next; n2 != NULL;
                         n2 = n2->next) {
                        if (n2->prev->d.t == tn->d.t) {
                            freenode(&tn->d.t->nc->d.c->things, n2->prev,
                                     NULL);
                        }
                    }
                }
        ...
        case tt_door:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_door].tail->prev; n != NULL;
                     n = n->prev) {
                    if (n->next->d.n->d.d->d &&
                        n->next->d.n->d.d->d->d.d->tagged == n) {
                        n = n->prev;
                    }
        ...
        default:
            printmsg(TXT_NODELETEMODE, init.bnames[view.currmode]);
    }
}
...
void dec_deletespecial(int ec) {
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            if ( !yesnomsg(TXT_KILLALL, l->tagged[tt_cube].size,
                           l->tagged[tt_door].size,
                           l->tagged[tt_thing].size) ) {
                return;
            }
                ...
                if (view.pcurrcube->d.c->nc[view.currwall] == NULL
                   && n->d.c->nc[l->tagged[tt_wall].head->no % 6] == NULL) {
                    if ( connectcubes(NULL, view.pcurrcube, view.currwall,
                                      l->tagged[tt_wall].head->d.n,
                                      l->tagged[tt_wall].head->no % 6) ) {
                        untagall(tt_wall);
                        view.pcurrwall = NULL;
                        l->levelsaved = l->levelillum = 0;
                    }
        ...
        default:
            printmsg(TXT_NOSDELETEMODE, init.bnames[view.currmode]);
    }
}
...
void dec_splitcube(int ec) {
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0, c.x[j] = 0; i < 8; i++)                           {
            c.x[j] += p[i].x[j] = cube->p[i]->d.p->x[j];
        }
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 8; i++) {
            np[i].x[j] = (p[i].x[j] - c.x[j]) * 0.5 + c.x[j];
        }
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < 6; j++) {
            if (newc[i]->d.c->nc[j] == view.pcurrcube) {
                break;
            }
        }
    ...
    for (j = 0; j < 6; j++) {
        for (i = 0; i < 6; i++) {
            if (i != wno[j] && i != oppwalls[wno[j]] &&
                newc[j]->d.c->nc[i] == NULL) {
