    ...
    for (i = 0; i < 3; i++) {
        if (c->d.c->walls[wallno[pointnum][0][i]] != NULL) {
            newwall_offset(c->d.c, c->d.c->walls[wallno[pointnum][0][i]],
                           (wallno[pointnum][1][i] + 2) & 3,
                           (wallno[pointnum][1][i] + 3) & 3);
        }
    }
}
                    ...
                    short int *new_coords)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        paraxis.x[j] =
            w->p[(corner + 2) & 3]->d.p->x[j] - w->p[corner]->d.p->x[j];
    ...
    for (j = 0; j < 3; j++) {
        paraxis.x[j] -= x * turnaxis.x[j];
    }

    normalize(&paraxis);

    for (j = 0; j < 2; j++) {
        t_uv[j] = w->corners[t_pe].x[j] - w->corners[t_ps].x[j];
    }
    ...
    for (j = 0; j < 2; j++) {
        new_coords[j] = w->corners[t_ps].x[j] + x * t_uv[j] + y * ot_uv[j];
    }
}
...
/*                      0 1 2 3 corner
   ...
   wall 5 (back):        3 2 1 0 */
    ...
    w->corners[op1].x[0] =
        (short int)(ex_x * LENGTH(&p[0]) / 640.0) + p1xoffs;
    w->corners[op1].x[1] =
        (short int)(ex_y * LENGTH(&p[0]) / 640.0) + p1yoffs;
    ...
    w->corners[op3].x[0] =
        (short int)( (ey_x * LENGTH(&p[1]) + ex_x * x) / 640.0 ) + p1xoffs;
    w->corners[op3].x[1] =
        (short int)( (ey_y * LENGTH(&p[1]) + ex_y * x) / 640.0 ) + p1yoffs;

    /* now the last point: */
    /* turn last point into the plane of the three others: */
    /* get the coordsystem of the turn axis */
    for (j = 0; j < 3; j++) {
        turnaxis.x[j] = w->p[op3]->d.p->x[j] - w->p[op1]->d.p->x[j];
    }
    ...
    for (j = 0; j < 3; j++) {
        p2.x[j] = w->p[op0]->d.p->x[j] - op.x[j];
    }

    lx = SCALAR(&p2, &turnaxis);

    for (j = 0; j < 3; j++) {
        p2.x[j] -= turnaxis.x[j] * lx;
    }
    ...
    for (j = 0; j < 3; j++) {
        p1.x[j] = op.x[j] + turnaxis.x[j] * x - p2.x[j] * y -
                  w->p[op0]->d.p->x[j];
    }
    ...
    y = SCALAR(&p1, &p[1]);
    w->corners[op2].x[0] =
        (short int)( (ex_x * x + ey_x * y) / 640.0 ) + p1xoffs;
    w->corners[op2].x[1] =
        (short int)( (ex_y * x + ey_y * y) / 640.0 ) + p1yoffs;
    centerx = centery = 0;
...
void recalcwall(struct cube *c, int wallno) {
    ...
    if (c->walls[wallno] != NULL) {
        newwall_offset(c, c->walls[wallno], 0, 1);
    }
}
...
int findnbwalltoline(struct node *c, struct cube *nc, int w, int wp1,
                     int wp2)
{
    int i, j;
    ...
    for (i = 0; i < 6; i++) {
        if ( nc->walls[i] != NULL &&
            (nc->nc[i] == NULL || nc->nc[i]->no != c->no) ) {
            for (j = 0; j < 4; j++) {
                if ( nc->walls[i]->p[j]->no == c->d.c->p[wallpts[w][wp1]]->no
                   && (nc->walls[i]->p[(j + 1) & 0x3]->no ==
                       c->d.c->p[wallpts[w][wp2]]->no
                      || nc->walls[i]->p[(j - 1) & 0x3]->no ==
                       c->d.c->p[wallpts[w][wp2]]->no) ) {
                    return i;
                    break;
    ...
    for (run = walls.head; run->next != NULL; run = run->next) {
        ...
        for (wp1 = 0; wp1 < 4; wp1++) {
            ...
            for (i = 0, w2p1 = w2p2 = -1; i < 4; i++) {
                if (w->p[i]->no == w1->p[wp1]->no) {
                    w2p1 = i;
                }
                else if (w->p[i]->no == w1->p[wp2]->no) {
                    w2p2 = i;
                }
            }
...
int findpath(struct list *dest) {
    ...
    for (s = 0, nc = NULL; s < 6; s++) {
        if (cc->d.c->nc[s] != NULL && (snc = cc->d.c->nc[s]->d.c->tagged) !=
            NULL) {
            if (nc != NULL) {
