...
void dec_calctxts(int ec) {
    ...
    switch (view.currmode) {
        ...
        default:
            printmsg(TXT_NOTXTCALC, init.bnames[view.currmode]);
    }
...
const char *structnames[7] = {
    "Cube", "Side", "Corner", "Thing", "Door", "Switch",
    "Producer"
};
void dec_enterdata(int ec) {
    ...
    whichstruct.select_lroutine = whichstruct.select_rroutine =
                                      ed_changestruct;
    checkmem( b[0] = w_addstdbutton(ow, w_b_choose, 0, 0, w_xwininsize(ow),
                                    w_ywininsize(ow) / 2, "Structure",
                                    &whichstruct, 1) );
    offset.d_xsize = -1;
    offset.max_length = 3;
    ...
    checkmem( b[1] =
                 w_addstdbutton(ow, w_b_string, b[0]->xpos, b[0]->ypos +
                                b[0]->ysize,
                                w_xwininsize(ow) / 2, w_ywininsize(ow) -
                                b[0]->ypos - b[0]->ysize, "Offset", &offset,
                                1) );
    checkmem( b[2] = w_addstdbutton(ow, w_b_string, b[1]->xpos + b[1]->xsize,
                                    b[0]->ypos + b[0]->ysize,
                                    w_xwininsize(ow) - b[1]->xpos -
                                    b[1]->xsize,
                                    w_ywininsize(ow) - b[0]->ypos -
                                    b[0]->ysize, "Data", &data, 1) );
...
void add_correlem(struct corridor *c, struct node *cube, int first) {
    ...
    for (i = 0, n = cube, p = 0; i < c->num_cubes && n->next != NULL;
         i++, n = n->next) {
        ce->cubes[i] = n;
        ...
        for (wp = 0; wp < 4; wp++) {
            ...
            if (n2->next == NULL) {
                ...
                ce->pnts[p] =
                    n->d.c->p[wallpts[first ? oppwalls[c->front_wall[i]] :
                                      c->front_wall[i]][wp]]->d.p;

                for (j = 0; j < 3; j++) {
                    ce->center.x[j] += ce->pnts[p]->x[j];
                }
                     ...
                     struct point *coords)
{
    ...
    for (i = 0; i < c->num_pnts; i++) {
        for (j = 0; j < 3; j++)                                  {
            p.x[j] = ce->pnts[i]->x[j] - ce->center.x[j];
        }
...
int connect_cube_layer(struct corridor *c, struct node *start) {
    ...
    for (n = start, i = 0; n->next != NULL && i < c->num_cubes;
         n = n->next, i++) {
        for (j = 0; j < 6; j++) {
            if (c->connected[i * 6 + j] >= 0 && n->d.c->nc[j] == NULL) {
                for (n2 = start, k = 0;
                     n2->next != NULL && k < c->connected[i * 6 + j] / 6;
                     n2 = n2->next,
                     k++) {
                    ;
                }

                if (n2->next && n2->d.c->nc[c->connected[i * 6 + j] % 6] ==
                    NULL) {
                    if ( !connectcubes(&c->points, n, j, n2,
                                       c->connected[i * 6 + j] % 6) ) {
                        return 0;
                    }
                }
...
int new_cube_layer(struct corridor *c) {
    ...
    checkmem( fcube = insertcube(&c->cubes, &c->points,
                                 c->elements.tail->d.ce->cubes[0],
                                 c->front_wall[0], c->depth) );

    for (i = 1; i < c->num_cubes; i++) {
        checkmem( insertcube(&c->cubes, &c->points,
                             c->elements.tail->d.ce->cubes[i],
                             c->front_wall[i], c->depth) );
    }

    if ( !connect_cube_layer(c, fcube) ) {
        for (n = fcube; n->next != NULL; n = n->next) {
            deletecube(&c->cubes, &c->points, n);
        }
...
struct node *recalc_correlem(struct corridor *c, struct node *n,
                             struct point *a, struct point *coords,
                             struct point *prev_coords)
{
    int i, j;
    ...
    for (i = 0; i < c->num_pnts; i++) {
        ...
        for (j = 0; j < 3; j++) {
            n->d.ce->pnts[i]->x[j] += n->d.ce->center.x[j];
        }
    }
...
void corr_get_pnt(struct point *ci, float t, struct point *p) {
    ...
    for (i = 0; i < 3; i++) {
        p->x[i] = ci[0].x[i] + ci[1].x[i] * t + ci[2].x[i] * t * t +
                  ci[3].x[i] * t * t * t;
    }
}
...
void corr_get_gradient(struct point *ci, float t, struct point *p) {
    ...
    for (i = 0; i < 3; i++) {
        p->x[i] = ci[1].x[i] + 2 * ci[2].x[i] * t + 3 * ci[3].x[i] * t * t;
    }
}
...
void corr_init_coeff(struct point *ci, struct track *st, struct track *et) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        ci[2].x[i] = 3 * et->x.x[i] - 3 * st->x.x[i] - 2 *
                     st->coords[2].x[i] * st->l_c2 -
                     et->coords[2].x[i] * et->l_c2;
        ci[3].x[i] = -2 * et->x.x[i] + 2 * st->x.x[i] + st->coords[2].x[i] *
                     st->l_c2 +
                     et->coords[2].x[i] * et->l_c2;
    }
}
...
float corr_get_twist(struct track *t, struct point *ref_coords) {
    ...
    if (fabs( twist = SCALAR(&n, &t->coords[2]) ) < 0.0001) {
        twist = 0;
    }
    else {
                          ...
                          struct node *start)
{
    ...
    for (i = 0; i < 3; i++) {
        dx1.x[i] = et->x.x[i] - st->x.x[i];
    }

    if (LENGTH(&dx1) < 2 * c->depth) {
        return 0;
    }
...
/* n=recalc_correlem(c,start,&st->x,st->coords,old_coords); */
    ...
    for (tn = curve.head->next, twist = 0.0; tn != NULL;
         tn = tn->next, twist += d_twist) {
        ...
        for (i = 0; i < 3; i++) {
            old_coords[i] = tn->prev->d.p[1 + i];
        }

        if (tn->prev->no > 0) {
            d_twist /= 2;
        }
        else if (tn->prev->no < 0) {
            d_twist *= 2;
        }
    ...
    if (start == NULL ||
        ( n =
             recalc_correlem(c, n, &et->x, et->coords,
                             old_coords) ) == NULL) {
        return NULL;
    }
...
int recalc_corridor(struct corridor *c) {
    ...
    for (tn = c->tracking.head->next; tn->next != NULL; tn = tn->next) {
        for (i = 0; i < 3; i++) {
            d.x[i] = tn->d.ct->x.x[i] - tn->prev->d.ct->x.x[i];
        }

        if (LENGTH(&d) < 3 * c->depth) {
            return 0;
        }

        if ( ( cn =
                  recalc_track(c, tn->prev->d.ct, tn->d.ct, cn) ) == NULL ) {
            return 0;
        }
    }

    for (kn = cn->next->next; kn != NULL; kn = kn->next) {
        kill_corr_elem(c, kn->prev);
    }
...
int movecorr(struct point *add, union move_params *params) {
    ...
    switch (c->b_stdform->d.ls->selected) {
        ...
        case 1:
            ...
            for (i = 0; i < 3; i++) {
                t->coords[2].x[i] = mt->coords[2].x[i] = d.x[i] / t->l_c2;
            }
        ...
        case 2:
            ...
            for (i = 0; i < 3; i++) {
                r.x[i] = add->x[i] - f * t->coords[2].x[i];
            }

            if (ct[4]->fixed) { /* ->t=mt1 */
                ct[1]->l_c2 = ct[3]->l_c2 = ct[4]->l_c2 = ct[4]->l_c2 +
                                                          CIRCLE_FACTOR *
                                                          ( SCALAR(&r,
                                                                   &t->coords
                                                                   [1]) <
             ...
             union move_params *params)
{
    ...
    if (c->b_stdform->d.ls->selected != 0) {
        return 0;
    }
                      ...
                      struct point *nv)
{
    ...
    if ( cube->d.c->nc[wall] != NULL || cube->d.c->walls[wall] == NULL
       || !testtag(tt_wall, cube, wall) ) {
        return;
    }

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 3; j++)                        {
            t.x[j] = cube->d.c->p[wallpts[wall][i]]->d.p->x[j] - a->x[j];
        }
        ...
        if (f != 0.0) {
            for (j = 0; j < 3; j++) {
                cube->d.c->p[wallpts[wall][i]]->d.p->x[j] -= f * nv->x[j];
            }
        }
    }

    my_assert( c->front_wall =
                  REALLOC( c->front_wall, (++c->num_cubes) * sizeof(int) ) );
    ...
    for (i = 0; i < 4; i++) {
        insertpnt(n, wallpts[oppwalls[wall]][i]);
    }
    ...
    for (i = 0; i < 4; i++) {
        cube->d.c->walls[wall]->corners[i] = w.corners[i];
    }
    ...
    for (i = 0; i < 4; i++) {
        if (cube->d.c->nc[nb_sides[wall][i]]
           && ( nbwall =
                   findnbwalltoline(cube,
                                    cube->d.c->nc[nb_sides[wall][i]]->d.c,
                                    wall, i, (i + 1) & 3) ) != -1) {
            make_start_cubes(c, cube->d.c->nc[nb_sides[wall][i]], nbwall, a,
                             nv);
        }
    }
}
...
void init_connected(struct corridor *c) {
    ...
    for (cube = l->tagged[tt_cube].head, cubeno = 0; cube->next != NULL
        ...
        for (i = 0; i < 6; i++) {
            ...
            if ( (nc = cube->d.n->d.c->nc[i]) != NULL &&
                testtag(tt_cube, nc) ) {
                for (nbwall = 0; nbwall < 6; nbwall++) {
                    if (nc->d.c->nc[nbwall] == cube->d.n) {
                        break;
                    }
                }

                my_assert(nbwall < 6);
                c->connected[cubeno * 6 +
                             i] = nc->d.c->tagged->no * 6 + nbwall;
            }
        }
    }
}
...
int make_start_corridor(struct corridor *c) {
    ...
    for (i = 0; i < 3; i++) {
        m1.x[i] =
            cube->p[wallpts[view.currwall][(view.curredge +
                                            1) & 3]]->d.p->x[i] -
            a.x[i];
        m2.x[i] =
            cube->p[wallpts[view.currwall][(view.curredge -
...
void change_num_tp(int insdel) {
    ...
    *new_tp = *l->cur_corr->tracking.head->d.ct;
    ...
    for (length = 0.0, n = l->cur_corr->tracking.head->next; n->next != NULL;
         n = n->next) {
        length += n->d.ct->length;
    }

    for (s = last_s = 0.0, n = l->cur_corr->tracking.head->next;
        ...
        for (t = 0.0; t <= 1.0; t += dt) {
            ...
            if ( s - last_s >= length /
                (l->cur_corr->tracking.size - 1 + insdel) ) {
                checkmem( new_tp = MALLOC( sizeof(struct track) ) );
                checkmem( addnode(&new_track, -1, new_tp) );
                new_tp->fixed = 0;
                new_tp->length = s;
                new_tp->l_c2 = LENGTH(&dx);
                new_tp->old_twist = et->old_twist *
                                    (s - st->length) / et->length;
...
void corr_setend(struct w_button *b) {
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (i = 0; i < 4; i++) {
            end->x.x[j] += c->p[wallpts[view.currwall][i]]->d.p->x[j];
        }
    ...
    for (i = 0; i < 3; i++) {
        end->coords[1].x[i] = e2.x[i] - f * e1.x[i];
    }
    ...
    switch (b->d.ls->selected) {
        case 0: /* user defined */
            ...
            if (st->x.x[0] == et->x.x[0] && st->x.x[1] == et->x.x[1]
               && st->x.x[2] == et->x.x[2]) {
                for (i = 0; i < 3; i++) {
                    et->x.x[i] -= et->coords[2].x[i] * MIN_CUBEDEPTH *
                                  65536.0;
                }
            }
        ...
        case 1: /* straight */
            ...
            if (c->tracking.size > 2) {
                for (n = c->tracking.head->next->next; n->next != NULL;
                     n = n->next) {
                    FREE(n->prev->d.ct);
            ...
            for (i = 0; i < 3; i++) {
                et->x.x[i] = st->x.x[i] + et->length * st->coords[2].x[i];
            }
        ...
        case 2: /* circle */
            ...
            if (c->tracking.size > 2) {
                for (n = c->tracking.head->next; n->next != NULL; n =
                         n->next) {
                    FREE(n->prev->d.ct);
                    killnode(&c->tracking, n->prev);
                }
            ...
            for (i = 0; i < 3; i++) {
                for (j = 0; j < 3; j++) {
                    et->coords[j].x[i] = st->coords[j].x[i];
                }

                mt1->coords[0].x[i] = mt2->coords[0].x[i] =
                                          mt3->coords[0].x[i] =
                                              st->coords[0]
                                              .x[i];
                ...
                mt1->x.x[i] = st->x.x[i] +
                              (-st->coords[1].x[i] +
                               st->coords[2].x[i]) * st->l_c2 * CIRCLE_FACTOR;
                mt2->x.x[i] = st->x.x[i] - st->coords[1].x[i] * st->l_c2 *
                              2 * CIRCLE_FACTOR;
                mt3->x.x[i] = st->x.x[i] +
                              (-st->coords[1].x[i] -
                               st->coords[2].x[i]) * st->l_c2 * CIRCLE_FACTOR;
...
struct w_window init_corr_win = {
    0, 400, 100, 150, 107, -1, -1, 0, NULL, 0, NULL,
    wb_drag | wb_close | wb_shrink, wr_normal, NULL, NULL, closerout_corr_win,
    NULL
};
#define NUM_STDFORMS 3
const char *stdforms_txt[NUM_STDFORMS] = {
    TXT_CUSTOM, TXT_STRAIGHT, TXT_CIRCLE
};
void init_corr_window(struct corridor *c, const char *levname) {
    ...
    checkmem( c->b_ok =
                 w_addstdbutton(c->win, w_b_press, 0, 0, xs / 2, -1, TXT_OK,
                                &ok, 1) );
    cancel = ok;
    cancel.l_routine = cancel.r_routine = corr_cancel;
    checkmem( c->b_cancel =
                 w_addstdbutton(c->win, w_b_press, xs / 2, 0, xs - xs / 2, -1,
                                TXT_CANCEL, &cancel, 1) );
    stdform.d_xsize = -1;
    ...
    checkmem( c->b_stdform =
                 w_addstdbutton(c->win, w_b_choose, 0, c->b_ok->ysize,
...
void dec_makeedgecoplanar(int ec) {
    ...
    for (n = l->tagged[tt_edge].head->next; n != NULL; n = n->next) {
        ...
        if (i == 4) {
            /* only one edge is tagged on this side. move it into the plane of
               the
               three others. */
            for (j = 0; j < 4; j++) {
            ...
            for (j = 0; j < 3; j++) {
                p[0]->x[j] = fr * r.x[j] + fs * s.x[j] + a.x[j];
            }
